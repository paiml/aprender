...
- 3 of those moves broke this table's own rule 1: #3421 and #3428 (must-carry in #3999 and #4001) and #3997 (named by five epics). They were **reverted** to their pre-move state, each with a comment, and are now decisions. A 4th, #3845 (the same defect as the DECIDE #3978), was reverted in quorum round 6. Round 7 also reverted #3714 on the 07:12Z "fold in MoE" ruling, but that ruling was superseded at 11:44Z (MoE → 0.71.0, on #3987), so round 8 re-applied the 0.71.0 move. **Net moves: 198**, plus 9 corrections re-moved (see Corrections).
...
**NOT applied (left untouched, pending): 40 rows.** These are the 13 `DECIDE` rows (operator: the original 8, #3951, and 4 of the reverted rows), 7 `close?`, 6 `verify-close`, 13 `0.69.1 (in flight)` (including the relabelled #3839) and 1 `none (pinned)`. No issue was closed by the triage itself; 4 duplicates were closed on the operator's separate ruling (see Corrections).

**Open issues per milestone after the moves, reverts, corrections and duplicate closes** (`gh issue list --milestone`, 2026-09-23 ~18:45 CEST; other sessions keep moving issues, so expect drift): 0.70.0 130 · 0.71.0 106 · 0.72.0 52 · 0.73.0 34 · 0.74.0 19 · 0.75.0 5.

0.70.0 also holds the 4 untouched `close?` rows, the 3 reverted DECIDE rows (#3421, #3428, #3845), the relabelled #3839, and issues other sessions added after this table's snapshot (#3988's moves, new filings). The milestone API's `open_issues` also counts pull requests.
...
4. **Not a milestone.** `0.69.1 (in flight)` (checked on no-milestone rows at first; the quorum extended it to a 0.70.0 row carrying a current operator 0.69.1 ruling, #3839; a ruling is checked for a LATER superseding ruling before use): worked in the current train, closes at the tag or carries to 0.70. `verify-close`: the release tree already implements it -- cited in code on origin/release/0.69.1-batch-2 @ c619dddd4 (not yet on main). `close?`: a superseded or stale epic/row. `none (pinned)`: a standing coordination thread.
...
- 0.70.0 today (178): 0.69.1 (in flight): 1 · close?: 4 · DECIDE 0.71.0/0.72.0: 1 · DECIDE 0.73.0/0.74.0: 2 · 0.70.0: 76 · 0.71.0: 44 · 0.72.0: 14 · 0.73.0: 16 · 0.74.0: 15 · 0.75.0: 5
...
| #3839 | 0.69.1 (in flight) | operator ruling 2026-09-23 07:12Z (recorded on #3714), "fold in 88% coverage": a 0.69.1 blocker. Its MoE half was superseded at 11:44Z; the coverage half was not. Never moved; relabelled after the #4024 quorum | Coverage owes 0.69.1: 87.82% vs floor 88, and COVERAGE_EXCLUDE_REGEX has described a different tree since Apri |
...
| #3714 | 0.71.0 | qwen3moe CUDA: the operator ruled MoE OUT of 0.69.1 and into 0.71.0 (#3987, 2026-09-23 11:44Z, "yes push"), superseding the 07:12Z "fold in MoE". Round 7 reverted this move on the older ruling in error; round 8 re-applied it | qwen3moe (Qwen3-30B-A3B, Qwen3-Coder-30B-A3B, Q4_K_M) has NO working CUDA path: apr run --gpu falls back to CP |
...
| #3987 | DECIDE 0.70.0/0.71.0/0.72.0 | qwen3moe verbs: named must-carry in #3998, #3994 and #4000 -- operator picks one **[operator ruled 0.71.0 on #3987 at 11:44Z ("yes push"): recommend confirming 0.71.0]** | qwen3moe CUDA reaches run+qa only: chat rc 8, serve 501/500, code rc 1 — serve loads no mapped model and would |
