...
**1 partially shipped** (001 Phase 1 of 4 — round-trip gate ENFORCED;
crash safety / recall threshold / cold-open latency pending Phases
2-4); 2 recommended with **pre-authored gates** (005, 006); 1
...
|---|---|---|---|---|
| **HELIX-IDEA-001** (Persistent HNSW — Phase 1) | `contracts/apr-hnsw-persistence-v1.yaml` | ACTIVE | `FALSIFY-HNSW-PERSIST-001` → `crates/aprender-core/tests/falsify_hnsw_persist_001.rs::reopen_top_k_matches_in_memory` <br>(Phases 2-4 pending — see §2.1 phase column) | `crates/aprender-contracts/tests/apr_hnsw_persistence_contract.rs` (6 assertions) |
...
|---|---|---|---|
| HELIX-IDEA-001 (Persistent HNSW) | `contracts/apr-hnsw-persistence-v1.yaml` | **v1.0.0 ACTIVE — Phase 1 (gate 001) shipped; Phases 2-4 (gates 002/003/004) pending amendment** | §2.1: 4 gates (`FALSIFY-HNSW-PERSIST-001..004`) |
...
**Status:** **Shipped (Phase 1 — round-trip)**; FALSIFY-HNSW-PERSIST-001
ENFORCED. Phases 2 (crash safety, gate 002), 3 (recall threshold,
gate 003), and 4 (cold-open latency, gate 004) ship as separate PRs
amending the contract. See §1.4 forward obligations.
**Contract:** `contracts/apr-hnsw-persistence-v1.yaml` v1.0.0 (ACTIVE).
**Effort:** Medium total; Phase 1 fit in one commit (~150 LOC + tests).
