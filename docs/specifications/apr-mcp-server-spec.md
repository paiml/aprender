...
**Headline count (v2 — 2026-04-18 session-row correction)**: of 20 Claude-Code feature categories, `apr code` has **6 SHIPPED** (built-in tools R/W/E/G/G/B, `-p` basics, REPL Phase 1, CLAUDE.md+APR.md, slash-command scaffold, **session store + auto-resume** — falsified into SHIPPED from PARTIAL on 2026-04-18 after reading `agent/session.rs:32-201`), **6 PARTIAL**, and **8 missing or not wired** (Status line flipped UNKNOWN → NONE same day). Parity is achievable — batuta's agent runtime is functionally complete — but the surface delta is larger than "just MCP client + Claude proxy". New tickets opened by this audit (unprefixed = to be filed): `PMAT-CODE-{SLASH,MEMORY,HOOKS,SKILLS,SPAWN,WORKTREE,CUSTOM-AGENTS,PERMISSIONS,WEB-TOOLS,NOTEBOOK,MONITOR,SESSION,CONFIG-LADDER,PLUGINS,IDE,NON-INTERACTIVE,REPL-PHASE2,STATUS-LINE,ORG-POLICY}-PARITY-001`.
