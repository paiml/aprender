...
**Headline count (v5.1 — 2026-04-18 post PMAT-CODE-ORG-POLICY-001 — EPIC CLOSEABLE)**: 21 rows total, row-level status is ground truth per `pv check-parity`: **14 SHIPPED** (claude-md-memory, builtin-tools-rwegs, session-management, **mcp-client**, **slash-commands**, **hooks**, **subagent-spawn**, **custom-agents**, **builtin-tools-web**, **skills**, **worktree-isolation**, **permission-modes**, **status-line**, **managed-org-policy**), **3 PARTIAL** (configuration-ladder, non-interactive-mode, keyboard-shortcuts), **4 NONE** (builtin-tool-notebook, builtin-tool-monitor, plugins-marketplace, ide-integrations). **All 4 P0 tickets + 5 P1 tickets + 2 P2 tickets closed in this cycle**: MCP-CLIENT, SLASH-PARITY, HOOKS, SPAWN-PARITY, CUSTOM-AGENTS, WEB-TOOLS, SKILLS, WORKTREE, PERMISSIONS, STATUS-LINE, ORG-POLICY. **All P0 and P1 priority buckets are empty** — the only remaining P1 is the scope-reduced session-management follow-up (which tracks `--continue`/`--fork`/`--name` flags, not a ground-truth row). **Closure status: BOTH CONDITIONS MET** — SHIPPED cap (14 ≥ 9) MET; MISSING cap (4 ≤ 4) MET. **Epic PMAT-CODE-PARITY-MATRIX-001 is CLOSEABLE.** FALSIFY-CODE-PARITY-005 passes. Remaining 4 MISSING rows (notebook, monitor, plugins, IDE) are P2 deferred surfaces tracked for future milestones but not epic-blocking. Follow-up tickets filed by this audit: `PMAT-CODE-{MEMORY,HOOKS-RUNTIME,SKILLS-TOOLS,SLASH-SKILLS,TASK-ASYNC,WORKTREE-RUNTIME,CUSTOM-AGENTS-TOOLS,CUSTOM-AGENTS-INIT,PERMISSIONS-RUNTIME,WEB-SEARCH,NOTEBOOK,MONITOR,SESSION,CONFIG-LADDER,PLUGINS,IDE,NON-INTERACTIVE,REPL-PHASE2,STATUS-LINE-RUNTIME,ORG-POLICY-RUNTIME}-PARITY-001`.
