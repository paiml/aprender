...
- `contracts/apr-code-parity-v1.yaml` — falsifiable encoding of the 21-row `apr code` ↔ Claude Code parity matrix; 5 falsification gates incl. row-by-row mechanical audit + headline aggregate invariant + prose↔YAML drift check; `status: ACTIVE` as of 2026-04-18 revision 5.0 (`pv validate` green via in-tree `aprender-contracts-cli`; epic PMAT-CODE-PARITY-MATRIX-001; current counts **13 SHIPPED / 3 PARTIAL / 5 NONE over 21 rows** per `pv check-parity` after **all 4 P0 + 5 P1 + 1 P2 tickets closed** — MCP-CLIENT, SLASH-PARITY, HOOKS, SPAWN-PARITY, CUSTOM-AGENTS, WEB-TOOLS, SKILLS, WORKTREE, PERMISSIONS, STATUS-LINE; SHIPPED side of closure threshold (≥9) is now MET, only MISSING side (≤4) still needs 1 more flip to close — all remaining MISSING rows are P2 deferred). Both gates now green: `pv validate` enforces SCHEMA; `pv check-parity` executes each row's `cross_check_command` + enforces headline aggregate invariant (FALSIFY-CODE-PARITY-002). Neither is a bash script.
**References**:
...
**Headline count (v5.0 — 2026-04-18 post PMAT-CODE-STATUS-LINE-001)**: 21 rows total, row-level status is ground truth per `pv check-parity`: **13 SHIPPED** (claude-md-memory, builtin-tools-rwegs, session-management, **mcp-client**, **slash-commands**, **hooks**, **subagent-spawn**, **custom-agents**, **builtin-tools-web**, **skills**, **worktree-isolation**, **permission-modes**, **status-line**), **3 PARTIAL** (configuration-ladder, non-interactive-mode, keyboard-shortcuts), **5 NONE** (builtin-tool-notebook, builtin-tool-monitor, plugins-marketplace, ide-integrations, managed-org-policy). **All 4 P0 tickets + 5 P1 tickets + 1 P2 ticket closed in this cycle**: MCP-CLIENT, SLASH-PARITY, HOOKS, SPAWN-PARITY, CUSTOM-AGENTS, WEB-TOOLS, SKILLS, WORKTREE, PERMISSIONS, STATUS-LINE. **All P0 and P1 priority buckets are empty** — the only remaining P1 is the scope-reduced session-management follow-up (which tracks `--continue`/`--fork`/`--name` flags, not a ground-truth row). **Closure status: SHIPPED cap (≥9) MET; MISSING cap (≤4) still NOT MET** — 5 NONE rows remain (all P2 deferred surfaces), so epic PMAT-CODE-PARITY-MATRIX-001 needs exactly **1 more MISSING→{SHIPPED,PARTIAL} flip** to close. Candidates: notebook / monitor / plugins / ide / org-policy (all P2). Follow-up tickets filed by this audit: `PMAT-CODE-{MEMORY,HOOKS-RUNTIME,SKILLS-TOOLS,SLASH-SKILLS,TASK-ASYNC,WORKTREE-RUNTIME,CUSTOM-AGENTS-TOOLS,CUSTOM-AGENTS-INIT,PERMISSIONS-RUNTIME,WEB-SEARCH,NOTEBOOK,MONITOR,SESSION,CONFIG-LADDER,PLUGINS,IDE,NON-INTERACTIVE,REPL-PHASE2,STATUS-LINE-RUNTIME,ORG-POLICY}-PARITY-001`.
