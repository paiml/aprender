...
**Headline count (v4.2 — 2026-04-18 post PMAT-CODE-SLASH-PARITY-001)**: 21 rows total, row-level status is ground truth per `pv check-parity`: **5 SHIPPED** (claude-md-memory, builtin-tools-rwegs, session-management, **mcp-client**, **slash-commands**), **5 PARTIAL** (subagent-spawn, builtin-tools-web, configuration-ladder, non-interactive-mode, keyboard-shortcuts), **11 NONE** (hooks, skills, worktree-isolation, custom-agents, permission-modes, builtin-tool-notebook, builtin-tool-monitor, plugins-marketplace, ide-integrations, status-line, managed-org-policy). The earlier v2 "6/6/8 over 20" was a category-level aggregation; `pv check-parity` now enforces row-level truth. Parity remains achievable — batuta's agent runtime is functionally complete — with two P0 tickets remaining (HOOKS, SPAWN-PARITY) before crossing the ≥9 SHIPPED / ≤4 MISSING closure threshold. New tickets opened by this audit (unprefixed = to be filed): `PMAT-CODE-{MEMORY,HOOKS,SKILLS,SPAWN,WORKTREE,CUSTOM-AGENTS,PERMISSIONS,WEB-TOOLS,NOTEBOOK,MONITOR,SESSION,CONFIG-LADDER,PLUGINS,IDE,NON-INTERACTIVE,REPL-PHASE2,STATUS-LINE,ORG-POLICY}-PARITY-001`.
