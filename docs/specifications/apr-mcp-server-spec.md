...
| **MCP client** (`.mcp.json` loader + stdio/sse/http) | PARTIAL | `McpClientTool` + `StdioMcpTransport` + `discover_mcp_tools` exist (`agent/tool/mcp_client.rs:262`) but `build_code_tools` never calls `register_mcp_tools` (`cli/agent_helpers.rs:219`). SSE/HTTP transports absent. `.mcp.json` loader absent (only TOML `AgentManifest.mcp_servers` is read) | **PMAT-CODE-MCP-CLIENT-001** (already registered) |
...
**Headline count (v3 — 2026-04-18 post-harness correction)**: 21 rows total (builtin-tools is split into 4 tool-surface rows in the contract), row-level status is now the ground truth: **3 SHIPPED** (claude-md-memory, builtin-tools-rwegs, session-management), **7 PARTIAL** (slash-commands, subagent-spawn, mcp-client, builtin-tools-web, configuration-ladder, non-interactive-mode, keyboard-shortcuts), **11 NONE** (hooks, skills, worktree-isolation, custom-agents, permission-modes, builtin-tool-notebook, builtin-tool-monitor, plugins-marketplace, ide-integrations, status-line, managed-org-policy). The earlier v2 "6/6/8 over 20" was a fuzzier category-level aggregation; `pv check-parity` forces the row-level count to be authoritative. Parity is achievable — batuta's agent runtime is functionally complete — but the surface delta is larger than "just MCP client + Claude proxy". New tickets opened by this audit (unprefixed = to be filed): `PMAT-CODE-{SLASH,MEMORY,HOOKS,SKILLS,SPAWN,WORKTREE,CUSTOM-AGENTS,PERMISSIONS,WEB-TOOLS,NOTEBOOK,MONITOR,SESSION,CONFIG-LADDER,PLUGINS,IDE,NON-INTERACTIVE,REPL-PHASE2,STATUS-LINE,ORG-POLICY}-PARITY-001`.
