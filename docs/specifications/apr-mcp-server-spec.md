...
- `contracts/apr-code-parity-v1.yaml` — falsifiable encoding of the 21-row `apr code` ↔ Claude Code parity matrix; 5 falsification gates incl. row-by-row mechanical audit + headline aggregate invariant + prose↔YAML drift check; `status: ACTIVE` as of 2026-04-18 revision 4.5 (`pv validate` green via in-tree `aprender-contracts-cli`; epic PMAT-CODE-PARITY-MATRIX-001; current counts **8 SHIPPED / 4 PARTIAL / 9 NONE over 21 rows** per `pv check-parity` after all 4 P0 tickets — MCP-CLIENT, SLASH-PARITY, HOOKS, SPAWN-PARITY — plus first P1 CUSTOM-AGENTS closed; epic is 1 SHIPPED row away from closure threshold ≥9/≤4). Both gates now green: `pv validate` enforces SCHEMA; `pv check-parity` executes each row's `cross_check_command` + enforces headline aggregate invariant (FALSIFY-CODE-PARITY-002). Neither is a bash script.
**References**:
...
**Headline count (v4.5 — 2026-04-18 post PMAT-CODE-CUSTOM-AGENTS-001)**: 21 rows total, row-level status is ground truth per `pv check-parity`: **8 SHIPPED** (claude-md-memory, builtin-tools-rwegs, session-management, **mcp-client**, **slash-commands**, **hooks**, **subagent-spawn**, **custom-agents**), **4 PARTIAL** (builtin-tools-web, configuration-ladder, non-interactive-mode, keyboard-shortcuts), **9 NONE** (skills, worktree-isolation, permission-modes, builtin-tool-notebook, builtin-tool-monitor, plugins-marketplace, ide-integrations, status-line, managed-org-policy). **All 4 P0 tickets (MCP-CLIENT, SLASH-PARITY, HOOKS, SPAWN-PARITY) are CLOSED**, and the first P1 of this wave (CUSTOM-AGENTS) also landed — the gap to ≥9 SHIPPED / ≤4 MISSING is now 1 row (any one of skills/worktree-isolation/permission-modes promoted, or any PARTIAL→SHIPPED flip). New tickets opened by this audit (unprefixed = to be filed): `PMAT-CODE-{MEMORY,HOOKS-RUNTIME,SKILLS,TASK-ASYNC,WORKTREE,CUSTOM-AGENTS-TOOLS,CUSTOM-AGENTS-INIT,PERMISSIONS,WEB-TOOLS,NOTEBOOK,MONITOR,SESSION,CONFIG-LADDER,PLUGINS,IDE,NON-INTERACTIVE,REPL-PHASE2,STATUS-LINE,ORG-POLICY}-PARITY-001`.
