...
- `contracts/apr-code-parity-v1.yaml` — falsifiable encoding of the 21-row `apr code` ↔ Claude Code parity matrix; 5 falsification gates incl. row-by-row mechanical audit + headline aggregate invariant + prose↔YAML drift check; `status: ACTIVE` as of 2026-04-18 revision 4.4 (`pv validate` green via in-tree `aprender-contracts-cli`; epic PMAT-CODE-PARITY-MATRIX-001; current counts **7 SHIPPED / 4 PARTIAL / 10 NONE over 21 rows** per `pv check-parity` after all 4 P0 tickets — MCP-CLIENT, SLASH-PARITY, HOOKS, SPAWN-PARITY — closed; P0 bucket now empty). Both gates now green: `pv validate` enforces SCHEMA; `pv check-parity` executes each row's `cross_check_command` + enforces headline aggregate invariant (FALSIFY-CODE-PARITY-002). Neither is a bash script.
**References**:
...
**Headline count (v4.4 — 2026-04-18 post PMAT-CODE-SPAWN-PARITY-001)**: 21 rows total, row-level status is ground truth per `pv check-parity`: **7 SHIPPED** (claude-md-memory, builtin-tools-rwegs, session-management, **mcp-client**, **slash-commands**, **hooks**, **subagent-spawn**), **4 PARTIAL** (builtin-tools-web, configuration-ladder, non-interactive-mode, keyboard-shortcuts), **10 NONE** (skills, worktree-isolation, custom-agents, permission-modes, builtin-tool-notebook, builtin-tool-monitor, plugins-marketplace, ide-integrations, status-line, managed-org-policy). **All 4 P0 tickets (MCP-CLIENT, SLASH-PARITY, HOOKS, SPAWN-PARITY) are now CLOSED** — the gap to ≥9 SHIPPED / ≤4 MISSING is now 2 rows of P1 work (promote 2 of {skills, worktree-isolation, custom-agents, permission-modes} into SHIPPED or PARTIAL and flip 1 PARTIAL like builtin-tools-web up). Parity is now a P1-only problem. New tickets opened by this audit (unprefixed = to be filed): `PMAT-CODE-{MEMORY,HOOKS-RUNTIME,SKILLS,TASK-ASYNC,WORKTREE,CUSTOM-AGENTS,PERMISSIONS,WEB-TOOLS,NOTEBOOK,MONITOR,SESSION,CONFIG-LADDER,PLUGINS,IDE,NON-INTERACTIVE,REPL-PHASE2,STATUS-LINE,ORG-POLICY}-PARITY-001`.
