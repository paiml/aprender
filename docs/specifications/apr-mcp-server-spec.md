...
- `contracts/apr-code-parity-v1.yaml` — falsifiable encoding of the 21-row `apr code` ↔ Claude Code parity matrix; 5 falsification gates incl. row-by-row mechanical audit + headline aggregate invariant + prose↔YAML drift check; `status: ACTIVE` as of 2026-04-18 revision 4.8 (`pv validate` green via in-tree `aprender-contracts-cli`; epic PMAT-CODE-PARITY-MATRIX-001; current counts **11 SHIPPED / 3 PARTIAL / 7 NONE over 21 rows** per `pv check-parity` after all 4 P0 tickets — MCP-CLIENT, SLASH-PARITY, HOOKS, SPAWN-PARITY — plus P1 CUSTOM-AGENTS + P1 WEB-TOOLS + P1 SKILLS + P1 WORKTREE closed; SHIPPED side of closure threshold (≥9) is now MET, only MISSING side (≤4) still needs 3 more flips). Both gates now green: `pv validate` enforces SCHEMA; `pv check-parity` executes each row's `cross_check_command` + enforces headline aggregate invariant (FALSIFY-CODE-PARITY-002). Neither is a bash script.
**References**:
...
**Headline count (v4.8 — 2026-04-18 post PMAT-CODE-WORKTREE-001)**: 21 rows total, row-level status is ground truth per `pv check-parity`: **11 SHIPPED** (claude-md-memory, builtin-tools-rwegs, session-management, **mcp-client**, **slash-commands**, **hooks**, **subagent-spawn**, **custom-agents**, **builtin-tools-web**, **skills**, **worktree-isolation**), **3 PARTIAL** (configuration-ladder, non-interactive-mode, keyboard-shortcuts), **7 NONE** (permission-modes, builtin-tool-notebook, builtin-tool-monitor, plugins-marketplace, ide-integrations, status-line, managed-org-policy). **All 4 P0 tickets + 4 P1 tickets closed in this cycle**: MCP-CLIENT, SLASH-PARITY, HOOKS, SPAWN-PARITY, CUSTOM-AGENTS, WEB-TOOLS, SKILLS, WORKTREE. **Closure status: SHIPPED cap (≥9) MET; MISSING cap (≤4) NOT YET MET** — 7 NONE rows remain, so epic PMAT-CODE-PARITY-MATRIX-001 cannot close until 3 more MISSING→{SHIPPED,PARTIAL} flips land. Priority order: permission-modes (P1), then 2 of the 6 P2 deferred rows (status-line / notebook / monitor / plugins / ide / org-policy). Follow-up tickets filed by this audit: `PMAT-CODE-{MEMORY,HOOKS-RUNTIME,SKILLS-TOOLS,SLASH-SKILLS,TASK-ASYNC,WORKTREE-RUNTIME,CUSTOM-AGENTS-TOOLS,CUSTOM-AGENTS-INIT,PERMISSIONS,WEB-SEARCH,NOTEBOOK,MONITOR,SESSION,CONFIG-LADDER,PLUGINS,IDE,NON-INTERACTIVE,REPL-PHASE2,STATUS-LINE,ORG-POLICY}-PARITY-001`.
