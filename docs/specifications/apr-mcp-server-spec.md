...
| **Session management** (`--resume`, `--continue`, transcript store) | PARTIAL | `apr code --resume [id]` flag exists (`commands_enum.rs:503`); session infra at `agent/session.rs`. Missing: `--continue` shorthand, durable JSONL transcript store matching Claude's `~/.claude/sessions/` layout, `--fork-session`, `--name` | PMAT-CODE-SESSION-PARITY-001 (new) |
...
| **Status line** (model / mode / cost / branch) | UNKNOWN | TUI code at `agent/tui.rs` / `tui_render.rs`; whether a persistent status line is wired needs one pass of code review | to-be-filed once status-line surface falsified |
...
**Headline count**: of 20 Claude-Code feature categories, `apr code` has **5 SHIPPED** (built-in tools R/W/E/G/G/B, `-p` basics, REPL Phase 1, CLAUDE.md+APR.md, slash-command scaffold), **7 PARTIAL**, and **8 missing or not wired**. Parity is achievable — batuta's agent runtime is functionally complete — but the surface delta is larger than "just MCP client + Claude proxy". New tickets opened by this audit (unprefixed = to be filed): `PMAT-CODE-{SLASH,MEMORY,HOOKS,SKILLS,SPAWN,WORKTREE,CUSTOM-AGENTS,PERMISSIONS,WEB-TOOLS,NOTEBOOK,MONITOR,SESSION,CONFIG-LADDER,PLUGINS,IDE,NON-INTERACTIVE,REPL-PHASE2,ORG-POLICY}-PARITY-001`.
