...
- **Client direction (PMAT-CODE-MCP-CLIENT-001, CLOSED 2026-04-18):** Our own `apr code` agent loads external MCP servers as tool providers via `manifest.mcp_servers[]` (TOML) and the existing `McpClientTool`, mirroring Claude Code's own MCP-client capability. `register_mcp_client_tools` (`agent/code.rs:371`) wires discovery + registration after `build_code_tools`. Follow-up PMAT-CODE-MCP-JSON-LOADER-001 (P2, deferred) adds the project-root `.mcp.json` file loader.
...
|--------|-----------------|-------------------------|--------|
| **Slash commands** (`/help` etc.) | PARTIAL (11 of ~21) | `agent/repl.rs` `SlashCommand` enum: `Help, Quit, Cost, Context, Model, Compact, Clear, Session, Sessions, Test, Quality` (+ `Unknown(name)`). Missing: `/config, /mcp, /review, /memory, /permissions, /hooks, /init, /resume, /add-dir, /agents, /debug, /rename, /upgrade` | PMAT-CODE-SLASH-PARITY-001 (new) |
| **CLAUDE.md / APR.md memory** | SHIPPED | `agent/code.rs:218` loads `APR.md` then `CLAUDE.md` from `--project`. No `@import` syntax yet, no `~/.claude/CLAUDE.md` user-level fallback, no auto-memory directory | PMAT-CODE-MEMORY-PARITY-001 (new) — add `@file` imports + user-level `~/.config/apr/APR.md` fallback + auto-memory at `~/.config/apr/projects/<hash>/memory/` |
| **Hooks** (`SessionStart`, `PreToolUse`, `PostToolUse`, `UserPromptSubmit`, `Stop`) | NONE in `apr code` | `grep -r "Hook\|pre_tool\|post_tool" crates/aprender-orchestrate/src/agent/` returns zero hits (only unrelated git pre-commit in `falsification/jidoka.rs:51`). Needs new `agent/hooks.rs` + manifest `[[hooks]]` table + exit-code→block/warn/allow semantics | PMAT-CODE-HOOKS-001 (new) |
| **Skills** (`/name` user-invocable + auto-loaded) | NONE | No `agent/skill*.rs`, no `.apr/skills/` loader. Claude's `SKILL.md` frontmatter (`description`, `when_to_use`, `allowed-tools`, `context: fork`) has no aprender analogue. Closest existing capability is `manifest.tools` allowlist, which is coarse | PMAT-CODE-SKILLS-001 (new) |
| **Subagent / Task tool** (Agent, TaskCreate/Get/List/Update) | PARTIAL | `SpawnTool` exists (`agent/tool/spawn.rs`), registered via `register_spawn_tool` (`cli/agent_helpers.rs:179`) when `Capability::Spawn` is declared in manifest. No TaskCreate/Get/List/Update surface yet, no worktree-isolated Agent variant | PMAT-CODE-SPAWN-PARITY-001 (new) — lift `SpawnTool` into the default registry; add `TaskRegistry` surface mirroring TaskCreate/Get/List |
| **Worktree isolation** (`EnterWorktree`/`ExitWorktree`) | NONE | No `worktree` module in `agent/`. Claude Code's `--worktree <name>` and `EnterWorktree` tool have no analogue. Would plug into existing `SpawnTool` as `SpawnMode::Worktree` | PMAT-CODE-WORKTREE-001 (new) |
| **Custom agents** (`.claude/agents/*/AGENT.md`) | NONE | No agent-discovery from a filesystem tree. Existing `AgentManifest` is TOML + single-file-scoped. No markdown-plus-frontmatter definition format | PMAT-CODE-CUSTOM-AGENTS-001 (new) |
| **MCP client** (`.mcp.json` loader + stdio/sse/http) | SHIPPED (core) — `.mcp.json` project-root loader MISSING | `register_mcp_client_tools` (`agent/code.rs:371-394`) now called from `cmd_code` right after `build_code_tools`, closing the gap that kept `McpClientTool` dormant. Under `--features code` (which enables `agents-mcp`), any `manifest.mcp_servers[]` is auto-discovered via `discover_mcp_tools()` and each discovered tool registered into the agent's tool registry. Unit test `test_register_mcp_client_tools_noop_when_empty` verifies idempotent no-op when no servers declared. Remaining scope: separate `.mcp.json` project-root loader (currently servers live under `AgentManifest.mcp_servers[]` in the TOML manifest) | PMAT-CODE-MCP-CLIENT-001 **CLOSED** 2026-04-18; follow-up `PMAT-CODE-MCP-JSON-LOADER-001` (P2, deferred) |
| **Permission modes** (`default`/`plan`/`acceptEdits`/`auto`/`bypassPermissions`) | NONE as modes | Per-tool capability allowlisting exists via `Capability` + manifest, but no request-time mode toggle analogous to Shift+Tab cycling. No `--permission-mode` flag on `apr code` | PMAT-CODE-PERMISSIONS-001 (new) |
| **Built-in tools — read/write/edit/grep/glob/shell** | SHIPPED | `build_code_tools` (`agent/code.rs:360-387`) registers `FileReadTool`, `FileWriteTool`, `FileEditTool`, `GlobTool`, `GrepTool`, `ShellTool`, `MemoryTool`, `PmatQueryTool`, `RagTool` (feature-gated). Rough 1:1 with Claude's Read/Write/Edit/Glob/Grep/Bash | — |
| **Built-in tools — WebFetch / WebSearch** | PARTIAL-INFRA | `NetworkTool` (`agent/tool/network.rs:20`) with host allowlisting + `BrowserTool` (`agent/tool/browser.rs:29`) gated behind `agents-browser` exist in the codebase but are **not** registered by `build_code_tools` | PMAT-CODE-WEB-TOOLS-001 (new) — wire `NetworkTool`/`BrowserTool` into default registry behind privacy-tier gate |
...
| **Configuration** (`~/.claude/settings.json` + precedence ladder) | PARTIAL | TOML `AgentManifest` path via `--manifest`, but no user-level `~/.config/apr/settings.json` with managed-org → user → project → local ladder. Env-var `APR_CODE_MODEL` honored in fallback chain (contract `apr-claude-proxy-v1.yaml`) | PMAT-CODE-CONFIG-LADDER-001 (new) |
...
| **Non-interactive mode (`-p` + `--output-format`)** | PARTIAL | `apr code -p <prompt>` implemented (`commands_enum.rs:509-515`, `code.rs::run_single_prompt`, `max_iterations.min(10)`). Missing: `--output-format json\|text\|stream-json`, `--input-format stream-json`, `--max-budget-usd`, newline-delimited streaming event objects | PMAT-CODE-NON-INTERACTIVE-PARITY-001 (new) |
| **Keyboard shortcuts** (Shift+Tab mode cycle, `!` prefix, `@path`) | PARTIAL | REPL Phase 1 shipped with slash commands + Ctrl+C (per `apr-code-feasibility-falsification.md` § GAP-A). Shift+Tab permission cycling, `!bash` inline, `@path` file-mention not yet implemented | PMAT-CODE-REPL-PHASE2-001 (new) |
| **Status line** (model / mode / cost / branch) | NONE | Falsified 2026-04-18: `grep -rin "status.?line\|StatusLine\|render_status\|status_bar\|statusline" crates/aprender-orchestrate/src/agent/` returned zero matches; the two files that matched the regex (`stack/publish_status/tests_extended.rs`, `bug_hunter/model_parity.rs`) are unrelated publish/parity reporting, not a REPL status line. TUI code exists for other surfaces but does not render a Claude-style model/mode/cost/branch indicator | PMAT-CODE-STATUS-LINE-001 (new, deferred) |
| **Managed org policy** (`/etc/claude-code/CLAUDE.md`) | NONE | No enforced-settings tier in the config ladder today | PMAT-CODE-ORG-POLICY-001 (new, deferred) |

**Headline count (v4 — 2026-04-18 post PMAT-CODE-MCP-CLIENT-001)**: 21 rows total (builtin-tools is split into 4 tool-surface rows in the contract), row-level status is ground truth per `pv check-parity`: **4 SHIPPED** (claude-md-memory, builtin-tools-rwegs, session-management, **mcp-client**), **6 PARTIAL** (slash-commands, subagent-spawn, builtin-tools-web, configuration-ladder, non-interactive-mode, keyboard-shortcuts), **11 NONE** (hooks, skills, worktree-isolation, custom-agents, permission-modes, builtin-tool-notebook, builtin-tool-monitor, plugins-marketplace, ide-integrations, status-line, managed-org-policy). The earlier v2 "6/6/8 over 20" was a category-level aggregation; `pv check-parity` now enforces row-level truth. Parity remains achievable — batuta's agent runtime is functionally complete — with three P0 tickets remaining (SLASH-PARITY, HOOKS, SPAWN-PARITY) before crossing the ≥9 SHIPPED / ≤4 MISSING closure threshold. New tickets opened by this audit (unprefixed = to be filed): `PMAT-CODE-{SLASH,MEMORY,HOOKS,SKILLS,SPAWN,WORKTREE,CUSTOM-AGENTS,PERMISSIONS,WEB-TOOLS,NOTEBOOK,MONITOR,SESSION,CONFIG-LADDER,PLUGINS,IDE,NON-INTERACTIVE,REPL-PHASE2,STATUS-LINE,ORG-POLICY}-PARITY-001`.

**Falsification cross-checks used in this audit:**
- `pmat query "build_code_tools" --include-source` confirmed 9 tools registered at `agent/code.rs:360-387`
