...
**Headline count (v4.3 — 2026-04-18 post PMAT-CODE-HOOKS-001)**: 21 rows total, row-level status is ground truth per `pv check-parity`: **6 SHIPPED** (claude-md-memory, builtin-tools-rwegs, session-management, **mcp-client**, **slash-commands**, **hooks**), **5 PARTIAL** (subagent-spawn, builtin-tools-web, configuration-ladder, non-interactive-mode, keyboard-shortcuts), **10 NONE** (skills, worktree-isolation, custom-agents, permission-modes, builtin-tool-notebook, builtin-tool-monitor, plugins-marketplace, ide-integrations, status-line, managed-org-policy). Parity remains achievable — batuta's agent runtime is functionally complete — with one P0 ticket remaining (SPAWN-PARITY) before the gap to ≥9 SHIPPED / ≤4 MISSING is pure P1/P2 work. New tickets opened by this audit (unprefixed = to be filed): `PMAT-CODE-{MEMORY,HOOKS-RUNTIME,SKILLS,SPAWN,WORKTREE,CUSTOM-AGENTS,PERMISSIONS,WEB-TOOLS,NOTEBOOK,MONITOR,SESSION,CONFIG-LADDER,PLUGINS,IDE,NON-INTERACTIVE,REPL-PHASE2,STATUS-LINE,ORG-POLICY}-PARITY-001`.
