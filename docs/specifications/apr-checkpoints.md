...
|----|------|----------|-------------|
| F-CKPT-001 | Adapter completeness | P0 | APR file contains ALL `lora_a`/`lora_b` tensors + classifier head. `count == 2 + 2 × lora_layers.len()` |
| F-CKPT-002 | Schema version present | P0 | `__checkpoint__.schema_version` exists in metadata |
| F-CKPT-003 | No training state in adapter | P1 | `.adapter.apr` has zero `__training__.*` tensors |
| F-CKPT-004 | Training state completeness | P0 | `.ckpt.apr` has optimizer moments for every trainable param. `count(optim) == 2 × count(trainable)` |
| F-CKPT-007 | Write NaN/Inf check | P0 | No tensor contains NaN or Inf values at write time |
| F-CKPT-008 | Write shape validation | P0 | Every tensor shape matches model config dimensions |
| F-CKPT-009 | Atomic writes | P0 | Write to `.tmp`, fsync, rename. Crash never corrupts existing checkpoint |
| F-CKPT-010 | Dtype consistency | P1 | All tensors use F32 unless explicitly quantized. Quantized checkpoints record scheme in metadata |
| F-CKPT-015 | Canonical ordering | P2 | Tensors sorted lexicographically by name |
| F-CKPT-017 | Provenance hash | P2 | `base_model_hash` matches canonical hash (§10.1) |

### 13.2 Read-Side Contracts (consuming checkpoints)

| ID | Name | Severity | Description |
|----|------|----------|-------------|
| F-CKPT-005 | Resume equivalence | P0 | Restored training produces val_loss within 0.05 of continuous run |
| F-CKPT-006 | Data hash verification | P1 | Resume rejects mismatched training data. Hard error; `--allow-data-mismatch` overrides |
| F-CKPT-011 | CRC32 verification | P0 | Reader verifies CRC32 on open. Corrupt file → hard error |
| F-CKPT-012 | SafeTensors header validation | P0 | Reader validates SafeTensors header before parsing tensors |
| F-CKPT-013 | Post-load NaN scan | P0 | Reader rejects tensors containing NaN/Inf after load |
| F-CKPT-014 | Shape-config validation | P0 | Loaded tensor shapes match model architecture config |
| F-CKPT-016 | Backward compatibility | P1 | Inference reader loads `.ckpt.apr`, skipping `__training__.*` |
| F-CKPT-018 | SafeTensors round-trip | P2 | `import(export(import(dir))) == import(dir)` (bit-identical) |
