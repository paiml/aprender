...
| G7 | **Fleet at 09:00Z:** 16 `intel-clean-room` runners (15 busy, `intel-clean-room-16` is `perf-solo`), `gx10-blackwell` **idle**, `yoga-gpu` **idle**. No lambda runner is registered. **12:30Z (§11):** two **disposable Docker runners** are online beside them — `gx10-eph` (group `gpu-nodes`) and `yoga-eph` (group `gpu-x86`), labels `self-hosted,Linux,<ARM64\|X64>,cuda,gpu,<box>,ephemeral,docker`: one `docker run --rm --gpus all` container per job, `~/models:ro` and the host CUDA mounted read-only (infra#500; persistent-runner retirement is infra#501). A workflow lands on them by adding `ephemeral, docker` to `runs-on`; aprender's CUDA release lane opts in with #3086 (builds inside `rust:1.93.0-bullseye`, **glibc 2.31 floor**) | `gh api orgs/paiml/actions/runners` `[V]` at both times |
...
| G9 | **yoga runs nothing but the release lane.** Its only jobs are `binary-release.yml` `build-apr-cuda` (x86_64) and `smoke-cuda`. Labels `self-hosted,Linux,X64,gpu,cuda,yoga,ada`; runner group `gpu-x86` (id 5): `allows_public_repositories=true`, `restricted_to_workflows=false`, `selected_workflows=[]` (re-measured 12:10Z after the `agy /teamwork-preview` review, §10 F1 — the 09:00Z list read showed `false`; the live value is `true`, so PR code from this **public** repo can already reach yoga and the group restricts it to no workflow) | `grep -n runs-on .github/workflows/*.yml`; `gh api orgs/paiml/actions/runner-groups` `[V]` |
...
| G14 | **Milestones:** `0.66.0` open 1 / closed 6 · `0.67.0` open 5 / closed 2 · `0.68.0` open 15 / closed 1 (the PP-066 rows R-0..R-8, C0-*, G-*, KEY, #2869 moved there 2026-09-09) · `0.69.0` (#6) and `0.70.0` (#7) **did not exist at discovery and were created by this ticket** (§10 F2). Open PRs: 33 | `gh api repos/paiml/aprender/milestones`; `gh pr list --state open` `[V]` |
...
|---|---|---|---|---|---|
| **67-A1** (P0-1, #3082) | 0.67 | `build-apr-cpu` matrix in `.github/workflows/binary-release.yml`: `cargo build --release -p apr-cli --bin apr --locked` on the **default features** (`crates/apr-cli/Cargo.toml` line 75: `hf-hub, safetensors-compare, inference, training, visualization, zram` — `cuda` is opt-in at line 89, so the default build is CPU-only `[V]`), `apr-<tag>-<target>-cpu.tar.gz` + `.sha256`, glibc floor printed to the job summary; the CPU lane follows #3086's shape — built inside `rust:1.93.0-bullseye` on the **ephemeral** runners (`ephemeral, docker` labels) so the asset's glibc floor is 2.31, not the host's; a **`smoke-cpu` job** on a GPU-less clean-room runner and on both GPU hosts proves each CPU asset runs (`apr --version`) **and is not a CUDA build** (`grep -ac libcuda.so apr` = 0 — the #3072 discriminator, whose cuda value is 1); `verify-cuda-assets` becomes **verify-apr-assets** and requires all **four** apr assets; a **pre-publish dogfood row** and a **release-criteria row** (C13 of `docs/specifications/PP-066-release-spec.md` §4, narrowed to Linux) so a tag cannot be promoted without them. Owed: scripts/check_release_assets.sh `<tag>` (exit 1 unless 4 apr + 4 sha256 + 8 pv assets; `--selftest` case table with a removed-asset RED row) | x86_64: yoga (or `intel-clean-room`); aarch64: gx10 | `gh release view v0.67.0 --json assets --jq '[.assets[].name] \| map(select(test("^apr-v0.67.0-(x86_64\|aarch64)-unknown-linux-gnu-(cuda\|cpu).tar.gz(.sha256)?$")))\| length'` = 8 | verify-apr-assets RED on a missing asset (observed once on the 0.66.0 backfill); dogfood row |
...
**This is a rescoping of the literal order, stated plainly (§10 F5):** the operator's words are *"100% and default"*, unconditional. This document applies "default" **per layer** and makes a kernel's default status conditional on parity **and** on not being slower than the production kernel. The reason is P-4 (correct before fast) and the withdrawn-claims record (`docs/BEATS.md`); the operator can restore the literal order with one comment on #3078 or #3079, and §11 will carry it. **What "default" means here, layer by layer** (decision D-2, §6):
...
**State (G7, G8):** gx10 is idle at PR time and runs ≈ 1 h/day of scheduled work, two thirds of it red. "Never idle" is a queue that is never empty, measured in job-minutes per day from the Actions API — not a load average (`[[feedback_high_load_is_intended_headroom]]`). **Which runner (G7, 12:30Z):** PR-time jobs on either GPU box target the **disposable** runner (`[self-hosted, Linux, ARM64, cuda, gx10, ephemeral, docker]` / `[…, X64, cuda, yoga, ephemeral, docker]`) — one container per job, gone afterwards; jobs that need a host daemon or host-installed comparators (cuda-nightly's Ollama, the 7B parity cells of F) stay on the persistent runners until infra#501 retires them, and every label set still names one box.
...
| Row | Train | Deliverable | Acceptance command | Gate |
|---|---|---|---|---|
| **67-D0** | 0.67 | **Security precondition before PR code executes on yoga:** runner group `gpu-x86` **already allows public repositories and restricts to no workflow** (G9, re-measured 12:10Z) — so the exposure exists today. The primary containment is the **disposable runner** (G7: `yoga-eph`, one `docker run --rm` per job, no persistent state) — PR-time jobs use it, never `yoga-gpu`; the group restriction is the second layer. Set `restricted_to_workflows=true` with `selected_workflows` = `ci.yml` (`cuda-unit`), `cuda-nightly.yml`, `binary-release.yml`, `qwen-story-daily.yml`; keep `pr-gate.yml`'s `authorize` job in front of any `pull_request` job that lands there; `[self-hosted, Linux, X64, cuda, yoga]` names one box | `gh api orgs/paiml/actions/runner-groups/5 --jq '{restricted_to_workflows, selected_workflows, allows_public_repositories}'` = `{true, [4 workflows], true}`; a dispatch smoke on yoga from this repo succeeds (`[U]` today) | infra |
...
| 67-E2 (P0-3, #3084) | 0.67 | **The merge queue mirrors the PR, never escalates to FULL on a moved main:** `merge_group` runs `reuse` (same tree) or the **same tier the PR ran** on the queue ref; `push` to main runs quick + guards; **FULL runs only in `coverage-nightly`/`full-nightly` and `scripts/dogfood.sh --phase pre-publish`** — decision D-1. Two rules close the hole the review named (§10 F4): (i) a selection **over the cap** (`CAP=3`, G5) runs the quick tier on touched ∪ tree-readers **plus `cargo check --workspace --all-targets --locked`** — compile-level integration of every reverse dependent, minutes not an hour (**Target** ≤ 8 min warm, `[U]` until the first run measures it); nothing else compiles the workspace at PR time — the reusable `ci / lint` and `ci / test` jobs are called with only `use_nextest: true`, so they lint and test the root facade `[V]`; (ii) a PR touching a **root manifest** (`Cargo.toml`, `Cargo.lock`, `rust-toolchain.toml`) keeps the FULL tier at PR and in the queue — dependency bumps are where compile-level integration breaks and the quick tier is blind to them. **Residual, named:** test *execution* of reverse dependents beyond the cap is deferred to the nightly FULL run (≤ 24 h) and to pre-publish — R2 | `bash scripts/ci_test_tier.sh --self-test` rows: `merge_group different tree, PR ran quick -> quick`, `merge_group different tree, PR ran full -> full`, `push -> quick`, `pull_request over cap -> quick+check-workspace`, `root manifest -> full`; a merge_group run of a quick PR ≤ 20 min | ci_test_tier case table |
...
| Row | Train | Deliverable | Host | Acceptance command | Gate |
|---|---|---|---|---|---|
| **67-F1** | 0.67 | **PP-26 witness green on gx10, honestly** (cuda-nightly is red on it today, G8): the logprob-margin instrument (master §12 row 22) classifies the c=16 parting. **Near-tie** (the parting slots sit inside the measured top-2 margin) → the witness threshold is set from the margin receipt and the change is documented beside it. **Defect** → the witness **stays RED** until W-H (68-F2) fixes the reduction order; 67-F1 then delivers the instrument only and the row's green moves to 68-F2. A goalpost is never moved to make a red light green (§10 F6); BSE-11 byte-compare green either way | gx10 | `gh run list --workflow cuda-nightly.yml --limit 2 --json conclusion` = success ×2 | cuda-nightly |
...
4. **Merge the bump, re-read the milestone, tag, release.** Once the bump PR has merged, and immediately before the tag, run `bash scripts/check_milestone_cut.sh X.Y.0` again. Exit ≠ 0 → `STOP tag`, no tag. The script names every open item, and an item reopened after the freeze shows up here: v0.68.0 was tagged with #3091 reopened 85 min earlier. This is the binding read; step 1's read is the early one. Then `gh release create vX.Y.0 --target <main tip>`; `binary-release.yml` fires on `release: published`: 4 `apr` + 4 `pv` builds, verify-apr-assets, smoke-cuda on yoga and gx10.
...
| R2 | **The FULL tier leaves PRs while the nightly FULL run is red** (G17); and test execution of reverse dependents beyond `CAP=3` is deferred to that nightly (§10 F4) | 67-E0 is a precondition; until it is green, D-1 is applied to the merge queue only and PRs keep today's tier — stated in the 0.67 epic, not assumed. Over-cap PRs compile the whole workspace at PR time (`cargo check --workspace --all-targets`); root-manifest PRs keep FULL |
...
## §10. Review record — `agy /teamwork-preview` (2026-09-10, agy 1.2.0)

**Provenance.** One `/teamwork-preview` lane dispatched through `paiml-agy-delegate` against HEAD `6fe3ab5c2` of the review worktree (conversation `41071d23-fa14-4132-ab0e-f5b945dcd99d`, `num_turns=1`, 218 s, schema-valid). **Fan-out unmeasured:** `fanout.sh` printed `children=unknown method=none label=unknown` — no teamwork project directory and no `.agents` newer than the launch appeared, so this is **one model answering once**, not a consensus, and the word is not used. Lane verdict **FAIL**, six findings; `lane-reduce.sh` exit 1 (not agreed). Two deviations the delegate measured and declared rather than hid: `--sandbox` was omitted although `writes=false`, because the sandboxed `gh api orgs/paiml/actions/runner-groups` returned `401 Requires authentication` while the unsandboxed call answered — and brief item 5 asked for exactly that endpoint; containment was the disposable worktree, a prompt that forbids writes, `lane-guard.sh --exec`, and HEAD + `git status --porcelain` snapshotted before and after (byte-identical, 0 lines). And a harness note was appended to the prompt because agy's shell tool does not inherit the launch cwd (bare `git rev-parse HEAD` fails under `/home/noah/.gemini` in both sandbox modes). The lane self-marked three findings `measured` with no tool transcript in its output; **every finding below was re-run by the orchestrator before a verdict**.

| # | Lane finding (severity as given) | Re-run | Verdict | Where applied |
|---|---|---|---|---|
| F1 | S2 — G9's `allows_public_repositories=false` for runner group `gpu-x86` is wrong; the API says `true` | `gh api orgs/paiml/actions/runner-groups/5` at 12:10Z → `allows_public_repositories=true, restricted_to_workflows=false, selected_workflows=[]` (the 09:00Z list read had shown `false`) | **Accepted.** The exposure is *worse* than written: PR code from this public repo can already reach yoga and no workflow restriction exists | G9; 67-D0 rewritten as "set `restricted_to_workflows=true` with four named workflows" |
| F2 | S2 — G14's "no 0.69.0 or 0.70.0 milestone exists" is false; both exist | `gh api repos/paiml/aprender/milestones` → #6 `0.69.0`, #7 `0.70.0` — **created by this ticket** after §0 was measured | **Accepted** as a stale-within-PR statement | G14 reworded: did not exist at discovery, created by PMAT-1097 |
| F3 | S1 — building `apr-cpu` on default features yields a CUDA binary because `cuda` is in `apr-cli`'s defaults; no `smoke-cpu` step | `git show origin/main:crates/apr-cli/Cargo.toml`: line 75 `default = ["hf-hub", "safetensors-compare", "inference", "training", "visualization", "zram"]`; `cuda` is a separate opt-in feature at line 89 | **Premise refuted** (the default build is CPU-only; the fix "match `nightly.yml`'s `--no-default-features --features inference`" would drop `training`, `hf-hub`, `zram` from the shipped CPU binary). **Remedy accepted:** a `smoke-cpu` job that runs each CPU asset and proves `grep -ac libcuda.so apr` = 0 | 67-A1 names the flags, the line numbers and the smoke |
| F4 | S1 — "merge_group never FULL" is unsafe: `CAP=3` means an over-cap PR's reverse dependents are never tested against the moved main | `scripts/gate_touched_crates.sh` `CAP=3` `[V]`; `.github/workflows/ci.yml` passes only `use_nextest: true` to `sovereign-ci.yml`, whose lint/test jobs default to the root package — nothing else compiles the workspace at PR time `[V]` | **Accepted, with a bounded remedy** that keeps the 20-minute budget: the queue mirrors the PR's tier; over-cap PRs add `cargo check --workspace --all-targets --locked` (compile-level integration); root-manifest PRs keep FULL; the residual (test *execution* beyond the cap, ≤ 24 h to the nightly) is named in R2 | 67-E2, R2, D-1 |
| F5 | S3 — D-2 silently rescopes the operator's unconditional "100 % and default" | D-2's dissent column and R5 already say it; the lane's point is that §2 B did not say it *at the point of use* (the delegate noted this too) | **Accepted** | §2 B opens with the rescoping stated plainly and the one-comment override path |
| F6 | S2 — "re-baseline the PP-26 witness at the classified value" moves the goalpost | reading; no command | **Accepted**: near-tie → threshold from the margin receipt, documented; defect → stays RED until W-H (68-F2) | 67-F1 |

**What the lane confirmed:** the §8 DAG is acyclic and forward (it ran FALSIFY-REL-06X-006), the §1.2 arithmetic, the A/B/C/D row shapes, and the §0 rows it re-derived other than G9/G14. **What it did not do:** measure fan-out (see above), or corroborate its own `measured` marks — which is why the re-run column exists.
...
| 2026-09-17 | #3445 (measured: v0.68.0 tagged 06:35Z with milestone 0.68.0 at open 2, #3091 reopened 05:10Z; the autopilot's tag step read no milestone) | §4 steps 1 and 4 read the milestone with `scripts/check_milestone_cut.sh` at the freeze and immediately before the tag. Carry = re-milestone + `slipped_from:`. Contract equation `milestone_settled_at_cut`, FALSIFY-REL-06X-009/010. Plan grilled 3/3 `do-not-implement-as-written` at v1, applied in v2 (`docs/audits/impl-PMAT-3445-plan.md`) |
