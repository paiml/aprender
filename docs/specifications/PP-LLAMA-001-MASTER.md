...
| 3.1 | 2026-09-03 | PP-9's live scan read the ledger's first pipe table only; a blank line after row 4 left rows 5 and 6 outside it, and a re-spend of row 6 as row 7 passed `spec_conformance.sh`. The scanner now refuses a ledger row outside the table it reads (L2), the ledger is re-joined, and `ledger_row_outside_table` / `ledger_rows_contiguous` are named beside PP-9's cases (PMAT-930) |
