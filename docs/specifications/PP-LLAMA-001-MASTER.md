...
| 3.1 | 2026-09-03 | PP-9's live scan read the ledger's first pipe table only; a blank line after row 4 left rows 5 and 6 outside it, and a re-spend of row 6 as row 7 passed `spec_conformance.sh`. L2 now refuses every ledger row outside the table it reads — any pipe line before the superseded heading whose first cell is a row id, in a run that does not open with a header row; column count and the leading pipe are not conditions, because the first form of L2 required them and the review quorum on #2861 escaped it four ways. Rows found by L2 also enter the L1 spend check. L3 refuses a first-table row whose cell count differs from the header's. The ledger is re-joined; nine cases are named beside PP-9's and scripts/mutate_spec_conformance.sh kills 8 of 8 (PMAT-930, PMAT-931) |
