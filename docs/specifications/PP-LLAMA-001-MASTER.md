...
| PP-9 | A cell is keyed `(host, workload, model, quant, commit, interleaved)`; once run at a key it is spent there and may not be re-run to green; a new commit is a new row | re-run at the same key and publish the second | re-run at a later commit accepted | ARMED | Appendix C ledger parser · `respend_same_key` / `respend_new_commit` (sh:scripts/spec_conformance.sh); `ledger_row_outside_table` / `ledger_rows_contiguous` beside them, with `ledger_row_no_leading_pipe`, `ledger_row_backticked_id`, `ledger_respend_outside_table` (L2), `ledger_row_trailing_pipes`, `ledger_row_extra_pipe` (L3), `ledger_side_table_ok` (CLEAN), `ledger_fragment_after_dummy_line` (L1,L2), `ledger_row_bold_id`, `ledger_second_table_same_header`, `ledger_row_extra_pipe_outside` (L2), `ledger_side_table_split_ok`, `ledger_side_table_same_width_ok`, `ledger_row_without_tier_outside_ok` (CLEAN), `ledger_first_table_not_ledger` (L0,L2), `ledger_row_under_foreign_header`, `ledger_row_after_superseded_heading` (L1,L2), `ledger_row_three_columns_off_outside`, `ledger_row_conformant_outside`, `ledger_row_nonnumeric_id_outside`, `ledger_row_backticked_tier_outside`, `ledger_reserved_word_in_side_table` (L2), `ledger_row_underscored_tier`, `ledger_respend_conformant`, `ledger_respend_emphasised_key`, `ledger_respend_zero_width_key` (L1) (sh:scripts/spec_conformance.sh); mutation set scripts/mutate_spec_conformance.sh, 15 of 15 killed over a green baseline |
...
| **1** | land #2809; Blackwell guard on `prefill_multi_prompt`; `perf041` in `cuda-nightly.yml`, missing marker = RED; PP-26 witness in the receipt | PP-26; §9 #1a, #2 | serve | — | **LANDED** 2026-09-20 (this PR), verified against `origin/main` rather than claimed: #2809 MERGED 2026-09-04; the Blackwell guard is `crates/aprender-serve/src/cuda/gpu_profile.rs:546` `multi_prompt_prefill_allowed` — **not** `src/gpu_profile.rs`, which is where a reader looking for it found nothing and read the row as undone — consulted at `generate_batched_streaming.rs:880` (`prefill_and_scatter`, printing `[PMAT-810] … refused`) and `:1045` (`recycle_slots_batch`), with its own case table at `:1283`-`:1310`; `perf041` is wired in `.github/workflows/cuda-nightly.yml` — the yield branch writes a `NOT_RUN` marker (`PERF041_OUT/marker.json`) so a yielded night cannot pass as a witness, and `scripts/check_perf041_marker.sh --selftest` runs as a step so the gate proves it can go RED on a night with no GPU; the PP-26 witness is `evidence/perf041/lambda/witness.json`. **The first wired gx10 night is EXPECTED RED** — that is §9 #1a's before-state, and it is a finding to file, not a flake to rerun. lambda witness taken 2026-09-02 at `a765c86a5` under v3.1: PASS on every band, `m=1` agreement recorded at 3 tokens (`evidence/perf041/lambda/witness.json`); under the v3.0 rule the same run was DEFECT on every `c>1` band, which is what §7.1's `[U]` anticipated. Expires **2026-09-19** |
| 2 | `scripts/perf002_prefill_path_probe.sh` + `decompose()` with mandatory refusals (negative slope, `R²` bound, fewer than 2 distinct token counts, bimodal → per-mode) | §9 #1 mechanism at `HEAD` | serve | — (needs a gx10 window) | **LANDED** (this PR) — `scripts/perf002_prefill_path_probe.{sh,py}`; `decompose()` carries the four mandatory refusals plus `implausibly_fast` (the harness guard), each naming itself and each mapping to exit 2, never to a defect claim. Measured gx10 (cc 121, `338f1d49`, qwen2.5-coder-1.5b-q4_k_m, ladder 64/128/256/384/513 ×3): **MECHANISM_CONFIRMED** — default arm linear at **9.042 ms/token** (r² 0.999947, n=15), `BATCHED_PREFILL=1` at **0.099 s** for ~512 tokens against the default arm's 4.72 s. Both halves of §10's registered prediction survive; slope reproduced at 9.054 and 9.042 across runs. §9 #1's **SIZE is NOT re-measured here** — the prefix cache is excluded by construction, not by label, because `prefix_cache_hits` is exposed nowhere (#3553), and without the label a prefill figure is a blend in an unknown proportion. Receipt: `docs/audits/impl-PMAT-3556-receipt.md` |
...
| 13 | `cargo bench` batched Q4_K GEMV at `M ∈ {4,8}` against `M ∈ {16,32}`; then the M=4 kernel | §7.3, §9 #6 | serve | 1 | **OPEN**, unblocked 2026-09-20 when row 1 was discharged. Expires **2026-09-30** (anchor: the 0.69.0 milestone is due 2026-09-23; this row was blocked by row 1 until 2026-09-20, so it gets one full train, not the three days left of the current one. The owner may move this date with a stated basis — it is a deadline, not an estimate.) |
...
| 15 | **gx10 shakedown cell**, W1, `n >= 5` interleaved, at a commit containing 0b, 0c, 1, 6, 7 → a new LEDGER row | first conformant receipt | perf-gate | 0b, 0c, 1, 6, 7, 12 | **OPEN**, unblocked 2026-09-20 when row 1 was discharged. Expires **2026-09-30** (anchor: the 0.69.0 milestone is due 2026-09-23; this row was blocked by row 1 until 2026-09-20, so it gets one full train, not the three days left of the current one. The owner may move this date with a stated basis — it is a deadline, not an estimate.) |
...
| **18** | **reference cell**, lambda, W1, `n >= 5` interleaved, both lanes, derived ladder, one commit; `armed_by` set for every band that PASSES; document → `ARMED` | §7.5; §9 #3, #4 sized conformantly | perf-gate | 15 clean at `--phase merge` | **OPEN**, unblocked when row 1 was discharged. Expiry **derived from row 15**, not typed here: §12's preamble makes `expires` a date only on ROOT rows, 15 is still live, and D3 refuses a blocked row carrying ANY date literal — `status / expires` is ONE column, so a narrated date is indistinguishable from a declared one and this cell therefore states none. The intent behind the date that stood here — this row cannot start until 15 is clean at `--phase merge`, and dating it the same day as its own blocker would guarantee the andon fires on work that was never startable — belongs on row 15's date when 15 is discharged, or as a typed expiry here at that moment. The owner may set it then with a stated basis. |
| **19** | **speed**: §9 #5 (per-token host work) — registered prediction at most 3.5 ms per token | throughput on the gated cell | serve | 1, 6 | **OPEN**, unblocked 2026-09-20 when row 1 was discharged. Expires **2026-09-30** (anchor: the 0.69.0 milestone is due 2026-09-23; this row was blocked by row 1 until 2026-09-20, so it gets one full train, not the three days left of the current one. The owner may move this date with a stated basis — it is a deadline, not an estimate.) |
...
| **21** | **speed**: §9 #1 KV-scatter root cause on Blackwell | gx10 c=1 fixed cost 16.75 s → under 1 s, conditional on row 2 showing the cost exists on the streaming transport | serve | 2 | **OPEN**, 2026-09-30 — dated here because row 2 was discharged and nothing derives this row's expiry any more (D2). Basis: it was blocked until 2026-09-20 and this is one full train past the 0.69.0 cut, matching rows 13/15/19. The owner may move it with a stated basis. **Row 2's condition is now MET**: the cost exists on the streaming transport and is linear at 9.042 ms/token (r² 0.999947, n=15, gx10 cc 121) — and `BATCHED_PREFILL=1` already reaches **0.099 s** at ~512 tokens, i.e. this row's "under 1 s" target is reachable on that path today; what remains is the KV-scatter root cause on the DEFAULT path. See `docs/audits/impl-PMAT-3556-receipt.md` |
