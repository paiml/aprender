...
| 3.1 | 2026-09-03 | PP-9's live scan read the ledger's first pipe table only; a blank line after row 4 left rows 5 and 6 outside it, and a re-spend of row 6 as row 7 passed `spec_conformance.sh`. L2 now refuses every ledger row outside the table it reads: any pipe line before the superseded heading whose first cell is a row id and whose width is within two columns of the header's, in a run that does not open with a header other than the ledger's — a run with the ledger's header or no header is read row by row, so neither a blank line nor a dummy first line hides a row; column count and the leading pipe are not conditions. Rows found by L2 also enter the L1 spend check. L3 refuses a first-table row whose cell count differs from the header's; L0 refuses a first table that is not the ledger. Two review rounds on #2861 shaped the rule (four escapes, then a rule that failed both ways). The ledger is re-joined; sixteen cases are named beside PP-9's and scripts/mutate_spec_conformance.sh kills 10 of 10 over a green baseline, a crash counted as unviable (PMAT-930, PMAT-931, PMAT-932) |
