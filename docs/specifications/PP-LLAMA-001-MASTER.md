...
| PP-21 | Receipt signature valid and `receipt.commit ⊇ commit-under-test` | unsigned; stale-by-one | signed, current | ARMED | `scripts/perf_receipt_sign.sh` · `sig_missing` / `sig_ok` (pg) |
...
| 3.1 | 2026-09-03 | PP-9's live scan read the ledger's first pipe table only; a blank line after row 4 left rows 5 and 6 outside it, and a re-spend of row 6 as row 7 passed `spec_conformance.sh`. L2 now refuses every ledger row outside the table it reads: any pipe line before the superseded heading whose first cell is a row id and that claims a conformance tier (`RECORDED` or `CONFORMANT`, the tiers PP-9 binds on) — no width, leading-pipe, run or header condition stands between such a row and the rule, because three review rounds on #2861 each removed one an author could satisfy. Rows found by L2 also enter the L1 spend check. L3 refuses a first-table row whose cell count differs from the header's; L0 refuses a first table that is not the ledger. The ledger is re-joined; twenty cases are named beside PP-9's and scripts/mutate_spec_conformance.sh kills 10 of 10 over a green baseline, a crash counted as unviable (PMAT-930, PMAT-931, PMAT-932, PMAT-933) |
