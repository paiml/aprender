...
| Item | Action | Δship | Effort | P | Notes |
|---|---|---|---|---|---|
| **P2-C** Widen corpus to > 2B tokens: codeparrot-python permissive + the-stack-v2 Python (audit recommendation) | (1) ✓ Author corpus merge contract — `contracts/corpus-merge-v3-v1.yaml` LANDED; (2) ✓ Pull plan scoped — bigcode/the-stack-v2-dedup data/Python = 15.38 GB compressed in 6 parquet shards (dry-run verified 2026-05-16); (3) Concat through `apr tokenize encode-corpus` — needs multi-source `--source` flag extension (in-scope for step 3); (4) Retokenize with Qwen tokenizer + NFC; (5) Dispatch pretrain under P0-J hard gate (PMAT-680) | +6 if val_loss < 3.5; +10 if val_loss < 3.0 | 6-12h CPU prep + 8-16h GPU train | **55%** (corpus diversity is the binding constraint per §49 + audit) | **NEW highest-EV next dispatch.** Audit Rec #1: P2-A2 cannot break the plateau; only P2-C can. **In progress — PMAT-681, see contract.** |
