...
**Version:** 2.55.0
**Atomic next action (v2.55.0):** pretokenize The Stack v2 filtered-Python for MODEL-2 convergence (unchanged from v2.51.0). **FALSIFY-QW2E-SHIP-004 / FALSIFY-SHIP-004 PARTIAL_ALGORITHM_LEVEL → DISCHARGED** on 2026-04-25 (task #152) via live `apr export → llama-cli` round-trip on noah-Lambda-Vector RTX 4090. `apr export /mnt/nvme-raid0/models/ship-two-001/qwen2.5-coder-7b-instruct-q4k.apr --format gguf -o /tmp/ship-004/qwen2.5-coder-7b-q4k-via-apr-export.gguf` exits 0 producing an 8.04 GB GGUF in Q4K passthrough mode (zero loss, 339 tensors, 20 metadata keys, contract-driven mapping for family qwen2). `xxd` confirms the first 8 bytes are `47 47 55 46 03 00 00 00` = magic `b"GGUF"` + version u32 LE = 3 ∈ {2, 3}. `llama-cli -m <gguf> --prompt "hello" -n 4 -ngl 99` loads the freshly-exported GGUF and emits 4 valid tokens ("Hello! How can") at 127.5 tok/s with exit 0. All three independent format-boundary verdicts PASS in one round-trip. Contract `qwen2-e2e-verification-v1.yaml` v1.8.0 → **v1.9.0** stays ACTIVE. Drift-prevention test `falsify_ship_004_yaml_binding_pins_discharged_status` added to `crates/aprender-core/src/format/ship_004.rs::ship_004_tests`. Spec v2.54.0 → **v2.55.0**. Coverage tally is now **36 PARTIAL + 9 DISCHARGED** (was 37 + 8; SHIP-004 promoted; **fourth MODEL-1 PARTIAL → DISCHARGED of the cycle**, after SHIP-009 #1054 + SHIP-001 #1056 already merged + SHIP-010 #1055 in flight). Evidence files: `evidence/ship-004-full-discharge/discharge-evidence-v1.json` + raw `llama-cli-run.txt`. Methodology: pure stack tooling — `apr export` + `xxd` + `llama-cli` end-to-end.
