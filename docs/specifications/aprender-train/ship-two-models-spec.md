...
**Version:** 2.54.0
**Atomic next action (v2.54.0):** pretokenize The Stack v2 filtered-Python for MODEL-2 convergence (unchanged from v2.51.0). **FALSIFY-QW2E-SHIP-001 / FALSIFY-SHIP-001 PARTIAL_ALGORITHM_LEVEL → DISCHARGED** on 2026-04-25 (task #151) via live `apr inspect /mnt/nvme-raid0/models/ship-two-001/qwen2.5-coder-7b-instruct-q4k.safetensors --json` on noah-Lambda-Vector RTX 4090. Apr inspect exits 0 with `format=SafeTensors AND tensor_count=339 AND total_params=7,615,616,512` (Qwen2.5-Coder-7B canonical counts). The 15.23 GB safetensors file loads end-to-end via the same `realizar::Model::load_safetensors` path AC-SHIP1-001 specifies — `Err(_)` would have surfaced as non-zero exit + error JSON. This PR also **backfills the FALSIFY-QW2E-SHIP-001 YAML block** that PR #1030 left missing (the v1.6.0 changelog narrative claimed the entry exists but the actual `falsification_tests` block was never written to disk). Combined: contract `qwen2-e2e-verification-v1.yaml` v1.7.0 → **v1.8.0** stays ACTIVE; FALSIFY-QW2E-SHIP-001 added directly at `discharge_status: DISCHARGED` with full discharged_evidence block. Drift-prevention test `falsify_ship_001_yaml_binding_pins_discharged_status` added to `crates/aprender-core/src/format/ship_001.rs::ship_001_tests`. Spec v2.53.0 → **v2.54.0**. Coverage tally is now **37 PARTIAL + 8 DISCHARGED** (was 38 + 7; SHIP-001 promoted; **third MODEL-1 PARTIAL → DISCHARGED of the cycle**, after SHIP-009 PR #1054 already merged and SHIP-010 PR #1055 in flight). Evidence files: `evidence/ship-001-full-discharge/discharge-evidence-v1.json` + raw `inspect-safetensors.json`. Methodology: pure stack tooling — `apr inspect --json` end-to-end, no `eprintln!`, no bash workaround per `feedback_apr_trace_not_eprintln.md`.

**Atomic next action (v2.53.0):** pretokenize The Stack v2 filtered-Python for MODEL-2 convergence (unchanged from v2.51.0). One of the two v2.52.0 session-discovered defects is now closed: **FALSIFY-SHIP-009 PARTIAL_ALGORITHM_LEVEL → DISCHARGED** on 2026-04-25 (task #149) via live `apr stamp` fixture-swap on the canonical lambda-labs staging artifact at `/mnt/nvme-raid0/models/ship-two-001/qwen2.5-coder-7b-instruct-q4k.apr`. Pre-stamp sha256=`0a854098d05b15921c173b7c8deb87c1cbecdffc66e918825c11a02775c73666` (size 8035635524, all 3 provenance fields `(missing)`) → post-stamp sha256=`a394dd286732a5f32dfb983fd2ea0eeba4d6239ac4c47e44bcfe62f590ddeb28` (size 8035635652, +128 bytes, license=`Apache-2.0` / data_source=`huggingface.co/Qwen/Qwen2.5-Coder-7B-Instruct` / data_license=`Apache-2.0`). `apr diff <pre> <post> --json` confirms tensor byte-identity (structural=0, tensor=0, quant=0, metadata=0; only file_size category shows the 128-byte delta). `apr inspect <teacher>.apr --json` round-trips all three keys with full string values, satisfying GATE-APR-PROV-004's v1.1.0 `full_discharge_blocks_on` "Fixture-swap only — no code change, no contract change" condition. Backup retained at `.pre-stamp.bak.apr`. Tooling chain proof: `apr stamp` (PR #1050 commit 71d23fb5e, task #141) + `apr inspect` provenance emission (PR #889 commit 8f0607d42, FALSIFY-SHIP-022 DISCHARGED, task #113) + `apr diff` tensor-identity. Three downstream irreversible-shipped follow-ups (apr-leaderboard checkpoint replacement, HF Hub re-upload of `paiml/qwen2.5-coder-7b-apache-q4k-v1`, publish-manifest sha256 bump) are documented under `discharged_evidence.deferred_irreversible_actions` and require explicit user authorization per `feedback_compute_pre_authorized.md`. Contract `apr-provenance-v1.yaml` v1.1.0 → **v1.2.0** stays ACTIVE; spec v2.52.0 → **v2.53.0**. Coverage tally is now **38 PARTIAL + 7 DISCHARGED** (was 39 + 6; SHIP-009 promoted; first MODEL-1 PARTIAL → DISCHARGED of the cycle, completing the local-fixture half of MODEL-1 SHIP-009). The remaining v2.52.0 defect (SHIP-007 parity-gate blocker on 7B GQA-7:1) is on a separate path tracked in `project_ship_007_attention_parity_investigation.md` — extending `TraceStep` enum with intra-FFN/attention variants behind a contract is the proper fix per `feedback_apr_trace_not_eprintln.md`. Evidence file: `evidence/ship-009-full-discharge/discharge-evidence-v1.json`.: FALSIFY-SHIP-001 DISCHARGED via apr inspect on real teacher safetensors + YAML backfill)
