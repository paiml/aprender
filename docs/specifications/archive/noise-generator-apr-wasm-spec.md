...
- **G2**: WASM-first architecture for browser deployment (**<2MB optimized**, <2s load) - *Aligned with PMAT Item 06*.
...
- **G6**: Training infrastructure for model generation (approximating analytical DSP curves).
- **G7**: Zero-allocation audio processing in the hot loop (AudioWorklet compatible).
...
│  │  + AudioWorklet Processor Interface                         │  │
│  └────────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────┘
```
        ...
        if let Some(slope) = self.spectral_slope {
            if !(-12.0..=12.0).contains(&slope) {
                return Err(NoiseError::InvalidConfig(
                    format!("spectral_slope {} must be in [-12, 12]", slope)
                ));
            }
        }
        if !(0.0..=1.0).contains(&self.texture) {
            return Err(NoiseError::InvalidConfig(
                format!("texture {} must be in [0, 1]", self.texture)
            ));
        }
        if !(0.0..=1.0).contains(&self.modulation_depth) {
            return Err(NoiseError::InvalidConfig(
                format!("modulation_depth {} must be in [0, 1]", self.modulation_depth)
            ));
        }
        if !(0.1..=10.0).contains(&self.modulation_rate) {
            return Err(NoiseError::InvalidConfig(
                format!("modulation_rate {} must be in [0.1, 10]", self.modulation_rate)
            ));
        }
        if ![256, 512, 1024, 2048].contains(&self.buffer_size) {
            return Err(NoiseError::InvalidConfig(
                format!("buffer_size {} must be 256, 512, 1024, or 2048", self.buffer_size)
            ));
        }
        Ok(())
    }
...
impl NoiseGenerator {
    ...
    /// Generate audio samples into output buffer (Zero-copy friendly)
...
### 5.4 WASM API (Zero-Copy Enhanced)
...
impl NoiseGeneratorWasm {
    ...
    /// Generate mono samples (allocating Vec<f32>)
    pub fn generate(&mut self, length: usize) -> Vec<f32>;

    /// Generate interleaved stereo samples (allocating Vec<f32>)
    pub fn generate_stereo(&mut self, length: usize) -> Vec<f32>;

    /// Zero-copy generation: Write directly to WASM memory buffer provided by JS
    /// JS passes a pointer to a Float32Array in WASM memory.
    pub fn generate_into_buffer(&mut self, output_ptr: *mut f32, len: usize);
...
| Status Display | Shows version, memory usage, and state |
...
SIZE=$(du -b $OUT_DIR/aprender_noise_bg.wasm | cut -f1)
if [ $SIZE -gt 2097152 ]; then
  echo "Error: WASM size $SIZE bytes exceeds 2MB limit (PMAT 06)"
  exit 1
fi
...
## 10. Quality Gates (PMAT Aligned)

### 10.1 Pre-Merge Requirements

| Gate | Command | Target | PMAT Ref |
|------|---------|--------|----------|
| Unit tests | `cargo test --lib audio::noise` | 100% pass | Item 05/29 |
| Property tests | `cargo test --test property_tests noise` | 100% pass | Item 11 |
| Coverage | `make coverage` | ≥95% on new code | Item 43 |
| Mutation | `cargo mutants --file src/audio/noise/` | ≥80% killed | Item 44 |
| WASM build | `cargo build --target wasm32...` | Success, **<2MB** | Item 06 |
| Clippy | `cargo clippy --features audio-noise -- -D warnings` | 0 warnings | Item 46 |
| Docs | `cargo doc --no-deps` | 100% public API | Item 10 |
...
7. **wasm.rs**: Feature-gated WASM bindings (with zero-copy)
