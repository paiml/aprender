...
`queue_wait_s` per **host class**, 10 slowest test **jobs**, and the §1 PRs-per-train number computed against the **required-check set**. *(Amended 2026-09-21, ruling on #3271 round 2: **host class**, not host — the fleet is managed per class (intel / gx10 / yoga / lambda / mini; `fleet_utilization.sh` and every operator rule use it) and the ledger has no per-host field worth grouping on; the class is derived from the runner name's prefix, an undeclared prefix is `other`, and `check_build_report.sh` covers each prefix and the unknown one. **Required-check set**, not `ci / gate` — two mechanisms answer "what is required on `main`", classic protection and ruleset 13878864, and they spell the gate differently; keying PRs/train on one name is the one-mechanism error. `make build-report` derives the set from both at run time and prints the set it used, and when `gh` cannot answer it uses the documented set and says so.)* *(Amended 2026-09-21, ruling on #3271 round 1: the build ledger is per job and cannot answer per target; per-target timing lives in nextest's junit output, which #3134 adds. Until that ledger exists `make build-report` reports the 10 slowest jobs and prints that the ledger lacks target granularity — instrument-first, the gap recorded rather than the requirement silently substituted. Per-target once #3134 lands.)*
