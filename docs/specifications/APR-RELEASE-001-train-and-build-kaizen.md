...
- **v0.68.0** — GitHub-only: tag, 16 assets. Not on crates.io. ~~`install.sh` verified on intel/gx10~~ **[X] REFUTED 2026-09-20 (#3546): no `install.sh` has ever existed.** `curl /releases/latest/download/install.sh` → **404**; `find` over the tree returns only `scripts/install_pr_review_tools.sh`; and the 16 assets are 8 `apr-*` + 8 `pv-*` tarballs and their `.sha256` — no installer among them. The claim is struck rather than deleted so the record shows what was believed. Not on crates.io (B2 found lib tests naming path-only sibling dev-deps, #3425). Release note points at 0.68.1. A published tag is never moved or deleted.
...
| **T-2 Dogfood** | `apr-dogfood` skill go/no-go receipt; `apr-cookbook` current; release notes generated | receipt exists for this sha — **plus one row, RE-AIMED 2026-09-20 (#3546, was #3366's installer row):** on a clean intel (x86_64) and a clean gx10 (aarch64), download the release ASSET for that host's arch+flavour from the tag, verify its published `.sha256` against a measured one, extract, and `apr --version` prints `v0.N.0`; both hosts, both rows green, or T-2 is red. **Why it was re-aimed:** the row it replaces asserted `install.sh --version v0.N.0`, and no `install.sh` has ever existed — 404 at the advertised URL, absent from the tree, absent from the 16 assets. Its own next clause said *a 404 there is a T-2 failure, not a docs nit*, so by its own rule T-2 has been RED since 2026-09-16 while 0.68.0, 0.68.1 and 0.68.2 all shipped: a gate whose failure nobody consumed. The replacement tests the artifact that actually ships, and was proven runnable on a third host (yoga, 2026-09-20: published sha256 `59e423a0…` equals measured, `apr 0.68.2 (v0.68.2+no-git)`). **The installer row returns when the installer ships, not before:** PP-066 R-6 (#2869 / PMAT-994) is the ticket that would add `install.sh` as a release asset, and T-2 gains an `install.sh` row in the same PR as that asset — a row may not precede the artifact it tests — **Nightly producers (operator 2026-09-17):** any nightly that feeds a T-2 row (today: `cuda-nightly.yml` → PP-26 marker) and fails **≥ 2 consecutive** runs auto-opens an issue on the next milestone and shows as `NIGHTLY-RED` in §7; the T-2 receipt records the witness host's provenance when it differs from the producer's (0.68: lambda sm_89 witness; gx10 producer red since 09-12, #3096 stays 0.70) — **Inherited T-2 (operator 2026-09-17):** T-2 on the release commit inherits the parent-sha preflight GO when the bump diff touches only `Cargo.toml`, `Cargo.lock`, `CHANGELOG.md`, `crates/*/Cargo.toml`; the receipt records `inherited_from: <sha>`; any other path → full T-2. First-chain rule: the clean-room on the tag runs through B2 and past it to the end; if a gate after B2 stops, the gate id is reported before any fix |
