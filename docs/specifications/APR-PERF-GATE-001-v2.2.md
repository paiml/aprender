...
tokenization:
  ...
  chat_template_sha256: "<REQUIRED>"         # PERF-057, v2.3
  corpus_prefill_tokens_median: <int>        # PERF-057, v2.3
comparator_tokenization:                     # PERF-057, v2.3
  # the same block, measured on the comparator lane. REQUIRED whenever any
  # band is gated by Arm B — I-11's comparison needs two operands.
```
...
**The last two fields are new in v2.3 (PERF-057), and they are the reason the rule now bites.** As specified through v2.2 this block would have been **identical on both lanes** in the measurement that motivated it, while the lanes differed by 4.09% on prefill. Measured 2026-08-29 — apr `0.64.0` (745fa8588) against `llama-server` 7746 (39173bcac), same W1 corpus, same `qwen2.5-coder-7b-instruct-q4_k_m.gguf` — both harnesses count tokens from the server's own `usage` object, so both lanes declare `method: server_usage` honestly, and the prompts they actually prefilled were 513 and 534 tokens (deterministic on both sides, `min == median == max`; constant delta −21, ratio 0.9607). It is not the vocabulary: on raw text apr, llama and the canonical tokenizer agree exactly at 505/505/505. It is the chat template — `llama-server` applies the jinja template embedded in the GGUF, which injects Qwen's default system message; apr applies a hardcoded ChatML builder (`crates/aprender-serve/src/api/realize_handlers.rs`, `format_chat_messages`) with no system message and a constant 8-token wrapper. And it is not correctable: short prompt 25 vs 43, W1 513 vs 534, special-token text 76 vs 57 — the delta **changes sign** with the input, so there is no constant offset and no constant ratio.

**Every other field in this block is a declaration.** A declaration cannot be wrong-and-detected; it can only be false. `chat_template_sha256` (the resolved wrapper each lane applied, digested over one fixed probe rendering) and `corpus_prefill_tokens_median` (the ids each lane actually prefills, median over the whole corpus, **after** the template) are outcomes, and they are the only fields here that a mis-declaration cannot survive. `corpus_prefill_tokens_median` is also §4.3.1's own requirement — "the receipt must declare which side of that boundary its count was taken on" — finally given a field to live in. **Equality is exact**, which is a rule rather than a threshold: two lanes that applied the same template and the same tokenizer to the same corpus produce the same count, not a nearby one, and any tolerance here would be an invented continuous threshold in the one area `perf-matrix.yaml`'s GROUNDING RULE forbids them. A consequence worth stating: under `server_usage` the two numbers are two servers' opinions and will not agree, so this rule is what makes the canonical `client_tokenizer` the only method a real receipt can pass with.

**What the rule still cannot see** (`scripts/perf_gate.sh::arm_c_tokenization` carries the same list): the two lanes' **corpus identity** is not compared — that is a join-key property (§4.4.8, "same workload file") and belongs beside host/accelerator/model/quantization, not here; two lanes agreeing exactly on a prefill length **outside** §4.3.1's declared `512 ± 8` still pass, because that band is not represented in `perf-matrix.yaml`; the two boolean declarations remain unverifiable when both lanes declare the same false value; and nothing binds `corpus_prefill_tokens_median` to an instrument — §4.5 requires streaming and **neither server reports `usage` in SSE at all**, so the number must come from a separate non-streaming replay and the gate cannot tell such a replay from a typed-in figure.
...
| **Tokenization poka-yoke** | `scripts/perf_gate.sh`, `scripts/lib/bench_receipt.py` | omit `tokenization.method` | present green | PROVEN — `tokenization_absent` / `baseline_healthy`; neutering the check gives 18/19. **Absence only**: through v2.2 the gate tested that the field was a non-empty STRING, and its own OK fixture carried `"hf-tokenizers"` — not one of the two legal values — so the input proving this row was itself illegal. The VALUE rule is the row below. |
| **Tokenization values + I-11 mismatch (PERF-057)** | `scripts/perf_gate.sh` | drop `corpus_prefill_tokens_median` from the compared key set | the other 16 tokenization rows stay green; a comment-only edit stays 60/60 | PROVEN — 60 → 59/60 on `tok_identical_blocks_divergent_prefill` **and nothing else**, which is the finding: with the measured prefill length out of the comparison, two lanes whose §4.4.6 blocks are identical field-for-field and whose prefill differs 513 vs 534 PASS. Five more mutations bite — neutering the legal-value test on `method` → 59/60 (`tok_method_illegal_value`); making a block mismatch non-fatal → **56/60** (the three declared-field mismatch rows plus the prefill row); dropping the comparator-block requirement on an Arm-B-gated band → 59/60; neutering `client_tokenizer` ⇒ `tokenizer_sha256` → 59/60; neutering the malformed-comparator check → 59/60, and that one reds as *"fail but never said is not an object"* — the row still failed, on an AttributeError, which is why every row here asserts the SENTENCE and not the colour. Revert 60/60 in every case. `ci.yml:1039`. |
...
**Coverage: 26 rows — `scripts/lib/mutation_registry.py` reads 18 PROVEN, 5 PARTIAL, 3 UNPROVEN** (measured on this branch; at `8c3abe631` it read 17/5/3 of 25 while the prose here said "15 PROVEN, 7 PARTIAL, 3 UNPROVEN" — the hand-maintained tally had drifted from the table it summarises, so PERF-057 quotes the scanner rather than keeping a second count by hand) (v2.2 recorded "1 of 24"; v2.1 1 of 21, v2.0 1 of 14). The denominator grew by one because the registry was missing a row, not because the gate grew. Target **26 PROVEN** before this document leaves DRAFT.
