...
| Claim-literal guard | `scripts/check_no_claim_literals.sh` | write `2.93x Ollama` into `docs/` | unrelated prose green | PARTIAL — rc=1 on the ASCII form, rc=0 on unrelated prose; 12 case rows plus 14 causal rows, not 8; `ci.yml:971`. **The `×` (U+00D7) spelling v2.2 named in this cell is GREEN**: `RATIO_RE` matches ASCII `x` only, so `2.93× Ollama` in `docs/` passes. Hole recorded here rather than widened in the same commit — widening the pattern moves `scripts/claim_literal_baseline.txt`, which is shrink-only. |
| **`[X]` figure guard** | `scripts/check_no_claim_literals.sh` | write `36.9x FasterTransformer` into `docs/` | unrelated prose green | PARTIAL — rc=1 verified for `36.9x FasterTransformer`, `23x static batching` and `1.8x vLLM`. Two looser spellings are GREEN: `36.9×` (U+00D7), which is what v2.2 named here, and `36.9x over FasterTransformer` — one intervening word defeats the adjacency, and that is the form §0.1 of this document itself uses. |
...
| Ratchet direction | `scripts/lib_baseline_ratchet.sh`, `scripts/check_baseline_ratchets.sh` | append one entry cloned from a baseline's own last real entry | delete an entry → green | PROVEN — `check_baseline_ratchets.sh` rc=1 and `check_no_claim_literals.sh` rc=1, both naming the added line and the comparand `merge-base with origin/main`; deletion rc=0. 31 case rows, `ci.yml:472`. |
