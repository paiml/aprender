...
| the merged tree's `guard_mutation_score`, **measured**. `PRREV-019` and `PRREV-020` were written on the same parent and merged here; the derived counts moved again (mutation set 217 → 237/237, `tests/pr-review.bats` 156 → 158) and **no sweep has run on the merged tree**. Neither lane's figure transfers: 215/215 was measured on `PRREV-015`'s tree, and `PRREV-020` gave every `row-*` fixture an `output_check` block, so the corpus the sweep runs against moved as well as its size. The set size above is derived by `scripts/mutate-guard.sh --list` and is certain; the kill count is Arm 3 of the `pr-review-receipt` job, and lands as a follow-up commit the way `804559ed5` discharged `03d795423`'s pending 185/185. §8 allows no ratchet here: below 100% blocks. | `PRREV-021` | **PENDING** — the number is unverified until Arm 3 reports |
