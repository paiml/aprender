...
- **The key issue — GPU discovery and the release binary.** *Solved on a branch, not yet on `main`, and only for discovery*: `apr devices` and the backend registry (R-0a, PR #3004) enumerate cpu/cuda/wgpu on every host, print one line per backend kind with a reason when unavailable, and were dogfooded on all four hosts (gx10 GB10, intel 2× W5700X, mini M4, lambda RTX 4090; re-measured live on lambda by an independent reader on 2026-09-06). The decision surfaces are untouched until R-0b: on the same default build `apr run … --gpu` still exits 9 with "no GPU backend compiled in", `--backend cuda` is refused from the build flag (rc 5), and `apr serve run --list-devices` prints "NO accelerator compiled in" while `apr devices` two commands earlier printed the RTX 4090. No host has yet printed `backend: cuda ready` from `apr devices`; the CUDA factory has only been exercised through the case table's cuda arm. *On track*: resolution reading the registry instead of compile-time flags (R-0b), cuda in the published crate's default features (R-2, gated on decision D-9), release assets on tagged releases with a promotion gate from four host receipts (R-5), a verifying one-line installer (R-6), README (R-7). *Blocked*: decisions D-3, D-9, D-11 are blank; D-9 gates R-2 and therefore the "user gets the GPU by default" outcome.
...
- No tagged release has ever carried an `apr` asset: `binary-release.yml` builds and uploads `pv` only (`--bin pv -p aprender-contracts-cli`, four Linux targets); `v0.65.2` carries eight `pv-*` files and nothing else, `v0.64.0` likewise. The only prebuilt `apr` a user can download is the `nightly` prerelease (5 targets, 10 files, rebuilt from `main` on any day with commits; today's from `027ed889d` at 08:28Z), built with apr-cli's **default** features on the four unix targets and `--no-default-features --features inference` on Windows only — none carries `cuda`. (S0 ledger row S0-16 states the Windows flag as if it applied to all five targets; a correction is owed to the ledger.) `README.md:23` itself hand-types `cargo install aprender  # CPU ONLY - no GPU backend is compiled in`.
- The four 0.65.2 "host receipts" are `cargo install` receipts from crates.io, each naming a different locally compiled binary sha256; R-5 turns them into receipts of one signed release asset per host.
...
- Was dogfooded on all four hosts (receipt `docs/audits/impl-PMAT-989-receipt.md`, "Four-host dogfood"; the three remote blocks are receipt text, the lambda block was re-run live): gx10 `NVIDIA GB10` Ready through wgpu/vulkan (unified, 122502 MiB); intel two `AMD … (RADV NAVI10)` Ready through wgpu/vulkan plus llvmpipe refused; mini `Apple M4` Ready through wgpu over Metal (the separate `metal` kind line reads `NoBackend`); lambda, default build, six lines: cpu Ready, `cuda unavailable reason=NotCompiled source=not-compiled`, `wgpu ready device[0]="NVIDIA GeForce RTX 4090" kind=discrete transport=vulkan`, a second wgpu line refusing llvmpipe, metal and hip `NoBackend`, then `selected: wgpu device[0]  reserve=3584MiB basis=[U] …`. The printed device name equals `nvidia-smi -L` (the C11 cross-check).
- Why the default build sees the 4090 at all: apr-cli's default features carry neither `cuda` nor `wgpu`, but `inference` pulls `realizar`, whose default `gpu` feature enables `trueno/gpu`, so wgpu adapter probing is compiled into every default `apr` while `accel.rs:28` (`cfg!(any(feature = "cuda", feature = "wgpu"))`) still answers "no accelerator". apr-cli's own `wgpu` feature is a marker (`wgpu = ["inference"]`), which is also why `--backend wgpu` is accepted by the same binary that refuses `--backend cuda`. That contradiction is exactly what R-0b removes.
- Honesty notes from the independent review: the contract claims only invariants (i) cpu always Ready and (iii) every entry carries `source` and a reason — (ii) resolved ∈ Ready, (iv) effective-config equals resolved and (v) no build-fact in resolution are declared non-goals until R-0b; the receipt and contract still say 9/9 for the case table while the branch carries 10 rows (the same-name fix added row 10) — to be corrected in the write-back; the non-hermetic FX rows (LD_PRELOAD stubs, real drivers, root) are host-dogfood receipts, not CI tests, while the spec card's A still lists FX-1..FX-14 inside the catalogue until #3003 lands. The catalogue's FX-7 row found a real library gap (reserve refusal not propagated to the API twin) and the intel run found REG-9's same-name collapse (two W5700X shared one `device_uid`), both fixed on the branch.
...
|---|---|---|---|---|
| resolution reads the registry: `--backend/--gpu/--device` refused by enumeration with the registry's reason; `accel.rs`, `dispatch.rs`, `serve/mod.rs` stop consulting `cfg!` | R-0b (issue #3002, PMAT-1060; DAG row lands with #3003) | open; on `agent/R-0` the build-flag reads remain at `accel.rs:28`, `serve/mod.rs:44/:78/:236/:656`, `dispatch.rs:173`, `lib.rs:230`, `bench.rs` ×3 and two serve handlers (C11 requires 0). Two scope gaps for R-0b to absorb: `apr serve run --list-devices` (`serve/mod.rs:41-62`) is not named in its title, and the amendment in #3003 duplicates the cfg-grep and refusal-table acceptance items into R-0b without removing them from R-0 — an omission to fix in the docs write-back | R-0a merged | 2026-09-19 (R-0) |
...
| `apr-*` assets for the 5 targets on every tagged release, built per D-10 (`cli,cuda` where cuda compiles without a toolkit — S0-14 measured on intel and mini; Windows `nvcuda.dll` not yet measured), sha256 + signed manifest (minisign per R-6/S0-19; which key signs the release manifest is still open), cut as `v0.66.0-rc1` prerelease, promoted only from four host receipts that install the asset and name its sha256 (`check_multiplatform_dogfood.sh --from-release`) plus H12 | R-5 (#2908) | open | R-0, DEC-D-10 (issue #2937 open; default stands) | 2026-10-09 |
| one-line installer at `/releases/latest/download/install.sh`: detect OS/arch, verify checksum and signature, end by printing `apr devices`; acceptance `bash tests/install/cases.sh` (`tests/install/` does not exist yet) | R-6 (#2909) | open | R-5 | 2026-10-16 |
...
**Mechanism.** `scripts/check_hardcoded_paths.sh --full-if-capable` compared `pmat analyze hardcoded-paths` `shipped_count` against `scripts/hardcoded_path_shipped_baseline.txt`, a single number (277) recorded on 2026-09-05 by #2985 with no instrument named. The mode was "self-arming": it skipped while the fleet's pmat (3.31.0) lacked the subcommand — which is why `main`'s own push CI stayed green for twelve consecutive runs through `027ed889d` (04:32Z): the step printed `SKIPPED (proven): pmat 3.31.0`. paiml/infra's `machines/intel/forjar.yaml` pins the fleet at 3.37.0 (`3.31.0 -> 3.37.0 (BSE-10b, PMAT-231)`), and the binary at `/home/noah/.cargo/bin/pmat` on the runners was replaced in place while jobs were running: 3.31.0 was observed at 07:11Z and 07:14Z, 3.37.0 at 07:47:50Z on `intel-clean-room-9` and 07:57Z on `intel-clean-room-11`. Every `guard-runner-labels` job whose ratchet step executed after 07:47:50Z counted 317 against 277. Re-scanning C0-5's own commit (`cdc0acb99`) and a pristine `027ed889d` under both 3.37.0 and 3.38.0 gives 317 / 14767 files scanned each time: the tree never changed, the instrument did. First red: #3007 at 07:47Z, then #3003, #3006, #3008, #3009. #3001 and #3005 entered the merge queue only because their PR-level CI ran under 3.31.0 before the flip; #3001's queue run then spent 105 min in `workspace-test` (green) and 70 min in `guard-runner-labels` (red at 08:19Z), with `ci / gate` still waiting for a clean-room runner since 07:53Z, so the bare `gate` must fail and evict it. #3007's bare `gate` already failed at 09:24Z.
...
2. `scripts/check_pmat_pinned.sh` — the operator's assertion verbatim, `grep -rEn '(^|[^_/])pmat ' scripts/ .github/workflows/ | grep -v pmat_bin` must be empty, prose included; 15-row case table. RED on `main` with 280 lines (the first run counted 281 because the guard's own fixture line still matched; fixed in the same commit series); 0 after the sweep of ~38 scripts and 5 workflows (presence probes became ENV failures; `dogfood.sh`'s pmat-verify WARN became FAIL; `verifier_pin.sh`'s fleet case takes the pin; python callers read `$PMAT`; workflow installs are at the pin).
...
5. Deployment precondition, stated rather than hidden: under G-10 a runner without a 3.37.0 binary (or a versioned root) fails the first analyser step it reaches (`ci.yml:680`, the complexity ratchet) with `FAIL (ENV)`; there is no longer a SKIPPED path. The fleet is at the pin today; a future forjar bump must ship with a `pmat_bin.sh` bump PR (the R4 sequence).
6. Evidence: 13/13 self-test rows (8 new fixture rows, both polarities); live on the branch: `REPORT BASELINE-INVALID{stamp=none,binary=3.37.0}` then `PASS (differential, delta +0)`; live mutation (a `/home/probe/…` string literal appended to `crates/apr-cli/src/main.rs`) → `delta +1` naming the file, rc 1; every step of `guard-runner-labels` replayed locally — the only reds (docker, apr-bin resolution, four self-tests) fail identically on a clean-`main` worktree. Contract `contracts/apr-pinned-analyser-ratchet-v1.yaml`.

**Other reds seen on the armed PRs, classified.** `present` (pr-review-quorum, `pull_request_target`) fails on every PR including the merged #3000 with `A2 no receipt directory at evidence/pr-review/<n>` — a receipt this session does not produce (driver A1: receipts advisory); it is in no required context or ruleset. #3004's CI has not run at all since 08:05Z: the run is `pending` with zero jobs behind its own concurrency group, whose predecessor run for `ad574f2a3` still holds a queued `ci / gate` while its other jobs were cancelled. `pr-review-receipt` CANCELLED on #3001 — the C0-6 class (mutation step exceeds the job timeout), a DAG row. `Build Book` on #3004 — real: `apr devices` had no chapter; fixed on the branch. `guard-runner-labels` on #3006 before the ratchet — real: an absolute `/usr/bin/apr` fixture path; fixed on the branch.
...
**Sequence to green `main` and the first merges:** G-10 PR → queue → `main` green; then #3001, #3003, #3004, #3005, #3006, #3007, #3008, #3009 serially through the queue (the merge-queue ruleset builds and merges one entry at a time, squash, `check_response_timeout` 180 min; #3001's entry measured 105 + 70 min of jobs plus a runner-pool wait, so entries are at risk of timing out even when green).
...
- 2026-09-12: R-3 (armed; #3001 sits at exactly the DAG's minimum slack and cannot land before the ratchet fix, so an expiry amendment is owed), DEC-D-3, DEC-D-11. G-6 is not in this set: its expiry was amended to 2026-09-26 and it is complete.
...
- **Queue throughput.** One merge-queue entry at a time and 1–2 h per run on a fleet shared across repos: the eight armed PRs plus G-10 are at least a day of serial merges after `main` goes green, and an entry whose jobs plus runner wait exceed 180 min is evicted while green (#3001's did). If throughput is the constraint, merge-queue batching is a ruleset change and is raised as a STOP, not applied.
...
`git rev-parse origin/main` = `027ed889d`; DAG and receipts read from `origin/main` with `git show`; branch facts from `origin/agent/{R-0,R-3,T-2,I-24,C0-4,I-25,I-26,C0-3}` and the local `agent/G-10`; 0.65.2 numbers from `evidence/dogfood/0.65.2/VERDICT.md`; the fleet pin from paiml/infra `machines/intel/forjar.yaml` via the GitHub API; the ratchet counts from `pmat analyze hardcoded-paths -p . -f json` under `~/.local/pmat/3.37.0/bin/pmat` (3.37.0) and `~/.cargo/bin/pmat` (3.38.0) on clean checkouts of `027ed889d` and `cdc0acb99`; PR and check states from `gh pr view`/`gh pr checks` and the in-session monitor, 2026-09-06 07:00–09:30Z. Written by the PP-066 orchestrator session. Three independent readers and three adversarial refuters (workflow `pp066-progress-verify`, 6 agents, 194 tool uses) re-checked the GPU-discovery, release-binary and CI facts: 61 of 66 facts confirmed; the five refuted (a DAG acceptance item "moved" that was in fact duplicated; a commit count and an ancestry claim distorted by a shallow local clone; a merge timestamp; a stale per-PR check state) are corrected above. Measurement caveat the refuters raised: the shared local git dir is a shallow clone (`.git/shallow`, 6 lines), so local ancestry counts are unreliable — ahead/behind figures here come from the GitHub API (`agent/R-0` is 17 commits ahead of `main`, 28 files, +3032/−63). Per-host `apr devices` blocks for gx10, intel and mini and the S0-14 builds are receipt and ledger text, not re-run here.
