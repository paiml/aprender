...
| File | Change |
|---|---|
| `scripts/check_milestone_cut.sh` | new gate: exit 0 only when the named milestone holds no open issue or PR except the train's own release epic (printed `ADMITTED`); 1 names each open item plus its carry/close remedy; 2 when it cannot judge |
...
|---|---|---|
| `bash scripts/check_milestone_cut.sh --self-test` | goal lane: "exits 0" (v1 file, superseded) | exit 0, `self-test OK: 25 case(s).` (v2 + release-epic rule) |
| bare run = self-test | — | exit 0, same line |
| mutation (a) judge always passes | lane: "mutations turned self-test RED" (no output) | exit 1; RED S2 S3 S4 S12 S15 S16 S18 S19 S20 S21 S23 |
| mutation (b) count cross-check deleted | — | exit 1; RED S6 S7 |
| mutation (c) `--paginate` dropped from the items read | — | exit 1; RED S15 |
| mutation (d) release-epic admission removed | — | exit 1; RED S17 S22 S23 |
| mutation (e) admission widened to any `epic` label | — | exit 1; RED S18 S20 S21 |
...
| live `check_milestone_cut.sh 0.69.0` | — | at `465cd96f2`: exit 1, 48 open. After the epic rule: exit 1, `ADMITTED #3080` (release epic), 24 other open |
| live `check_milestone_cut.sh 0.70.0` | — | exit 1, `ADMITTED #3081`, 28 other open |
| `check_no_claim_literals.sh` (a review lane saw "universe EMPTY" in its sandbox) | — | exit 0 on the tree |
...
- **F-9 (defect found at pre-merge review, fixed in this PR).** The review delegate measured, and the
  orchestrator re-measured, that each train's release epic sits in its own milestone and is closed after
  publish. #3078 (milestone 0.67.0) was closed 2026-09-13T09:14Z, and v0.67.0 was tagged 07:35Z. As first written,
  the binding pre-tag read would therefore have been RED at every cut. The gate now admits exactly that one item:
  an issue, labelled `epic`, whose title is literally `EPIC: release train <M>` followed by end or whitespace. It
  prints the item as `ADMITTED`; two claimants exit 2. Cases S17–S23 and mutations (d) and (e) cover it. The
  0.67.0/0.66.0 first-green reads were taken after those milestones closed, not at their cut instants. That limit
  is stated here and not claimed away.
- **F-10 (rail defect, measured by the review delegate from agy transcripts).** In the `quorum-review.sh`
  round at `dfe085294` (3/3 PASS, 0 findings), lanes 2 and 3 read lane 1's verdict file through the
  script's `$WORK` path before finishing (lane 2 at 13:49:04Z, lane 3 at 13:49:16Z). Independence held for
  lane 1 only. All three returned a `tree_witness` copied from another session's lane clone, and the rail
  stages and checks no witness. Lane 1 opened no file. That round's 3/3 is recorded here as one
  uncontaminated PASS plus two PASSes cast after seeing it. It also missed F-9, which the delegate found. The
  round is superseded by the round on the fixed head.
- **F-11.** 06x §4 step 1 originally said step 8 "closes the milestone on `open_issues`". Step 8 says only
  "close the milestone". The text now attributes the `open_issues` refusal to the train driver.
