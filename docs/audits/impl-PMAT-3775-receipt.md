...
1. **Every exit writes exactly one JSON document, with #3720's field names.**
   ...
   - `cmd_code` wraps `cmd_code_inner`. An error returned from it leaves `apr` through `CliError::Aprender`, exit 1 (`crates/apr-cli/src/error.rs`), and its document says exit 1.
   - The early refusals carry typed kinds: `invalid_input`, `max_turns_exhausted`, `hook_blocked`.
   - The no-model exit writes `no_model` with exit 5 before `process::exit`.
...
## Checks
- `cargo test -p aprender-orchestrate --lib agent::`: 892 passed.
- `cargo clippy -p aprender-orchestrate --lib --tests -- -D warnings`: clean. `cargo fmt --check`: clean.
...
- `scripts/guard_tree.sh --no-cargo`: see the commit that adds this receipt.
