# PRM-001 analysis plan v2 (pre-registered for spec PRM-001 v3, 2026-09-25)

Frozen together with spec `docs/specifications/PRM-001-prometheus.md` §2–§5,
`crates/aprender-review-experiment/src/stats.rs` and prompt v1 by `docs/audits/prm-001/prereg.lock`
(`rex-prereg-v2`). Any byte change to one of those files is a new spec version (R-1); data
gathered under the old version is `exploratory`. Plan v1 (`docs/audits/rex-001/analysis-plan.md`)
and its lock are superseded, frozen and never edited.
...
| H7 independence (a gate, outside the family) | κ_err(apr, each voter) vs max κ_err over voter–voter pairs, same items | `error_pairs`, `error_kappa`, `h7_holds` | holds iff every apr–voter κ ≤ the max voter–voter κ; undefined κ is undecided, never holds |
...
After PRM-05 (was REX-05), project the test-split Wilson half-width on recall at the pilot's point estimate
