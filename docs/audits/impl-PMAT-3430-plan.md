...
The block below is the API **contract** — names, signatures, semantics — not literal source. The implementation additionally carries what `crates/aprender-quant/Cargo.toml`'s lint table demands (`missing_docs`, `pedantic`): a doc comment on every public item and variant, `#[must_use]` on every value-returning method (serve's enum already does this), backticked identifiers in docs. `cargo clippy -p aprender-quant --lib -- -D warnings` is part of Phase 2's acceptance, so none of that is left to taste. `non_camel_case_types` does not fire on variants (measured by lanes 1–3; the three existing enums carry no allow for it).
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]   // + impl Display + impl std::error::Error
pub enum GgmlTypeError { Removed { id: u32, name: &'static str }, Unknown { id: u32 } }
impl GgmlType {   // THE COMPLETE leaf API. Every name and signature that exists on any of the three enums today is here, unchanged.
    // from serve's GgmlQuantType (crates/aprender-serve/src/gguf/types.rs:136-221):
    pub const fn from_id(id: u32) -> Option<Self>;          // match, never transmute; all 35 live ids
    pub const fn as_id(self) -> u32;
    pub const fn as_byte(self) -> u8;                        // every id <= 42
    pub const fn as_str(self) -> &'static str;               // == TRAITS[id].name; measured equal to serve's 16 strings today ("Q4_K", "IQ2_XXS", "BF16", …)
    pub fn from_str_lossy(s: &str) -> Option<Self>;          // exact upstream name or its all-lowercase form, as today
    // from compute's GgmlType (crates/aprender-compute/src/inference/gguf.rs:68-101):
    pub const fn block_bytes(&self) -> usize;                // == TRAITS[id].type_size; `&self` kept so compute's call sites are untouched
    pub const fn block_size(&self) -> usize;                 // == TRAITS[id].blck_size
    pub const fn tensor_bytes(&self, n_elements: usize) -> usize; // compute's rounding-up formula, unchanged
    // new:
    ...
    pub const fn checked_tensor_bytes(self, n_elements: usize) -> Option<usize>; // reads TRAITS directly (const-callable); None on overflow or n % blck_size != 0; M4 (#3432) adopts it
}
impl std::fmt::Display for GgmlType { /* as_str, as serve has today — the only trait impl on any of the three (measured) */ }
// `blck_size` / `type_size` are TRAITS FIELD names only (upstream's spelling); there is no second method spelling.
```
...
- **Q1-c. M1 is behaviour-preserving: no crate accepts an id or a name it refused before — enforced at every id→enum and name→enum BOUNDARY, not only at the GGUF parser.** v1 gated the parser only; the grill found boundaries outside it. The measured list — serve: `gguf/dtype.rs:17` and `:54`, `infer/mod.rs:39`, `apr/mod.rs:324` (APR dtype byte), `apr/special_tokens.rs:210`, `apr/dequant.rs:235`, test `gguf/tests/loader_tests_apr.rs:311`. compute: private `from_u32` in `inference/gguf.rs`. **core has NO boundary** (v2 was wrong): `crates/aprender-core/src/format/gguf/reader.rs:325` keeps `dtype: u32` raw and nothing in core turns an integer or a string into `GgmlType` — core builds the enum from literals only, so core gets no admission function and no `u32` field is retyped. Mechanism: each crate gets ONE admission function (`admitted_from_id(u32) -> Option<GgmlType>`, and in serve `admitted_from_name(&str)`) that is `GgmlType::from_id` filtered by a `const ADMITTED: [GgmlType; N]` reproducing the former set exactly (compute 15, serve 16), and every boundary above calls it instead of `from_id`/`from_str_lossy`. **Boundary function names survive:** compute's private `fn from_u32(v: u32) -> Option<GgmlType>` stays under that name as a free function in `inference/gguf.rs` and IS compute's admission function (it cannot remain an inherent method of a re-exported type); its one caller changes from `GgmlType::from_u32(x)` to `from_u32(x)`. serve's boundary functions (`gguf/dtype.rs`, `infer/mod.rs:39`, `apr/mod.rs:324`, `apr/special_tokens.rs:210`, `apr/dequant.rs:235`) keep their names and signatures. So the Phase 1 characterization tests call the same symbols before and after; the single permitted edit to a Phase 1 test in Phase 3 is that `GgmlType::from_u32` → `from_u32` path change, named here in advance. **Proof is a characterization test written in Phase 1, before any change,** placed as a `#[cfg(test)]` child module of each boundary's own module (a child module sees its ancestors' private items, so no visibility is widened; the only non-test edit in Phase 1 is the `mod …;` line)**:** for each boundary function, its output for every id 0..=255 (and every name in `TRAITS` plus case variants) is snapshotted on `origin/main`, and the same test must pass unchanged after Phases 3–4. Wildcard arms (`_ =>`; ~22 across 8 files that name these enums, e.g. `crates/aprender-compute/src/inference/model.rs:734` "quantized norms … return zeros") are therefore not widened in practice: they are already reached today by admitted quantized ids, and a non-admitted variant can only arrive by a new literal in new code. Phase 3/4 receipts still list every wildcard site with its disposition. **Exhaustive matches (no wildcard) stop compiling — E0004 — and that is the compiler enumerating them.** Rule: such a match is replaced by delegation to the leaf (`TRAITS`-backed method), never patched with a bare `_ =>`. One is known: `GgufTensor::byte_size`, `crates/aprender-core/src/format/gguf/types.rs:361`, becomes `self.dtype.tensor_bytes(elements as usize)`. The hand-written arm sizes `Q4_1` at 18 bytes/block (`Q4_0 | Q4_1 => … * 18`, `:365`); upstream and `TRAITS` say 20 — a latent export-size defect. **It is NOT fixed inside M1** (round 3, lane 1: a fix riding a refactor breaks M1's own behaviour-preserving claim; adopted). It lands first as its own issue and its own small PR — Phase 0b in §4: split the arm, `Q4_1 => … * 20`, one test. After that, delegation in Phase 3 is value-identical for all 12 of core's variants, and Phase 1 asserts exactly that: `byte_size` for each of the 12, on a block-aligned and a non-aligned element count, snapshotted before and unchanged after. Any OTHER exhaustive match the compiler reports whose delegated value differs from its hand-written arm is a stop-the-line finding, handled the same way — never a silent fix.
...
- **Q1-f. #3405's private `QTYPE_LABELS` (13 rows, `crates/aprender-serve/src/gguf/qwen3_moe_load.rs:90`)** is replaced in this ticket — it is a fourth table of the same fact. `qtype_label(qtype: u32)` (`:107`) takes an arbitrary `u32`, so the replacement is `GgmlType::from_id(qtype).map_or_else(|| format!("qtype {qtype}"), |t| format!("{}({qtype})", t.as_str()))` — never a raw `TRAITS[id]` index, which would panic for id ≥ 43. The 13 existing labels are measured equal to upstream names. Second named delta, diagnostics only: ids the 13-row table lacked (e.g. 9, 15–29) now render their name instead of `qtype N` in that error message.
...
|---|---|---|---|
| 0b | **separate PR, lands first:** core `GgufTensor::byte_size` sizes `Q4_1` at 20, not 18 (own issue) | crates/aprender-core/src/format/gguf/ | `cargo test -p aprender-core --lib -- format::gguf` incl. a new `Q4_1` test that is RED before the one-line fix |
| 1 | RED: falsifier guard, upstream size fixture, boundary characterization tests (snapshot taken on unchanged code; `#[cfg(test)]` child modules + their `mod` lines only) | scripts/check_one_ggml_type_enum.sh, contracts/ggml-type-v1.yaml, crates/aprender-quant/tests/, crates/aprender-serve/src/gguf/, crates/aprender-serve/src/apr/, crates/aprender-serve/src/infer/, crates/aprender-compute/src/inference/, crates/aprender-core/src/format/gguf/ | `bash scripts/check_one_ggml_type_enum.sh` exits 1 naming exactly 3 definitions; its case table passes (must-match: the 3; must-not-match: `GgufValueType`, `GgufValue`, `QuantType`, a commented-out enum); characterization tests GREEN on unchanged code |
| 2 | `GgmlType` + `TRAITS[43]` in aprender-quant | crates/aprender-quant/ | `cargo test -p aprender-quant && cargo clippy -p aprender-quant --lib -- -D warnings` (lib AND tests/): every live id round-trips, 8 removed ids are `None` / `Err(Removed)`, `TRAITS` vs the pinned fixture with the exception list exactly `[Q8_1]` |
| 3 | compute + core re-export, compute admission, `Bf16`→`BF16`, `GgufTensor::byte_size` delegates (value-identical for all 12 after Phase 0b) | crates/aprender-compute/src/inference/, crates/aprender-core/src/format/ | `cargo test -p aprender-compute --lib -- inference:: && cargo test -p aprender-core --lib -- format::` — includes the Phase 1 characterization tests, unchanged |
| 4 | serve re-export, admission at all 7 boundaries, `QTYPE_LABELS` removed via `from_id` (no raw index) | crates/aprender-serve/src/gguf/, crates/aprender-serve/src/apr/, crates/aprender-serve/src/infer/, crates/aprender-serve/tests/apr_coverage.rs | `cargo test -p aprender-serve --lib -- gguf:: apr:: infer:: && cargo test -p aprender-serve --test apr_coverage` + `bash scripts/check_one_ggml_type_enum.sh` exits **0** + `cargo check --workspace --all-targets` (apr-cli's `tests_trace_dispatch.rs` uses core's re-export) |
| 5 | DoD: `pv validate`, mutation (re-add a second enum → guard RED; change one `ADMITTED` set → characterization RED; set `TRAITS[9].type_size = 40` → fixture RED; `TRAITS[3].type_size = 18` → core byte_size characterization RED), diff quorum, receipt, PR | docs/audits/, .github CI selection line (needs the operator's web-UI merge click) | `make gate` |
...
### Round 2 — v2, 3 lanes, same models: **1/3 PASS** (lane 3 PASS with notes; lanes 1, 2 FAIL)

Reducer: `agreed=false`, `partial=true` — lane 2 exited 3, `LANE ISOLATION VIOLATED - a shared ref changed (refs/remotes/origin/HEAD, refs/remotes/origin/main)`. repo_root is a linked worktree sharing the main checkout's git dir; another session fetched `origin` during the lane window. Lane 2's own clone came back byte-identical. Its findings were kept because each was re-run below, not because its verdict was counted.

| finding | lanes | re-run | disposition in v3 |
|---|---|---|---|
| exhaustive match `GgufTensor::byte_size`, core `format/gguf/types.rs:361` → E0004 | 1, 2, 3 | confirmed; **and its `Q4_1` arm is 18 where upstream is 20** (found on the re-run, no lane saw it) | Q1-c: delegation rule + the named Q4_1 delta |
| core `format/gguf/reader.rs` is not a boundary (stores raw `u32`) | 1, 2 ; lane 3 agreed core has none | grep: no `u32 → GgmlType` anywhere in core | Q1-c corrected; core gets no admission function |
| §2 API block omitted `as_byte as_str from_str_lossy block_bytes block_size Display` | 2 ; 3 (`block_size` vs `blck_size`) | only trait impl on the three is serve's `Display` (grep) | §2 block is now the complete API; one method spelling |
| `TRAITS[id]` raw index panics for `qtype ≥ 43` in `qtype_label` | 2 | `qwen3_moe_load.rs:107` takes arbitrary `u32` — confirmed | Q1-f: via `from_id` |
| cargo test filters `inference:: format:: gguf:: apr:: infer::` select tests | 2, 3 verified | — | unchanged |

### Round 3 — v3, 3 lanes, same models: **1/3 PASS** (lane 3 PASS; lanes 1, 2 FAIL). Reducer `partial=false`; no isolation violation this round. Lanes 1 and 2 compiled probe crates inside their sandboxed clones (KEPT, nothing escaped).

No lane disputed the architecture any more: home crate, 35/43, Q8_1 = 36, boundaries (compute 1, serve 7, core 0), `byte_size` as the only exhaustive match, Table E and X1 were each called sound by all three. What failed is below.

| finding | lanes | re-run | disposition in v4 |
|---|---|---|---|
| Q4_1 18→20 must not ride the refactor — land it first, separately | 1 blocking ; 2, 3 accepted it in-ticket | the 18 is confirmed (`types.rs:365`) | adopted: Phase 0b, own issue + PR |
| §2 block fails `clippy -D warnings` in aprender-quant (`missing_docs`, `must_use_candidate`, doc backticks) | 1, 2 | lint table read: `missing_docs = warn`, `pedantic = warn` | §2 preamble: block is the contract; clippy is in Phase 2's `A_i` |
| aux types lack derives; `GgmlTypeError` lacks `Display`/`Error` | 2 | — | added |
| `const fn checked_tensor_bytes` cannot call non-const `block_size` | 2 | — | all size methods `const fn`; `&self` kept |
| Phase 1 tests on compute's private `from_u32` break in Phase 3 (E0599) | 2 ; lane 1 said reachable | both are right: reachable from a child module, and the inherent method cannot survive a re-export | Q1-c: `from_u32` survives as a free fn; the one path edit is named in advance |
| Phase 1 scope omits serve `gguf/dtype.rs` | 2 | `gguf/tests/` was too narrow — confirmed | Phase 1 scope widened to the boundary modules' directories |
