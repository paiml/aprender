...
impl GgmlType {   // THE COMPLETE leaf API. Every name and signature that exists on any of the three enums today is here, unchanged.
    ...
    pub fn block_bytes(&self) -> usize;                      // == TRAITS[id].type_size
    pub fn block_size(&self) -> usize;                       // == TRAITS[id].blck_size
    pub fn tensor_bytes(&self, n_elements: usize) -> usize;  // compute's rounding-up formula, unchanged
    // new:
