...
| `crates/apr-cli/src/dispatch.rs:177` "that path is ~20x slower (~20 tok/s vs ~400)" | S-1 / S-3 | **remove in R-7** (CLI output is in the ratchet's universe): the refusal names the path, never a ratio |
| `crates/apr-cli/src/commands/chat_load_tokenizers.rs:230` "For ~4x faster inference" | S-1 | **remove in R-7** |
...
| `book/src/examples/qwen-inference.md:9` "225+ tok/s GPU", `book/src/examples/showcase-benchmark.md:20` "851.8 tok/s", `book/src/best-practices/performance.md:250` | S-1 / I-18 | **remove in R-7** (the book is in the guard's universe; R-7 owns the user-facing rewrite) |
| `crates/aprender-zram/README.md:25` "19x faster", `crates/aprender-viz/README.md:35` "2X faster than btop's" | S-* (speed) | **remove in R-7** AND widen the ratchet: `crates/*/README.md` and `Cargo.toml` `description` are outside `check_no_claim_literals.sh`'s universe (`:1091` unions `book/**/*.md`, `docs/**/*.md`, root `*.md`, `crates/*/src/**/*.rs`) — SPEC-2.0 follow-up: widen the universe and RE-MUTATE in the widened scope (baseline set-aperture) |
| `docs/BEATS.md:121` (~5000× cold-start), `:131` (1.015–1.109× Ollama parity) | — | **keep — receipted** (the beat scoreboard is gated against `contracts/` by `readme_contract.rs`; the 1.371× headline at `:245` is the WITHDRAWN record and must never be quoted as live — lane 2's `:38` ruling was a misplaced citation and is rejected) |
