...
- **`excessive_precision`.** Inheriting `[workspace.lints]` is not possible. Cargo forbids inherit + override, and the
...
- **Refusals, never verdicts.**
  ...
  - A partial workspace: every member must be checked, 79/79.
  - A different clippy version, recorded in the baseline: the lint set is not monotonic.
- **First green on a real run.** 79 members checked; 3262 == baseline; 2m28s warm on lambda.
...
- **Case table.** `--self-test`, 9 rows, no cargo: unchanged green; rise; new key; stale; key gone; unfinished;
  no build-finished; note-level ignored; the version-only package-id form (a real parse bug found on the first
  census: aprender-core came out as "0.69.0").
