...
- **M8 (shift-left)**
  - release_gate_classes (69 derived gates, 47 real / 22 bookkeeping after the Sonnet round made `contracts` real; 13 rows / 11 mutants).
  ...
  - autopilot watch_gate/run_preflight: the publish re-reads the watch, aged by its own timestamp (check_publish_reads_watch.sh, 6 rows / 5 mutants).
