---
status: partial
partial_reason: "waiting on C0-5 (#2985) to merge — this branch rebases onto it, bumps the README contract count, then opens its PR; flip to complete with the DAG write-back after merge"
ticket: PMAT-980
row: G-6
issue: 2895
epic: 2873
...
| check | result |
|---|---|
| `bash scripts/check_roadmap_diff_additive.sh --self-test` | rc 0, `14/14 rows` — row 9: the real `6d9ba274a` re-serialisation reads `reserialised=361`, trimmed → PASS |
