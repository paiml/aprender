...
| PMAT-4072: the FRAGMENT changes only `notes` (null → cites `docs/audits/quorum-PMAT-4072-fe8c9dff7.json`). The generated `roadmap.yaml` row ALSO changes `status` inprogress→completed and `assigned_to` aprender-ont3a→aprender-ea: not an edit — at e5929e6e5 the fragment `docs/roadmaps/entries/PMAT-4072.yaml` already said `completed`/`aprender-ea` (commit 5c1dbe01f), and the fold's committed aggregate was stale against it; `make roadmap-aggregate` (entries/ is the source of truth) synced it | check_roadmap_completion_is_cited: "FAIL docs/roadmaps/roadmap.yaml PMAT-4072 claims completed and cites nothing, and is NOT in the frozen baseline — so it is a NEW unprovable claim" |
| PMAT-4438 fragment adopted, PMAT-4431 fragment, roadmap.yaml regenerated. Measured: every aggregate row that differs e5929e6e5→HEAD is PMAT-4072 (above; HEAD equals its fragment at e5929e6e5 on status/assigned_to) and PMAT-4431 (added); no other row changes | check_roadmap_fragment_required / check_roadmap_sorted (read committed HEAD); rc 0 after |
