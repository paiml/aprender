...
| P0 | mcp | `apr mcp (stdio transport, all tools/call)` | mcp:apr mcp | #2393 | #2434 | tools/call responses are silently DROPPED when stdin reaches EOF — server exits 0 having answered initialize and tools/list but not the tool call |
| P0 | mcp | `apr.serve` | mcp:apr.serve | #2388 | #2415&nbsp;✅ | apr.serve builds an argv the 0.63.0 CLI rejects (`apr serve <model>` needs a subcommand) — child dies with clap exit 2, tool returns SUCCESS with a pid+url that is already dead |
...
| P1 | cli | `cbtop` | cli:cbtop | #2397 | #2432 | `apr cbtop --iterations 0` fabricates a perfect PASS/green report (7/7 falsification, all bricks 100/A, 989 tok/s) from zero measurements — turning any CI gate green |
| P1 | cli | `cbtop` | cli:cbtop | #2397 | #2432 | `apr cbtop --ci` exits 0 while the report it just printed says "Status: FAIL / CI: red" — the CI gate ignores its own verdict |
...
| P1 | cli | `rosetta convert` | cli:rosetta | #2382 | #2420&nbsp;✅ | `apr rosetta convert --quantize` is a no-op for every value (int8, int4, fp16) and silently accepts garbage values — output size is identical to an unquantized convert |
| P1 | cli | `rosetta verify` | cli:rosetta | #2382 | #2420&nbsp;✅ | `apr rosetta verify --json` emits invalid JSON — bare `inf` literals are rejected by standard parsers |
| P1 | cli | `run` | cli:run | #2378 | #2427 | `apr run --stream` emits one event per token but every event's `text` field is empty, so nothing streams |
| P1 | cli | `run` | cli:run | #2378 | #2427 | `apr run --trace-output FILE` always writes `"events": []`, and `--trace-level chrome` ignores `--trace-output` and writes trace-<epoch>.json into the CWD |
| P1 | cli | `run` | cli:run | #2378 | #2427 | `apr run --trace --trace-level layer` prints per-step timings that are hardcoded fractions of wall time, presented as measurements, and contradicts the real profile in the same output |
| P1 | cli | `run` | cli:run | #2378 | #2427 | `apr run` silently accepts a GGUF whose final-norm tensor is half the declared hidden size — exit 0, no warning, altered output |
| P1 | cli | `run` | cli:run | #2378 | #2427 | `--backend`, `--trace-level` and `-f/--format` accept any string silently; `--backend <typo>` prints "Backend override: <typo>" and quietly runs the default backend |
| P1 | cli | `run (--features cuda build)` | cli:run | #2378 | #2427 | `--features cuda` build dumps developer debug traces — raw pointers and raw tensor values — to stderr on a normal `apr run` |
| P1 | cli | `run (wgpu Q4_K numerics)` | cli:run | #2378 | #2427 | The wgpu Q4_K backend is numerically wrong on RTX 4090 (cosine 0.884 vs CPU) and is still tried first by default |
...
| P1 | http | `POST /v1/embeddings, POST /v1/chat/completions/stream` | http:openai | #2375 | #2367&nbsp;✅ | /v1/embeddings and /v1/chat/completions/stream are registered and advertised but always 404 "No model available" even with a model loaded |
| P1 | http | `POST /v1/explain` | http:openai | #2375 | #2367&nbsp;✅ | /v1/explain returns hardcoded fake SHAP values and prediction=0.95 with HTTP 200 — output is independent of the input features and of whether any model is loaded |
| P1 | http | `GET /realize/model` | http:realize | #2402 | #2435 | /realize/model returns fabricated provenance and model metadata: content_hash is the literal string "blake3:0" repeated 16 times, size_bytes is hardcoded 0, and context_length/quantization/format are hardcoded constants that contradict the loaded model |
| P1 | http | `POST /realize/reload` | http:realize | #2402 | #2435 | /realize/reload's 501 tells the user to "Start server with --registry flag" — `apr serve run` has no --registry flag, so the endpoint is unreachable by any documented invocation |
...
| P1 | cli | `check` | json-flag-emits-text | #2390 | #2411 | `apr check --json` exits 0 when stages FAIL; the same command without --json exits 5 |
...
| P1 | cli | `awq-lint, gptq-lint, imatrix-lint, fp8-lint` | lint-passes-on-bad-obs | #2389 | #2431 | Four lints read the pass threshold from the very observation file they are linting, with no CLI override — a failing artifact self-certifies and the lint exits 0 |
| P1 | cli | `dry-sampling-lint, gbnf-lint` | lint-passes-on-bad-obs | #2389 | #2431 | dry-sampling-lint and gbnf-lint exit 0 on an observation with no recognized keys (and on wrong-typed fields) — a vacuous PASS, while all 10 sibling lints fail closed |
| P1 | cli | `gbnf-lint, dry-sampling-lint` | lint-passes-on-bad-obs | #2389 | #2431 | gbnf-lint and dry-sampling-lint exit 0 (PASS) when a sub-field has the wrong JSON type — a real, detectable violation is silently suppressed |
| P1 | cli | `lint` | lint-passes-on-bad-obs | #2389 | #2431 | apr lint can never exit 0 — exits 5 with "0 error(s)" on all 18 models tested (including pristine ones), and has no flag to change the warning gate |
| P1 | cli | `nf4-lint` | lint-passes-on-bad-obs | #2389 | #2431 | `nf4-lint` codebook falsifier prints "codebook matches (16 entries)" and exits 0 when the observation supplies NO codebook at all (empty, wrong-typed, or mistyped field) |
| P1 | cli | `ollama-tools-lint` | lint-passes-on-bad-obs | #2389 | #2431 | `--request-file` is documented as "Optional … enables tool-name allowlist gate", but the allowlist gate runs unconditionally and NO input passes without it |
| P1 | cli | `otlp-lint` | lint-passes-on-bad-obs | #2389 | #2431 | otlp-lint with no gate flags runs zero checks and exits 0 on any valid JSON, printing a header that reads as a clean PASS report |
| P1 | cli | `tool-use-lint` | lint-passes-on-bad-obs | #2389 | #2431 | `tool-use-lint` exits 0 on an empty or mistyped observation, reporting all three gates as "classifier skipped" |
| P1 | cli | `typical-p-lint, tool-use-lint` | lint-passes-on-bad-obs | #2389 | #2431 | typical-p-lint / tool-use-lint exit 0 on present-but-wrong-typed fields and mislabel them '(missing fields — classifier skipped)'; exit 0 when zero classifiers ran |
| P1 | cli | `unified-search-lint` | lint-passes-on-bad-obs | #2389 | #2431 | unified-search-lint prints [PASS] and exits 0 while its own output line says expected_count_ok=false — string-typed expectations silently disarm the gate |
...
| P1 | cli | `kv-timeline-lint` | nan-threshold-disarms | #2391 | #2422 | `--preempt-threshold NaN` (or `=-1`) silently disarms the preemption gate: the same body that FAILS at 0.95, 99 and inf reports `preemption_trigger : Ok` and exits 0 |
| P1 | cli | `typical-p-lint` | nan-threshold-disarms | #2391 | #2422 | `typical-p-lint` silently skips a classifier when its section is mistyped or wrong-typed and exits 0 — and is internally inconsistent about it (`mass` rejects a bad type, `range` swallows it) |
...
| P1 | cli | `--quiet (global)` | quiet-verbose-inert | #2401 | #2439 | `--quiet` is advertised in every subcommand's help but is a byte-for-byte no-op on 14 of 16 runnable commands |
| P1 | cli | `all 16 lint commands (-q / --quiet)` | quiet-verbose-inert | #2401 | #2439 | `-q, --quiet  Quiet mode (errors only)` is advertised in every lint's --help but has no effect — the full report is still printed to stdout |
| P1 | cli | `inspect,debug,validate,tensors,tree,hex` | quiet-verbose-inert | #2401 | #2439 | -q/--quiet and -v/--verbose are advertised on every command in the family and produce byte-identical output on all of them |
...
| P2 | cli | `cbtop` | cli:cbtop | #2397 | #2432 | cbtop benchmark reports carry a hardcoded date (2026-01-11) — the timestamp is ~7 months wrong in both text and JSON output |
| P2 | cli | `cbtop` | cli:cbtop | #2397 | #2432 | `apr cbtop --json` without `--headless` silently launches the interactive TUI instead of honouring the documented "requires --headless" constraint |
| P2 | cli | `code` | cli:code | #2398 | — | `apr code --resume <nonexistent-session-id>` silently starts a fresh session and exits 0 |
| P2 | cli | `convert (output::kv_table)` | cli:convert-export | #2392 | #2437 | Single-row kv_table renders an unterminated box - header separator emitted with no bottom border |
| P2 | cli | `export` | cli:convert-export | #2392 | #2437 | apr export reports original_size as the dequantized tensor total, not the input file size - it is constant regardless of input and contradicts the same command's --plan input_size |
| P2 | cli | `export / merge / shard` | cli:convert-export | #2392 | #2437 | export, merge and shard silently destroy pre-existing output files and have no --force flag, while convert and quantize in the same CLI refuse without --force |
...
| P2 | cli | `hex` | cli:diag-misc | #2394 | #2428&nbsp;✅ | apr hex with no mode flag takes 105 s / 2.0 GB RSS on a 533 MB GGUF and 221 s / 3.1 GB on a 1.0 GB GGUF, where apr tensors reads the same file in 0.6 s |
| P2 | cli | `inspect` | cli:diag-misc | #2394 | #2428&nbsp;✅ | apr inspect --vocab and --weights are stubs on APR files that redirect the user to `apr tensors`, which shows neither vocabulary nor weight statistics |
| P2 | cli | `inspect` | cli:diag-misc | #2394 | #2428&nbsp;✅ | inspect's "Quantization" field reports the embedding tensor's dtype, mislabelling a Q4_K_M model as Q6_K and contradicting the general.file_type it prints in the same output |
| P2 | cli | `inspect` | cli:diag-misc | #2394 | #2428&nbsp;✅ | inspect given a directory silently picks one model inside it and never names the file in human output |
| P2 | cli | `inspect,validate` | cli:diag-misc | #2394 | #2428&nbsp;✅ | inspect --quality and validate --quality report two different 0-100 scores for the same model (60/100 vs 3/100) |
| P2 | cli | `trace` | cli:diag-misc | #2394 | #2428&nbsp;✅ | `apr trace` reports "Layers: 1 / No layer information in metadata" for .apr files -- its own help calls .apr the expected input, and sibling commands read 24 layers from the same file |
| P2 | cli | `trace --diff` | cli:diag-misc | #2394 | #2428&nbsp;✅ | `apr trace --diff` without `--reference` prints "Diff mode requires --reference" and then runs a normal trace anyway, exiting 0 |
...
| P2 | cli | `kv-timeline-lint,nccl-diag-lint,ollama-chat-lint,ollama-tool` | cli:lint-family | #2377 | #2438 | A malformed JSON observation file is reported as `error: Invalid APR format:` — these commands never read an APR model, and the message sends users to the wrong file |
| P2 | cli | `nf4-lint,registry-quota-lint,rm-gc-lint,shared-cache-lint,un` | cli:lint-family | #2377 | #2438 | The 16-command lint family splits into two incompatible exit-code dialects: missing file is 3 for eleven commands and 1 for five; bad JSON is 4, 5, or 1 depending on which you called |
| P2 | cli | `quant-preservation-lint` | cli:lint-family | #2377 | #2438 | quant-preservation-lint dumps ~13 MB of stdout (19 lines) by inlining entire tokenizer vocab and merge tables into the divergence report |
| P2 | cli | `quant-preservation-lint` | cli:lint-family | #2377 | #2438 | `quant-preservation-lint` emits a 13 MB report with a single 4.8 MB line when a tokenizer array diverges — it inlines the whole 151k-entry vocab as a Rust Debug string |
| P2 | cli | `typical-p-lint` | cli:lint-family | #2377 | #2438 | `apr typical-p-lint --help` prints its only required flag with an empty description, unlike all 15 sibling lint commands |
| P2 | cli | `profile` | cli:profile | #2395 | #2440 | `apr profile` advertises --energy/--callgraph/--fail-on-naive that print "not yet implemented. Flag ignored.", and every run reports Hardware as "Unknown Unknown (24 cores, 512)" plus a CONTRACT WARN |
...
| P2 | http | `GET/POST unknown route (404 body)` | http:native | #2376 | #2429&nbsp;✅ | The 404 body tells clients "See /health for available endpoints", but /health returns no endpoint list |
| P2 | http | `POST /generate` | http:native | #2376 | #2429&nbsp;✅ | max_tokens is unbounded and generation is not cancelled when the client disconnects — one abandoned request keeps ~250% CPU busy for the rest of the server's life with zero open connections |
...
| P2 | cli | `hang-trace-lint` | lint-passes-on-bad-obs | #2389 | #2431 | hang-trace-lint --world-size 0 unconditionally reports `Ok { ranks_seen: 0 }` and exits 0 regardless of what the trace directory contains |
| P2 | mcp | `apr mcp / JSON-RPC batch array` | mcp:apr mcp | #2393 | #2434 | A JSON-RPC batch array is rejected with a serde-internal message ("invalid type: map, expected a string") that tells the client nothing about batching |
| P2 | mcp | `apr mcp / JSON-RPC error codes` | mcp:apr mcp | #2393 | #2434 | Well-formed JSON that is missing `jsonrpc` or `method` is reported as -32700 Parse error instead of -32600 Invalid Request — inconsistent with the server's own handling of a WRONG jsonrpc value |
| P2 | mcp | `apr mcp / ping` | mcp:apr mcp | #2393 | #2434 | The base-protocol `ping` method returns -32601 Method not found; a client using ping keepalive will treat the server as dead |
| P2 | mcp | `apr mcp / tools/call argument validation` | mcp:apr mcp | #2393 | #2434 | A required argument supplied with the wrong JSON type is reported as "Missing required argument", contradicting the request the client actually sent |
| P2 | mcp | `apr.run / apr.bench (relayed CLI error text)` | mcp:apr.run | #2403 | #2433 | Error text relayed through MCP carries a doubled 'Inference failed: Inference failed:' prefix |
...
| P2 | cli | `beat-run` | nan-threshold-disarms | #2391 | #2422 | `apr beat-run` leaks Rust `Some(..)` Debug formatting into user-facing output, blames the contract for a non-finite --measured value, and reports a missing contract file as "Invalid APR format" |
| P2 | cli | `imatrix-lint` | nan-threshold-disarms | #2391 | #2422 | imatrix-lint stamps the same falsify_id (FALSIFY-CRUX-B-07-001) on two distinct gates, so a --json consumer keying on falsify_id cannot tell them apart |
| P2 | cli | `kv-timeline-lint --preempt-threshold` | nan-threshold-disarms | #2391 | #2422 | --preempt-threshold=nan is accepted and silently disarms the preemption gate, which then positively reports 'preemption_trigger : Ok' on a body that fails at the default threshold |
| P2 | cli | `stamp` | nan-threshold-disarms | #2391 | #2422 | `apr stamp --license` writes arbitrary non-SPDX text into an artifact's provenance, while `apr validate-manifest` enforces SPDX on the same field |
| P2 | cli | `all 16 lint commands (-q/--quiet, -v/--verbose)` | offline-fails-open | #2379 | #2416&nbsp;✅ | -q/--quiet ('Quiet mode (errors only)') and -v/--verbose are advertised in every lint's --help but are completely inert — output is byte-identical |
| P2 | cli | `kv-timeline-lint,nf4-lint,prometheus-lint,typical-p-lint (al` | offline-fails-open | #2379 | #2416&nbsp;✅ | `-q/--quiet` ("Quiet mode (errors only)") and `-v/--verbose` are advertised on all 16 lint commands and have no effect on any of them |
