...
|---|---|---|---|
| `check_build_report.sh` 44/44 | worker | 44/44, exit 0 (16/16 when first written; 35 at adoption; 42 after round 2's seven rows; 44 after round 3 replaced the vacuous bare-`gate` source-grep with two behavioural rows and pinned the binding-check row's required set; counted with `grep -cE '^\s+ok '` on the self-test output) | yes |
...
3. **Turns 26 vs 43+45 — two different quantities, and this note first described them wrongly.** Round 3's lane 1 read `impl-estimates.jsonl:43` and was right to: the row is `{ticket: PMAT-3225, phase: "all", est: 0, actual: 26, unit: "turn"}` — `actual: 26` is the **orchestrator's** per-ticket turn total, the unit `estimate.sh` pools, and `est: 0` is the (absent) K̂. The receipt's **43** and **45** are the subagent worker's and the delegate's **tool-use counts** (§ dispatch table above: "43 tool uses / 127,706 tok"), a different quantity. Neither claims to be the other; the round-2 version of this note said the JSONL held K̂, which was false for this row and is corrected here. Data unchanged.

**Round 3 on `7748875cd` (2 PASS / 1 FAIL), lane 1 again, three findings, all real and applied:** (a) the binding-check case row had become network- and state-dependent the moment the set was derived at run time — `BUILD_REPORT_REQUIRED_CHECKS` (a JSON array) now pins it for tests and operators, the report prints `[pinned (BUILD_REPORT_REQUIRED_CHECKS)]`, and the derive path keeps its own un-pinned and hidden-`gh` rows; (b) the "bare `gate` spelling" row grepped the **script's own source** for the literal `"gate"` — vacuous by construction — and is replaced by a ledger that records the gate under both spellings and asserts both are measured and the slower binds; (c) the turns note, corrected above.
