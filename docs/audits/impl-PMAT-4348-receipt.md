...
| Ask | This diff |
|---|---|
| 1 narrow imports | 2 bare `import Mathlib` removed. RegressionAnalytic.lean 96.8 s / 6.13 GB -> 19.6 s / 2.34 GB; HeadMapping.lean (old, bare `import Mathlib`) 136.7 s / 6.09 GB -> 7.4 s / 1.92 GB. rc 0, lean v4.29.0-rc4, LEAN_NUM_THREADS=2, 8G/200% scope |
...
| guard | scripts/check_leanchecker_scoped.sh (26/26 case rows) now also scans ci/*.yml, ci/*.cmd and forjar*.yaml. Planted bare leanchecker in ci/sections.yml: old guard rc=0, new rc=1. Also RED in a .cmd fragment, forjar.yaml and the Makefile. Auto-wired, no CI edit needed: guard_tree.sh's universe is `git ls-files 'scripts/check_*.sh'` with no `cargo ` token (grep -c = 0) and a `--help` that advertises `--self-test`; measured `guard_tree.sh --list --no-cargo` prints scripts/check_leanchecker_scoped.sh; `--self-test` 26/26 rows; run: ok, 565 tracked script/CI file(s) |
