...
- **Round 3 (02:17Z, after the gpt-oss reset; head 0b1a3676c+):** gemini still returned 429 "Resets in 26h12m51s", and gpt-oss answered a one-line probe. So only one distinct non-author id was available, and the round is one seat, `--width 1 --lane-model gpt-oss-120b-medium --fallback-model gpt-oss-120b-medium`, with the 2 missing seats requested from the cop. **Result, head f91a4e862: NO-VERDICT.** gpt-oss answered the pre-check probe, then returned 503 "No capacity available" twice on the real brief, and the chain (pinned to gpt-oss, so it could not fall through to a same-family Claude seat) was exhausted. No lane read the diff. The artifact is archived, never committed. The PMAT-3773 and PMAT-3788 single-seat rounds at 02:18–02:28Z had the same outcome. All 3 seats are now requested from the cop.
