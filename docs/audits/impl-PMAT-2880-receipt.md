# impl receipt — PMAT-2880 (slices 1–6), branch 1c/2880-vnni-reachable
...
| 6 | (slice-6 commit) | aarch64 NEON Q4_K×f32 dot (`fused_q4k_dot_neon`), dispatched by `fused_q4k_dot_simd` via `selected_dot_kernel(DotOp::Q4kF32)`; `KNOWN_SCALAR_GAPS` now empty. M4: `q4k_f32_neon_matches_f64_reference` (tol 1e-5·Σ\|w·a\|, dispatcher bit-equal to NEON), per-value probe, rejects; 147/147 incl. every existing `fused_q4k_dot*` test. Mutant (high nibbles scaled by sc1) RED. M4 bench 7.08–7.23× vs scalar. aarch64 clippy: 0 findings in touched files |
