...
## Operator rulings during this train (2026-09-12, verbatim) and what changed

1. **"you DO HAVE SSH (ssh gx10, ssh yoga), etc"** (~16:0xZ) — I had written "no SSH, §3.5" into three tickets and this receipt while gx10-pool3 sat at 100 %. Measured over ssh within minutes: gx10 916 GB, **6.4 GB free**; `/mnt/nvme-raid0 -> /home/noah/eph-work/intel-mirror` (root symlink, 2026-09-10, declared nowhere); 250 GB of per-PR target dirs under `targets/{aprender-ci,sovereign-ci-aprender}/<pr>` (33 dirs, largest 44 GB = #3139's aarch64 workspace-test); the five pool runners are docker containers, not units; no reaper. yoga: 935 GB, 59 %, same symlink, 271 GB in 58 PR dirs, no reaper. **Reclaimed by hand 16:05Z**: 23 dirs by the intel reaper's critical rule (keep newest 3 per root + `gh-readonly-queue` + `main`) + docker builder cache → **146 GB freed** (7 → 153 GB free). Script kept: `gx10-reclaim.sh` (scratchpad).
2. **"also this needs to automated disk clearing/reaping and managed by forjar and p0 if it gets blocked"** — done the same hour: `machines/{gx10,yoga}/forjar.yaml` gain `ci-reaper` + `ci-disk-watch` (ONE script source, intel's; per-host units with thresholds citing the 44 GB measurement: CRITICAL 90 = 2×, PRESSURE 180 = 2× critical; model-cache budgets 80 GB because gx10 holds 62 GB in `~/.apr/cache`), the symlink and targets root declared. `forjar apply -t ci-reaper` (it prompts `[y/N]` — the first attempt hung 10 min on that): gx10 10 converged 2.1 s, yoga 10 converged 3.7 s. Verified on the box 16:27Z: both timers `enabled`+`active` on both hosts; first `ci-reaper.service` run gx10 177 → 226 GB free (49 GB, 13 sweep steps, pressure → normal), yoga 365 → 434 GB (69 GB); `ci-disk-watch.service` ran clean. PR **paiml/infra#549**, auto-merge armed.
3. **"these two boxes: yoga and gx10 should be always 80% full of PRs from aprender if ANY queue pressure on intel: update memory as well and any designs. not acceptable to have slow releases when boxes are idel"** (~16:05Z) — memory: `feedback_yoga_gx10_80pct_of_aprender_prs_under_intel_pressure`; spec amended in this PR (e87476986: selector row 0, §1 packing rule, §3.4 rewritten, §3.5 SSH, §5 P0·Pack + P0·Reap, §7 `pack:` line, §8 stops). Actions: the four PRs I had dequeued were re-armed (#3139 #3175 #3046 #3068), the four withdrawn CI runs re-run (#3163 #3052 #3056 #3048), the two non-additive roadmap PRs trimmed and armed (#3134 #3093 — both still DIRTY on code files: `backward.rs`; `impl-estimates.jsonl` + `tree_reader_tests.txt`; they need a rebase, not a trim). Fleet-pack ledger samples (`*-fleet-pack-HHMM.json`): 16:29Z intel 15/16 gx10 2/6 yoga 2/5 → P0-UNDERUTILIZED; 16:37Z gx10 4/6 yoga 3/5 → still under 0.8. The two idle runners per GPU box are the GPU-only ones (`gx10-blackwell`, `gx10-eph`, `yoga-gpu`, `yoga-eph`): their jobs are short and advisory (gpu-quick, cuda-unit) or the nightlies already dispatched; reaching 0.8 there needs GPU-lane work (§5 P2) or relabelling them clean-room, which would put the release's CUDA asset lane behind hour-long jobs — not done, named for the operator.
4. **"update spec … it is P0 if they are underutilized"** — e87476986 above.

**The resolver defect the packing rule exposed (#3186 → PR #3187).** With the queue holding five entries, groups 34704287677 (#3139) and 34704288441 (#3046) ran their `workspace-test` on gx10-pool3/pool2 (the arch-neutral pin from #3139's own change, stacked) and were RED on ONE guard: `check_roadmap_diff_additive.sh` → `merge-base(origin/main, HEAD) is unresolvable (shallow checkout) and HEAD is not a merge commit nor a commit on the origin/main tip`. GitHub builds entry N as a squash on entry N−1's squash; `scripts/lib/resolve_base.sh` had no rule for a single-parent head whose parent is not the main tip, so with `max_entries_to_build=3` **two of every three queue builds were RED by construction** — the real cause behind this morning's four dequeues and #3177's burn. Fix: under `merge_group`, deepen by one and take that parent (rows 19–20 in the guard's case table, self-contained fixture; mutation measured: origin/main's resolver turns row 19 RED; 20/20; bashrs 0 errors). PR #3187 armed; its own group uses the merged tree's resolver, so it can land stacked. Until it is on main, the four re-armed PRs fail at position ≥ 2 (they did, auto-merge dropped at ~16:40Z) — they are re-armed by `drain-held.sh` right after #3187 merges.

## Post-tag drain — PACKING policy (post-tag.sh steps 1–2 replaced; step 3 is NOT run)
`drain-held.sh` (rewritten 16:35Z under ruling 3): one pass arms **every** open non-draft PR in `drain-order.txt` whose roadmap diff is additive, re-runs withdrawn/red CI first, names non-additive ones for a trim; the queue builds 3 at a time. Re-run every wakeup. Order: 3139 3175 3046 3068 3163 3052 3056 3048 3134 3093. post-tag.sh's step 3 (`ssh yoga … sed -i /etc/github-runner-ephemeral.env` to restore `clean-room` on yoga-eph) is ad-hoc SSH and stays unrun (§3.5); the yoga label lives in the P2 infra PR.
