...
| `cargo test -p aprender-contracts-cli` (every target) | green; `ont_release_readiness` has 25 cases (the green base is synthesized from pv's own derived list) |
...
| the ont4b shape ratchet | 18 → 24; the six new shapes are named in the test |

## Not in this diff (so it carries `Refs`)
- **S2.5's sampling controls** (a8's: sampled ≠ greedy; same seed ×2 identical; seed A ≠ seed B; top-k 1 / T 0 greedy). Sampling args are numeric (role `other`), and the v1.1 surface carries no typed marker that says "this arg samples", so pv cannot derive them without a name list. That would be the #3745 defect again. It needs a `SamplingArg` marker from S1; I've raised it with the cop.
