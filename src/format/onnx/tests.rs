...
fn test_real_onnx_file_debug() {
    ...
    let onnx_path_buf = std::path::PathBuf::from(&home).join("src/trueno-rag/.fastembed_cache/models--Qdrant--all-MiniLM-L6-v2-onnx/snapshots/5f1b8cd78bc4fb444dd171e59b18f3a3af89a079/model.onnx");
    let path = onnx_path_buf.as_path();
    if !path.exists() {
