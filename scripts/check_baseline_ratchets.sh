...
#   set-aperture
#         — `set`, plus the admissions a widened guard needs (PERF-049). Two
#           entry shapes:
#             <path>:<line>            admitted when that line is BYTE-IDENTICAL
#                                      at the comparand AND the diff changes the
#                                      owning guard, named in the value.
#             <path>TAB<hash>TAB<n>    admitted when the comparand's copy of
#                                      <path> already carries n lines with that
#                                      content key, per the owning guard's own
#                                      `--keys-of`. NOT gated on a guard edit: a
#                                      claim that merely MOVED is not an aperture
#                                      change, and requiring one there is the
#                                      line-drift red with extra steps.
#           Everything else is refused as `set` refuses it, and every admitted
#           entry is printed. Without this the guard could never be widened at
#           all: `check_no_claim_literals.sh` could not see the `2.93× Ollama`
#           it was built for, and reading it reveals 18 claims that were already
#           in the tree. See lib_baseline_ratchet.sh.
       ...
       && git -C "$SR" -c commit.gpgsign=false commit -qm 'no baseline yet' >/dev/null 2>&1; then
        ...
        if git -C "$SR" add -A \
           && git -C "$SR" -c commit.gpgsign=false commit -qm 'add baseline' >/dev/null 2>&1; then
            ...
            # -- set-aperture, CONTENT-KEYED entries. A coordinate is not a
            # property of a claim: PERF-006 wrote no claim and slid three
            # baselined ones onto new lines, which (a) reads as newly written
            # with no legal remedy. The content shape exists for that, and
            # every branch of it must be shown to REFUSE.
            #
            # The scratch guard is a real keyer, because the library asks the
            # OWNING GUARD for the key rather than re-deriving it. `keyer.sh`
            # is byte-identical between the comparand and the working tree on
            # purpose: it pins that (b) does NOT gate this shape.
            cat > "$SR/keyer.sh" <<'KEYER'
#!/usr/bin/env bash
# Scratch keyer: the same normalisation and key as check_no_claim_literals.sh.
set -euo pipefail
[ "${1:-}" = "--keys-of" ] || { printf 'usage: --keys-of <file> <display>\n' >&2; exit 2; }
kf="${2:?}"; kd="${3:?}"
[ -f "$kf" ] || { printf 'no such file\n' >&2; exit 2; }
while IFS= read -r l || [ -n "$l" ]; do
    s="${l//$'\t'/ }"
    while [ "$s" != "${s//  / }" ]; do s="${s//  / }"; done
    s="${s#"${s%%[![:space:]]*}"}"; s="${s%"${s##*[![:space:]]}"}"
    h=$(printf '%s' "$s" | sha256sum)
    printf '%s\t%s\n' "$kd" "${h:0:16}"
done < "$kf"
KEYER
            chmod +x "$SR/keyer.sh"
            ...
            printf 'GUARD v1\n' > "$SR/guard.sh"
            git -C "$SR" add -A >/dev/null 2>&1
            git -C "$SR" -c commit.gpgsign=false commit -qm 'content base' >/dev/null 2>&1
            SR_CONT=$(git -C "$SR" rev-parse HEAD)
            # The key of the claim as it stands at the comparand. Computed with
            # the keyer, never typed in: a hardcoded hash would pass a table
            # that no longer agrees with the keyer, which is the drift this
            # whole delegation exists to prevent.
            CK=$(bash "$SR/keyer.sh" --keys-of "$SR/pre.md" pre.md)
            CK_HASH=${CK##*$'\t'}
            # ... and the key of a claim that exists ONLY in the working tree.
            printf 'a claim this branch just wrote\n' > "$SR/.newclaim"
            CK_NEW=$(bash "$SR/keyer.sh" --keys-of "$SR/.newclaim" pre.md)
            CK_NEW_HASH=${CK_NEW##*$'\t'}
            rm -f "$SR/.newclaim"

            ct_row() { # ct_row <label> <want-rc> <baseline> <pre.md> [<guard-arg>]
                # `${5-...}` and NOT `${5:-...}`: an EMPTY fifth argument is the
                # "called with no owning guard" row, and `:-` would silently
                # substitute the keyer and turn that row green.
                local got owner="${5-keyer.sh}"
                printf '%b' "$3" > "$SR/$P"
                printf '%b' "$4" > "$SR/pre.md"
                ( BASELINE_RATCHET_BASE_REF="$SR_CONT" \
                  baseline_ratchet_check "$SR" "$P" set-aperture "$owner" ) >/dev/null 2>&1
                got=$?
                rows=$((rows + 1))
                if [ "$got" != "$2" ]; then
                    printf 'FAIL  %-46s want rc=%s got rc=%s\n' "$1" "$2" "$got"
            ...
            CT_OK="# header\npre.md\t${CK_HASH}\t1\n"
            # ADMITTED: the claim predates the comparand and the guard did NOT
            # move. This row is the reason the shape exists.
            ct_row 'content key predates, guard unchanged'  0 "$CT_OK" 'the old claim, 2.93 times faster\n'
            # ADMITTED: the same claim, pushed down the file. THE DRIFT CASE.
            ct_row 'content key survives line drift'        0 "$CT_OK" 'a new heading\nand a paragraph\nthe old claim, 2.93 times faster\n'
            # REFUSED: PERF-028's laundering shape, in the content spelling.
            ct_row 'content key this branch WROTE refuses'  1 "# header\npre.md\t${CK_NEW_HASH}\t1\n" 'a claim this branch just wrote\n'
            # REFUSED: one copy at the comparand, two recorded. Without the
            # count a duplicate rides in free on the original's key.
            ct_row 'content count above comparand refuses'  1 "# header\npre.md\t${CK_HASH}\t2\n" 'the old claim, 2.93 times faster\nthe old claim, 2.93 times faster\n'
            # ADMITTED once the comparand really does carry both copies is NOT
            # assertable here (the comparand is fixed), so the inverse is: a
            # count the comparand DOES cover stays green.
            ct_row 'content count within comparand admits'  0 "$CT_OK" 'the old claim, 2.93 times faster\nthe old claim, 2.93 times faster\n'
            # REFUSED: the file is not at the comparand at all.
            ct_row 'content file absent at comparand'       1 "# header\nfresh.md\t${CK_HASH}\t1\n" 'the old claim, 2.93 times faster\n'
            # REFUSED: malformed key and malformed count. A parse failure must
            # never read as "no growth".
            ct_row 'content non-hex key refuses'            1 "# header\npre.md\tZZZZZZZZZZZZZZZZ\t1\n" 'the old claim, 2.93 times faster\n'
            ct_row 'content short key refuses'              1 "# header\npre.md\tdeadbeef\t1\n" 'the old claim, 2.93 times faster\n'
            ct_row 'content zero count refuses'             1 "# header\npre.md\t${CK_HASH}\t0\n" 'the old claim, 2.93 times faster\n'
            ct_row 'content missing count field refuses'    1 "# header\npre.md\t${CK_HASH}\n" 'the old claim, 2.93 times faster\n'
            # REFUSED: an owning guard that cannot key. `guard.sh` is the plain
            # text fixture, so it has no --keys-of, and "could not check" must
            # be a red rather than an admission.
            ct_row 'content guard cannot key refuses'       1 "$CT_OK" 'the old claim, 2.93 times faster\n' 'guard.sh'
            ct_row 'content with NO owning guard refuses'   1 "$CT_OK" 'the old claim, 2.93 times faster\n' ''
            # And the admission must NAME the entry, exactly as the coordinate
            # shape must. A silent admission is this file's defect one level up.
            printf '%b' "$CT_OK" > "$SR/$P"
            printf 'the old claim, 2.93 times faster\n' > "$SR/pre.md"
            ct_out=$( BASELINE_RATCHET_BASE_REF="$SR_CONT" \
                      baseline_ratchet_check "$SR" "$P" set-aperture keyer.sh 2>&1 )
            rows=$((rows + 1))
            case "$ct_out" in
                *"APERTURE REVEAL"*"$CK_HASH"*) : ;;
                *)  printf 'FAIL  a content-keyed admission did not NAME the entry.\n'
                    bad=1 ;;
            esac
