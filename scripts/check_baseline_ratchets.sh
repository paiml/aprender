...
classify() { # classify <basename> -> "<kind>[<TAB>reason]", rc 1 if unclassified
    case "$1" in
        ...
        comparator_pin_citations.txt)            printf 'set\n' ;;   # files citing a SUPERSEDED llama.cpp pin as dated history / fixture, one line each (scripts/check_comparator_pin_citations.sh, #3741)   # known llama.cpp consumers not resolving through llama_bin.sh, each owned by an issue (scripts/check_comparator_consumers_resolve.sh, #3740)
