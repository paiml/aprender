...
# C1 C2 C3 C10 C12 moved to 0.67 with their tracks (SPEC-2.0); they are listed as 0.67 and never credited here.
set -uo pipefail
...
cmd_of() {
    case "$1" in
        ...
        C5)  echo 'bash scripts/train_parity.sh --validate evidence/train-parity   # (0.67 lane for the numbers; 0.66 credits the harness receipts schema-valid) ' ;;
        ...
        C1|C2|C3|C10|C12) echo '0.67 (SPEC-2.0: moved with its track; never credited in 0.66)' ;;
        *) return 1 ;;
    esac
}
CREDITED="C0 C4 C5 C6 C7 C8 C9 C11 C13 C14"
...
case "${1:-}" in
    ...
    --all) rc=0; for c in $CREDITED; do run_one "$c" || rc=1; done; [ "$rc" = 0 ] && printf 'ALL CREDITED (C0 C4 C5 C6 C7 C8 C9 C11 C13 C14)\n'; exit "$rc" ;;
    --self-test)
        n=0; red=0
        ...
        t 0 "--list names exactly ten credited commands and five 0.67"  bash -c "[ \$(bash '$0' --list | grep -c '^C[0-9]* *bash ') -eq 10 ] && [ \$(bash '$0' --list | grep -c '0.67 (SPEC-2.0') -eq 5 ]"
