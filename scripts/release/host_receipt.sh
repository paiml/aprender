...
if nv_failed:
    unmeasured.append(f"accelerator: nvidia-smi is present but failed (rc {nv_failed[0]}): {nv_failed[1]}")
if nv:
...
r = json.load(open(p)); r[name + "_attempt"] = {"status": "refused", "rc": int(rc), "reason": " | ".join(tail)}
json.dump(r, open(p, "w"), indent=2); open(p, "a").write("\n")
PY
}
...
if int(rc) != 0 or not isinstance(out, dict):
    ...
    r.setdefault("unmeasured", []).append(f"generate: `apr run --format json` {why}: {(err[-1] if err else '')}")
else:
    ...
    if fallback:
        r.setdefault("unmeasured", []).append(f"generate: apr's accelerated path was not used: {fallback}")
json.dump(r, open(p, "w"), indent=2); open(p, "a").write("\n")
PY
else
