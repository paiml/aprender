...
missing_receipts() {  # $1 = 8-char SHA; asset names on stdin; prints the missing kinds
    ...
    names=$(cat)
    grep -Eq "^clean-room-.*${sha}" <<<"$names" || missing+="clean-room "
    grep -Eq "^(dogfood-.*${sha}|receipt-.*${sha}.*[.]json)" <<<"$names" || missing+="dogfood "
    grep -Eq "^fresh-container-.*${sha}" <<<"$names" || missing+="fresh-container "
    printf '%s' "$missing"
}
