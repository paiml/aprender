...
if [ "$SELF_TEST" = 1 ]; then
  ...
  if [ -z "$ONLY_CASE" ]; then
    ...
    rmutant wa-near-tie       red-model-wrong-answer-not-near-tie 's/^                elif ma > NEAR_TIE or mo > NEAR_TIE:/                elif False:/'
    rmutant wa-no-logits      red-model-wrong-answer-no-logits    's/^                if ma is None or mo is None:/                if False:/'
    ...
    rmutant wa-official       red-model-wrong-answer-not-official 's/^            elif a.get("prompt_ids") != off.get("template_prompt_ids"):/            elif False:/'
