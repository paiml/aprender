...
PY
  ...
  row=$(python3 - "$rid" "$qa_row" "$be_json" "$qa_rc" "$rreq" "$got" "$rfile" "$rinv" "$fit_json" <<'PY'
import json, sys
...
print(json.dumps({"id": rid, "file": rfile, "inventory_only": inv_only, "present": True, "sha_ok": True,
                  ...
                  "golden_output": qa.get("golden_output"), "backends": be, "fit": fit, "green": green}))
PY
)
  printf '%s\n' "$row" >> "$ROWS"
  EXECUTED=$((EXECUTED + 1))
