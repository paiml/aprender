...
# Stamped like apr_locked (#4051): the helper writes the moment it held the lock to GPU_ACQ_STAMP.
apr_exclusive() {
  local t0 t1 acq rc verb="$1"
  # bashrs DET002: #4051 timing stamp: wall-clock time IS the measured quantity, not a build input.
  # bashrs disable-next-line=DET002
  t0=$(date +%s.%N); acq=$(mktemp 2> /dev/null || echo /dev/null)
  GPU_ACQ_STAMP="$acq" GPU_OWNED_PREFIX="$(readlink -f "$APR")" GPU_LOCK="$GPU_LOCK" GPU_WAIT_SECS="$LOCK_WAIT" \
    choom -n 1000 -- bash scripts/lib/gpu_exclusive_run.sh "$APR" "$@"; rc=$?
  # bashrs DET002: #4051 timing stamp: wall-clock time IS the measured quantity, not a build input.
  # bashrs disable-next-line=DET002
  t1=$(date +%s.%N)
  stamp_call gpu "$t0" "$(cat "$acq" 2> /dev/null)" "$t1" "$rc" "$verb"
...
try: print(json.load(open(sys.argv[1])).get("architecture") or "unknown")
  ...
  for b in "${bes[@]}"; do
    ...
    if [ "$flag" = --gpu ] && [ -n "$GPU_NAME" ]; then
      # #3964: the GPU leg proves it had the card to itself. CONTENDED (a foreign process shared
      # the card, or the card/lock never cleared) declines as ENV. UNVERIFIED (apr never appeared
      # on the card) is NOT declined: a GPU leg that ran on the CPU is the fallback defect this
      # ladder exists to catch, and rc=75 already records it as ran=false.
      # shellcheck disable=SC2086
      ...
      if grep -q 'gpu_exclusive_run: CONTENDED' "$run_e"; then
        echo "decline: ENV the GPU was not exclusive for apr run $rid ($b) -- $(grep -A3 'gpu_exclusive_run: CONTENDED' "$run_e" | tr '\n' ' ' | cut -c1-300). Not a model verdict." >&2
        exit 2
      fi
    else
      # shellcheck disable=SC2086
