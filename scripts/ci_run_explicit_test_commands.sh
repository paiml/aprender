...
run() {
    ...
    if [ -n "$wfile" ] && [ "$workers" -eq 1 ]; then
        printf 'REFUSE --weights without --workers > 1: weights only shape pool buckets, never a host shard\n' >&2; return 2
    fi
    out=$(parse "$dir") || return $?
    mapfile -t cmds <<< "$out"
    total=${#cmds[@]}
    ...
    if [ "$m" -gt 1 ]; then
        ...
        for cmd in "${cmds[@]}"; do
            k=$((k + 1))
            if [ "$(( (k - 1) % m + 1 ))" -eq "$n" ]; then mine+=("$cmd"); fi
        done
        if [ "${#mine[@]}" -eq 0 ]; then
