...
GH="${NIGHTLY_GH:-gh}"
# bashrs SEC010: $ROOT is the operator's --root, the nightly store.
...
if [ -f "$DIR/verdict.json" ]; then
  ...
  # bashrs DET002,SEC010: a RED verdict is set aside under a unique epoch name inside $DIR, the certify dir.
  # bashrs disable-next-line=DET002,SEC010
...
# bashrs SEC010: $DIR is under the operator's --root.
# bashrs disable-next-line=SEC010
...
# bashrs SEC010: $SRC is the checkout being certified; cd only.
# bashrs disable-next-line=SEC010
...
if [ -n "$CERT" ] && [ -f "$CERT" ]; then
  # bashrs SEC010: copies into $DIR, under the operator's --root.
  # bashrs disable-next-line=SEC010
...
PY
# bashrs SEC010: atomic place of the verdict inside $DIR.
# bashrs disable-next-line=SEC010
