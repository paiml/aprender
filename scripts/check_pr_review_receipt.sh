...
pc_cache_key() {
  ...
  # THE TOOLCHAIN IS PART OF THE KEY. A cross-vendor review of this cache named the
  # failure the first three inputs cannot see: the controls do not exercise the guard's
  # BYTES, they exercise the guard running THROUGH jq, check-jsonschema, minisign and
  # git. If check-jsonschema starts returning 0 for an invalid document -- a bad
  # upgrade, a broken venv, a shim earlier on PATH -- control 1 stops firing while the
  # guard's own bytes are unchanged, the key is unchanged, and the cache serves a
  # transcript proving a property that is no longer true. That is the disarmed-guard
  # class this repository has hit repeatedly, reached through a cache instead of an edit.
  #
  # So each tool contributes its RESOLVED PATH, the sha256 OF THAT FILE, and its
  # --version. The path alone would miss an in-place upgrade; the version alone would
  # miss a rebuilt interpreter; the content hash alone would miss a shim shadowing it
  # from elsewhere on PATH. A missing tool returns 1 -- no key, no cache, controls run.
  local tf t tp
  tf=""
  for t in jq check-jsonschema minisign git sha256sum; do
    tp=$(command -v -- "$t" 2>/dev/null) || return 1
    [ -n "$tp" ] || return 1
    tf="$tf$t $tp $(sha256sum -- "$tp" 2>/dev/null | cut -d' ' -f1) $("$t" --version 2>&1 | head -1)
"
  done
  tf=$(printf '%s' "$tf" | sha256sum | cut -d' ' -f1)

  printf '%s-%s-%s-%s\n' "${gs:0:24}" "${ss:0:24}" "${sd:0:24}" "${tf:0:24}"
}

# A cached transcript is accepted only if it carries this terminator over its own bytes.
# `[ -s ]` was the whole check and it is true of a ONE-BYTE FRAGMENT: a job killed
# mid-write (this queue evicts jobs; it has happened) left a truncated file that every
# later run treated as a hit, turning a partial write into a permanent free pass. The
# writer is now atomic (tmp + mv within the same directory) AND stamps the terminator,
# so a torn or truncated entry fails the hash and is a MISS. Whitelist, not blacklist:
# the entry must PROVE it is complete, rather than merely fail to look broken.
PC_CACHE_MARK='__PC_CACHE_END__'
pc_cache_body() {   # <file> -> body on stdout, rc 0 iff the terminator verifies
  local f=$1 last body
  last=$(tail -n 1 -- "$f" 2>/dev/null) || return 1
  case "$last" in "$PC_CACHE_MARK "*) ;; *) return 1 ;; esac
  body=$(sed '$d' -- "$f" 2>/dev/null) || return 1
  [ "$(printf '%s\n' "$body" | sha256sum | cut -d' ' -f1)" = "${last#"$PC_CACHE_MARK" }" ] || return 1
  printf '%s\n' "$body"
}
  ...
  if [ -n "$_k" ]; then
    ...
    if [ -d "$PC_CACHE_DIR" ]; then
      _t=$(mktemp "$PC_CACHE_DIR/.wr.XXXXXX" 2>/dev/null) && {
        { printf '%s\n' "$PC_TRANSCRIPT"
          printf '%s %s\n' "$PC_CACHE_MARK" \
            "$(printf '%s\n' "$PC_TRANSCRIPT" | sha256sum | cut -d' ' -f1)"
        } > "$_t" 2>/dev/null && mv -f -- "$_t" "$PC_CACHE_DIR/$_k" 2>/dev/null
