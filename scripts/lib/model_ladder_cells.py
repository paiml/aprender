...
def _rungs_floor(rungs_doc, rungs_main, out):
    """The context-rungs file at origin/main is a floor, like the ladder itself."""
    rc = 0
    if rungs_main is None:
        return rc
...
def _item_modes(item, fails):
    """The thinking modes an inventory item owes; problems with its declaration go to `fails`."""
    ...
    return want if want is not None else modes  # the evidence decides what is owed, not the producer's own claim


def _pass_problems(c, mode, tok, fit, need, total):
    bad = []
...
def _refusal_problem(c, label, fit, need, total):
    req = c.get("required_bytes")
    if fit and need is not None:
        return f"{label} REFUSED but it fits: {need} B <= {total} B total -- a co-tenant or a wrong arithmetic, not the host's capacity"
    if total is None or req is None or int(req) <= int(total):
        return f"{label} REFUSED without an honest arithmetic (required {req} vs total {total})"
    return None


def _judge_cell_rows(got, label, mode, tok, fit, need, total):
    """(outcome, failure) for one owed cell: outcome is 'pass', 'refused' or 'fail'."""
    if not got:
        return "fail", f"{label} MISSING"
    if len(got) > 1:
        return "fail", f"{label} has {len(got)} rows -- ambiguous"
    c = got[0]
    v = c.get("verdict")
    if v == "pass":
        bad = _pass_problems(c, mode, tok, fit, need, total)
        return ("fail", f"{label} 'pass' is not a pass: " + ", ".join(bad)) if bad else ("pass", None)
    if v == "refused":
        why = _refusal_problem(c, label, fit, need, total)
        return ("fail", why) if why else ("refused", None)
    return "fail", f"{label} {v}: {_loud(c.get('reason', ''), 80)}"


def _owed_rungs(item, ctx, rungs, long_ids, long_for, consumer_max, fails):
    """Yield (rung id, token count) for every rung this item owes; unmeasurable rungs go to `fails`.

    A generator on purpose: consumed lazily, a rung's own failure lands in `fails` in rung order,
    interleaved with the cell failures of the rungs before it, as the report has always read."""
    lo, _why_long = owes_long(item, long_for)
    for rung in rungs:
        rid = rung["id"]
...
def _judge_item(item, rows, total, J, tally):
    """Every failure for one inventory item on one host; updates the cross-host maps in J and `tally`."""
    ...
    modes = _item_modes(item, fails)
    for rid, tok in _owed_rungs(item, ctx, J["rungs"], J["long_ids"], J["long_for"], J["consumer_max"], fails):
        fit, need = fits(item, total, tok)
        for verb in J["verbs"]:
            for mode in sorted(modes):
                tally["owed"] += 1
                key = (sha, verb, mode, rid)
                J["passed_anywhere"].setdefault(key, False)
                outcome, why = _judge_cell_rows(rows.get((f, verb, mode, rid), []), f"{verb}/{mode}/{rid}",
                                                mode, tok, fit, need, total)
                if outcome == "pass":
                    tally["passed"] += 1; J["passed_anywhere"][key] = True
                elif outcome == "refused":
                    tally["refused"] += 1
                else:
                    fails.append(why); J["failed_somewhere"].add(key)
    return fails


def _judge_host(hid, R, rc, J, out):
    ...
    for item in R.get("inventory") or []:
        fails = _judge_item(item, rows, total, J, tally)
        f = item.get("file")
...
def _long_rung_holes(receipts, long_for, out):
...
def _passes_nowhere(passed_anywhere, failed_somewhere, out):
    rc = 0
...
def judge(L, receipts, rungs_doc, out, rungs_main=None):
    ...
    for hid, R in receipts.items():
        rc = _judge_host(hid, R, rc, J, out)
    rc = max(rc, _long_rung_holes(receipts, long_for, out))
    rc = max(rc, _passes_nowhere(J["passed_anywhere"], J["failed_somewhere"], out))
    return rc
