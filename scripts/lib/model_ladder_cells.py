...
def _rungs_floor(rungs_main, rungs_doc, out):
...
def _pass_defects(c, mode, tok, fit, need, total):
    bad = []
...
def _judge_row(c, label, mode, tok, fit, need, total):
    """One measured cell row -> (failure line or None, 'pass' | 'refused' | None)."""
    v = c.get("verdict")
    if v == "pass":
        bad = _pass_defects(c, mode, tok, fit, need, total)
        if bad:
            return f"{label} 'pass' is not a pass: " + ", ".join(bad), None
        return None, "pass"
    if v == "refused":
        req = c.get("required_bytes")
        if fit and need is not None:
            return f"{label} REFUSED but it fits: {need} B <= {total} B total -- a co-tenant or a wrong arithmetic, not the host's capacity", None
        if total is None or req is None or int(req) <= int(total):
            return f"{label} REFUSED without an honest arithmetic (required {req} vs total {total})", None
        return None, "refused"
    return f"{label} {v}: {_loud(c.get('reason', ''), 80)}", None


def _judge_item(item, rows, total, S, tally):
    """Every cell one inventoried model owes on one host; returns its failure lines."""
    f, sha = item.get("file"), item.get("sha256")
    fails = []
    failed_somewhere = S["failed_somewhere"]
    ...
    modes = _owed_modes(item, fails)
    lo, why_long = owes_long(item, S["long_for"])
    for rung in S["rungs"]:
        rid = rung["id"]
        if rid in S["long_ids"] and not lo:
            continue
        tok = _tokens(rung, ctx, S["consumer_max"])
        if tok is None:
        ...
        for verb in S["verbs"]:
            for mode in sorted(modes):
                ...
                S["passed_anywhere"].setdefault(key, False)
...
def judge(L, receipts, rungs_doc, out, rungs_main=None):
    ...
    if not verbs:
        ...
        return 1
    S = {
        "rungs": rungs, "consumer_max": consumer_max, "verbs": verbs, "long_for": long_for,
