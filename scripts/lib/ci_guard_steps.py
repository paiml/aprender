...
EXPR = re.compile(r"\$\{\{.*?\}\}")
DEFAULT_JOBS = ("guard-cargo", "guard-tree")
...
def cmd_run(args):
    ...
    for job in args.jobs:
        ...
        for i, s in guard_steps(steps):
            ...
            if reason:
                rows.append((job, i, "SKIP", 0.0, f"{name}  [{reason}]"))
                continue
            senv = dict(env)
            ...
            with open(log, "w", encoding="utf-8") as out:
                rc = subprocess.run(["bash", "--noprofile", "--norc", "-eo", "pipefail", "-c", s["run"]],
                                    cwd=root, env=senv, stdout=out, stderr=subprocess.STDOUT,
                                    stdin=subprocess.DEVNULL, check=False).returncode
            status = "PASS" if rc == 0 else "FAIL"
            failed += rc != 0
            rows.append((job, i, status, time.monotonic() - t0,
                         name if rc == 0 else f"{name}  [rc={rc}, log {log}]"))
            print(f"{status} {job}#{i} {name}", file=sys.stderr, flush=True)
    print(f"{'job':12s} {'step':>4s} {'result':6s} {'sec':>6s}  name")
    for job, i, st, sec, name in rows:
        print(f"{job:12s} {i:4d} {st:6s} {sec:6.1f}  {name}")
...
def main(argv=None):
    ...
    for name in ("list", "check-run-all", "run"):
        sp = sub.add_parser(name)
        sp.add_argument("jobs", nargs="*", default=list(DEFAULT_JOBS))
    ...
    if not args.jobs:
        args.jobs = list(DEFAULT_JOBS)
    return {"list": cmd_list, "check-run-all": cmd_check, "run": cmd_run}[args.cmd](args)
