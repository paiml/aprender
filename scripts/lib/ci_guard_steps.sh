...
#
# ci/sections.yml's jobs: block is read by yaml_json below: an awk reader for the block-YAML subset
# the job YAML is written in (mappings, `- ` sequences, plain / 'single' / "double"
...
# that guessed would be a guard that checks something other than CI.
#
...
# ── shared reads ────────────────────────────────────────────────────────────
# The guard jobs are sections of car's fat-job CI: their definitions live in
# ci/sections.yml (under `jobs:`, verbatim job YAML) and scripts/ci/fat_driver.py
# runs them inside the fat jobs. The `gate` job that reads their results is in
# .github/workflows/ci.yml.
WORKFLOW="ci/sections.yml"
GATE_WORKFLOW=".github/workflows/ci.yml"
JOBS=""   # a file holding the jobs map as JSON

# jobs_block <file> -- the top-level `jobs:` mapping only. sections.yml's other
# top-level keys (sovereign-ci, matrix-pins) are fat_driver.py's, carry flow
# maps outside the reader's subset, and hold no job; they are not read, so the
# reader still refuses anything it cannot read INSIDE the jobs.
jobs_block() {
    awk '/^[^ \t#]/ { keep = ($0 ~ /^jobs:[ \t]*$/) } keep' "$1"
}

load_jobs() {
    local tmp err blk
    tmp="$(mktemp)" && err="$(mktemp)" && blk="$(mktemp)" || die "mktemp failed"
    CLEAN+=("$tmp" "$err" "$blk")
    [ -r "$WORKFLOW" ] || die "cannot read $WORKFLOW: no such file"
    jobs_block "$WORKFLOW" > "$blk" || die "cannot read $WORKFLOW"
    [ -s "$blk" ] || die "cannot read $WORKFLOW: no top-level jobs: mapping"
    if ! yaml_json "$blk" 2> "$err" | jq -c '.jobs // {} | if type == "object" then . else error("jobs: is not a mapping") end' > "$tmp" 2>> "$err"; then
        die "cannot read $WORKFLOW (jobs: block): $(tr '\n' ' ' < "$err")"
    fi
    JOBS="$tmp"
}
...
guard_jobs() {
    ...
    [ -n "$out" ] || die "no guard-* job in $WORKFLOW is read by the gate in $GATE_WORKFLOW"
    printf '%s\n' "$out"
}
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        --gate-workflow) [ "$#" -ge 2 ] || die "--gate-workflow needs a value"; GATE_WORKFLOW=$2; shift 2 ;;
        *) break ;;
    esac
done
cmd="${1:-}"
