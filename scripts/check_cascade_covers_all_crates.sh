...
#
#   R6 THE FILE   scripts/release/publish-order.txt is the ROOT workspace's part of the derived
#                 order, GENERATED (`cascade_universe.py --order --names --root-only`), and must
#                 equal it line for line: as a SET and as an ORDER. It is kept, not deleted, because
#                 two readers hold it: publish_strict.sh walks it at T-4, and infra#898's release
#                 sensor reads it AT THE TAG as the set the cascade published (infra-3c, 2026-09-21,
#                 option A). Every line must also be a name that sensor accepts -- [a-z0-9_-]+, no
#                 blank line, no comment -- or it refuses the release by name and posts nothing.
#
...
# R6: the committed file against the derived root order. A separate function because its inputs
# are two files, not the sequence the cascade walks.
run_file_rule() {  # committed_file derived_root_order_file
    local file="$1" derived="$2" bad
    if [ ! -s "$derived" ]; then
        printf 'FAIL  R6 the derived root order is empty -- the DERIVATION is broken\n'; return 1
    fi
    if [ ! -f "$file" ]; then
        printf 'FAIL  R6 %s is missing: publish_strict.sh and the release sensor (infra#898) read it at the tag\n' "${file##*/}"; return 1
    fi
    bad=$(awk '!/^[a-z0-9_-]+$/ { printf "      line %d: [%s]\n", NR, $0 }' "$file")
    if [ -n "$bad" ]; then
        printf 'FAIL  R6 publish-order.txt has line(s) the release sensor refuses (one crates.io name per line,\n'
        printf '      [a-z0-9_-]+, no blank line, no comment):\n%s\n' "$bad"; return 1
    fi
    if ! cmp -s "$file" "$derived"; then
        printf 'FAIL  R6 publish-order.txt differs from the derived root order -- regenerate it: make publish-order\n'
        printf '      (python3 scripts/lib/cascade_universe.py --order --names --root-only . > scripts/release/publish-order.txt)\n'
        diff "$file" "$derived" | sed -n '1,12s/^/      /p'
        return 1
    fi
    printf 'ok    R6 publish-order.txt IS the derived root order (%s crates, same set, same order)\n' "$(grep -c . "$file")"
}
...
if [ "${1:-}" = "--self-test" ]; then
    ...
    # R6: the committed file. Row 12a is the control.
    printf '%s\n' base mid top > "$TD/derived"
    file_row() {  # name expect_rc needle content...
        local name="$1" want="$2" needle="$3" out rc; shift 3
        printf '%s\n' "$@" > "$TD/pofile"
        out="$(run_file_rule "$TD/pofile" "$TD/derived" 2>&1)"; rc=$?
        if [ "$rc" != "$want" ] || ! grep -qF -- "$needle" <<< "$out"; then
            printf 'FAIL  %s: exit %s (want %s): %s\n' "$name" "$rc" "$want" "$(tr '\n' '|' <<< "$out")"; fails=1; return 1
        fi
        printf 'ok    %s\n' "$name"
    }
    file_row 'row 12a the file equal to the derivation passes' 0 'IS the derived root order' base mid top
    file_row 'row 12b the same set in another ORDER is REJECTED' 1 'differs from the derived root order' base top mid
    file_row 'row 12c a crate missing is REJECTED' 1 'differs from the derived root order' base mid
    file_row 'row 12d an extra crate (a facade, or publish = false) is REJECTED' 1 'differs from the derived root order' base mid top face
    file_row 'row 12e a blank line is REJECTED (the sensor refuses it by name)' 1 'the release sensor refuses' base '' mid top
    file_row 'row 12f a comment line is REJECTED' 1 'line 1: [# generated]' '# generated' base mid top
    file_row 'row 12g a name the sensor refuses (uppercase) is REJECTED' 1 'line 2: [Mid]' base Mid top
    # the MUTANT: the order comparison weakened to a set comparison -- row 12b must turn it RED
    declare -f run_file_rule | sed 's/if ! cmp -s "\$file" "\$derived"; then/if ! cmp -s <(sort "$file") <(sort "$derived"); then/' > "$TD/r6_mut.sh"
    if ! grep -q 'sort "\$file"' "$TD/r6_mut.sh"; then
        printf 'FAIL  row 13 the R6 order-comparison anchor is gone -- the mutant is identical\n'; fails=1
    elif ( . "$TD/r6_mut.sh"; file_row 'mutant' 1 'differs from the derived root order' base top mid > /dev/null ); then
        printf 'FAIL  row 13 the set-only mutant still REJECTED a reordered file -- row 12b does not discriminate\n'; fails=1
    else
        printf 'ok    row 13 mutant: a set-only comparison ACCEPTS a reordered file (row 12b RED)\n'
    fi
...
if ! python3 "$UNIVERSE_PY" --order --names --root-only "$REPO_ROOT" > "$W/root_order"; then
    printf 'FAIL  the derived root order could not be read; nothing was checked.\n'; exit 1
fi
rules_rc=0
run_rules "$W/uni" "$W/edges" "$W/seq" "$REPO_ROOT" || rules_rc=1
run_file_rule "${REPO_ROOT}/scripts/release/publish-order.txt" "$W/root_order" || rules_rc=1
if [ "$rules_rc" -eq 0 ]; then
    printf '\nPASS  the cascade walks every publishable crate, in dependency order, and publish-order.txt is that order\n'
    exit 0
fi
printf '\nFAIL  see rows above\n'
exit 1
