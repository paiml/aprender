...
#
#   bash scripts/mutants_diff_gate.sh <diff> [--cap N] [--jobs J] [--max-missed M] [--out DIR]
...
gate() { # gate <diff> <cap> <jobs> <max-missed> <out>
  local diff=$1 cap=$2 jobs=$3 max=$4 out=$5 cargo=${MUTANTS_GATE_CARGO:-cargo} n rc oc
  [ -s "$diff" ] || { echo "ok    empty diff: nothing to mutate"; return 0; }
  mkdir -p "$out" || return 1
  "$cargo" mutants --workspace --in-diff "$diff" --list > "$out/list.txt" 2> "$out/list.err"; rc=$?
  if [ "$rc" -ne 0 ]; then
  ...
  "$cargo" mutants --workspace --no-times --timeout 300 -j "$jobs" --in-diff "$diff" --output "$out" -- --lib; rc=$?
...
STUB
  ...
  row() { # row <name> <want rc> <needle> [ENV=VAL...] -- runs THIS script's gate against the stub
    ...
    out=$(env MUTANTS_GATE_CARGO="$T/cargo" STUB_ARGS="$T/args-$name" "$@" \
          bash "$GATE_SCRIPT" "${GATE_DIFF:-$T/pr.diff}" --cap 3 --jobs 2 --max-missed "${MAXM:-0}" --out "$T/out-$name" 2>&1); rc=$?
...
no-workspace-on-list~workspace-on-list-and-run~"$cargo" mutants --workspace --in-diff "$diff" --list~"$cargo" mutants --in-diff "$diff" --list
no-workspace-on-run~workspace-on-list-and-run~"$cargo" mutants --workspace --no-times~"$cargo" mutants --no-times
