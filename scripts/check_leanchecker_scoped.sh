...
# overloaded". `pv discharge check --leanchecker` / `pv discharge run` now cap it by default (<= 8 threads inside
# `systemd-run --user --scope --slice=agent.slice -p MemoryMax=24G -p CPUQuota=800%`). This guard keeps every
# OTHER door shut: a tracked script, Makefile or workflow line that runs leanchecker directly is RED unless the
# same line carries `systemd-run` and `MemoryMax=`, and `--leanchecker-unscoped` (pv's opt-out) is RED anywhere.
#
...
#
# Surfaces: tracked .github/workflows/*.yml|*.yaml, Makefile, *.mk, and *.sh anywhere. Comment lines are skipped.
...
scan() {
    ...
    done < <(git -C "$ROOT" ls-files -- '.github/workflows/*.yml' '.github/workflows/*.yaml' 'Makefile' '*.mk' '*.sh')
