...
if [ "${1:-}" = "--self-test" ]; then
  ...
  # Rows 28-32 (#4048): a module FILE declared out of line under a test-only cfg is test
  ...
  # 28: `#[cfg(test)] mod guard;` -> SKIPPED, named on stderr, not reported.
  ...
  # SIGPIPEs the writer and a TRUE match reads as a failure (it did, on row 31).
  e="$(escerr "$TD/o")"
  if [ -z "$(esc "$TD/o")" ] && grep -q 'SKIPPED (cfg(test)-only module.*src/guard.rs' <<< "$e"; then
    printf 'ok    row 28 a #[cfg(test)] mod file'"'"'s escaping include is skipped, by name\n'
  else
    printf 'FAIL  row 28 cfg(test)-only module file: out=[%s] err=[%s]\n' "$(esc "$TD/o")" "$(escerr "$TD/o")"; fails=1
  fi
  # 29: the same file declared as a plain `mod guard;` IS host-compiled -> RED.
  mkdir -p "$TD/p/src"; printf 'mod guard;\n' > "$TD/p/src/lib.rs"; printf '%b' "$guard" > "$TD/p/src/guard.rs"
  if [ "$(esc "$TD/p" | grep -c .)" = "1" ]; then
    printf 'ok    row 29 the same include in a non-test module is reported\n'
  else
    printf 'FAIL  row 29 a non-test module'"'"'s escape was not reported\n'; fails=1
  fi
  # 30: a module whose test cfg covers only SOME items -> the escaping item is judged.
  mkdir -p "$TD/q/src"; printf 'mod part;\n' > "$TD/q/src/lib.rs"
  printf '#[cfg(test)]\nfn t() {}\npub const X: &str = include_str!("../../../../gone.yaml");\n' > "$TD/q/src/part.rs"
  if [ "$(esc "$TD/q" | grep -c .)" = "1" ]; then
    printf 'ok    row 30 cfg(test) on some items does not make the file test-only\n'
  else
    printf 'FAIL  row 30 a partly-test module hid its production escape\n'; fails=1
  fi
  # 31: a module declared BY a test-only module file is test-only too (transitive).
  ...
  if [ -z "$(esc "$TD/r")" ] && grep -q 'src/guard/deep.rs' <<< "$e"; then
    printf 'ok    row 31 a submodule of a test-only module is skipped too\n'
  else
    printf 'FAIL  row 31 transitive test-only module: out=[%s]\n' "$(esc "$TD/r")"; fails=1
  fi
  # 32: `#[cfg(any(test, ...))] mod x;` is compiled outside tests -> RED.
