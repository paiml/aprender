...
fn main() {
    ...
    let d_dev=DeviceBuffer::from_host(&stream,&data).expect("upload quantized data to device");
    ...
    let y_slice_dev=DeviceBuffer::<f32>::zeroed(&stream,m).expect("allocate zeroed y buffer for slice kernel");
    module.q4k_matvec_atomic(&stream,LaunchConfig::for_num_elems(total),&d_dev,&x_dev,&y_slice_dev).expect("launch slice");
    let y_slice=y_slice_dev.to_host_vec(&stream).expect("copy slice-kernel y buffer to host");

    // ---- Run NEW raw-ptr kernel ----
    let y_raw_dev=DeviceBuffer::<f32>::zeroed(&stream,m).expect("allocate zeroed y buffer for raw-ptr kernel");
    module.q4k_matvec(
        &stream,
    ...
    let y_raw=y_raw_dev.to_host_vec(&stream).expect("copy raw-ptr-kernel y buffer to host");
    ...
    for _ in 0..20 {
        let yz=DeviceBuffer::<f32>::zeroed(&stream,m).expect("allocate zeroed warmup y buffer");
        ...
        let _=yz.to_host_vec(&stream).expect("copy warmup y buffer to host");
    }
    ...
    for _ in 0..reps {
        ...
        let _=y_raw_dev.to_host_vec(&stream).expect("copy timing y buffer to host");
        times.push(t0.elapsed().as_secs_f64()*1e6/(iters as f64));
    }
    times.sort_by(|a,b|a.partial_cmp(b).expect("timing samples must be comparable (no NaN)"));
