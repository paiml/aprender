#!/usr/bin/env bash
# #3522 OXIDE-001 port: run the causal conv1d + SiLU port on THIS host and write
# evidence/kernels/gdn_causal_conv1d_silu/<host>.json (schema apr-kernel-receipt/v1).
#
...
host="$(hostname -s)"
out_dir="$repo/evidence/kernels/gdn_causal_conv1d_silu"
log="$(mktemp)"
trap 'rm -f "$log"' EXIT
...
print(json.dumps({"schema": "apr-kernel-receipt/v1", "kernel": "gdn_causal_conv1d_silu",
