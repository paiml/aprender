#!/usr/bin/env bash
# #3522 OXIDE-001 port: run the gated delta-rule chunk-scan port on THIS host and write
# evidence/kernels/gdn_delta_rule_scan/<host>.json (schema apr-kernel-receipt/v1).
#
...
host="$(hostname -s)"
out_dir="$repo/evidence/kernels/gdn_delta_rule_scan"
log="$(mktemp)"
trap 'rm -f "$log"' EXIT
...
print(json.dumps({"schema": "apr-kernel-receipt/v1", "kernel": "gdn_delta_rule_scan",
