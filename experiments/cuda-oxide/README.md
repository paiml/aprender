...
| `gated-rmsnorm/` | `gated_rmsnorm_exp` (A, libdevice exp silu) + `gated_rmsnorm_ex2` (B, ex2 mirror) — GDN per-head RMSNorm × silu(gate), 1 warp/head, **safe Rust** (`DisjointSlice<LinearTiles<4>>`, no `unsafe`) | **GO on all three CUDA hosts** (lambda 4090 0.91×, yoga 4060 0.94–0.95×, gx10 GB10 1.00×) vs hand-PTX `GatedRmsNormKernel`, parity cos=1.0000000 / max\|Δ\| 1.4e-6 vs f64. First KERNEL-001 (#3522 O-1) receipts: `evidence/kernels/gdn_gated_rmsnorm/`. See `gated-rmsnorm/RESULTS.md`. |
