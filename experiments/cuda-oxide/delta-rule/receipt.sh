...
# evidence/kernels/gdn_delta_rule/<host>.json (schema apr-kernel-receipt/v1).
#
...
host="$(hostname -s)"
out_dir="$repo/evidence/kernels/gdn_delta_rule"
log="$(mktemp)"
trap 'rm -f "$log"' EXIT
...
print(json.dumps({"schema": "apr-kernel-receipt/v1", "kernel": "gdn_delta_rule",
