#!/usr/bin/env bash
# #3522 OXIDE-001 port: run the causal GQA prefill flash-attention port on THIS host and write
# evidence/kernels/gdn_prefill_flash_attention_256/<host>.json (schema apr-kernel-receipt/v1).
#
...
#   PATH=/usr/local/cuda-13.3/bin:$PATH flock /tmp/apr-gpu.priority flock /tmp/apr-gpu.lock ./receipt.sh
set -euo pipefail

here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
repo="$(git -C "$here" rev-parse --show-toplevel)"
host="$(hostname -s)"
out_dir="$repo/evidence/kernels/gdn_prefill_flash_attention_256"
log="$(mktemp)"
trap 'rm -f "$log"' EXIT
...
print(json.dumps({"schema": "apr-kernel-receipt/v1", "kernel": "gdn_prefill_flash_attention_256",
