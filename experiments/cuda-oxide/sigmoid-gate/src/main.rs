// #3522 OXIDE-001 — pure-Rust cuda-oxide port of the GDN sigmoid gate.
//
// Target = hand-PTX `SigmoidGateKernel`
// (crates/aprender-gpu/src/kernels/gdn/sigmoid_gate.rs, entry `gdn_sigmoid_gate`):
// elementwise, grid ceil(n/256), block 256, n baked into the PTX, params
// (x, gate), in place.
//
//   x[i] *= 1 / (1 + exp(-gate[i]))
//
// CPU reference = `apply_sigmoid_gate` in crates/aprender-serve/src/gguf/inference/
...
// the one write goes through `DisjointSlice<f32, LinearTiles<1>>`, one element per
// thread, minted from the launch-checked thread index.
//
// OUT OF PLACE, for the reason l2-norm gives: a safe kernel cannot read `x` as
// `&[f32]` and write the same memory through a `DisjointSlice`, so the port takes
// (x, gate, out). Both authorings move the same bytes: two loads and one store per
// element.
//
// Two variants, differing only in the exponential:
//   (A) sigmoid_gate_exp — `exp(-g)` (libdevice), the more precise
//   (B) sigmoid_gate_ex2 — `exp2(-g * log2 e)`, mirroring the hand PTX's ex2.approx
// Both divide with `/`, which is `div.rn.f32`, the hand PTX's divide.
//
...
// evidence/kernels/gdn_sigmoid_gate/<host>.json.
...
/// Timed widths, each with a committed hand-PTX baseline (n is baked into it).
/// 2048 is Qwen3.5-0.8B's full-attention output (the device test's shape).
...
mod kernels {
    ...
    pub fn sigmoid_gate_exp(x: &[f32], gate: &[f32], mut out: DisjointSlice<f32, LinearTiles<1>>) {
        let t = thread::index_1d_u32(launch_context);
        let i = t.get() as usize;
        let Some(mut run) = out.thread_run32(t) else {
            return;
        };
        for k in 0..run.len() {
            let s = 1.0f32 / (1.0f32 + (-gate[i]).exp());
            let v = x[i] * s;
    ...
    /// (B) sigmoid via `exp2(-g * log2 e)`, the hand PTX's form.
    #[kernel(launch_context = launch_context)]
    #[launch_bounds(256)]
    #[launch_contract(domain = 1, coordinates = u32, block = (256, 1, 1))]
    pub fn sigmoid_gate_ex2(x: &[f32], gate: &[f32], mut out: DisjointSlice<f32, LinearTiles<1>>) {
        let t = thread::index_1d_u32(launch_context);
        let i = t.get() as usize;
        let Some(mut run) = out.thread_run32(t) else {
            return;
        ...
        for k in 0..run.len() {
            let s = 1.0f32 / (1.0f32 + ((-gate[i]) * std::f32::consts::LOG2_E).exp2());
            let v = x[i] * s;
            if let Some(mut slot) = run.at(k) {
                slot.write(v);
            }
...
/// f64 evaluation of `apply_sigmoid_gate` (forward_qwen35.rs).
fn cpu_sigmoid_gate(x: &[f32], gate: &[f32]) -> Vec<f32> {
    x.iter()
        .zip(gate)
        .map(|(&v, &g)| (f64::from(v) / (1.0 + (-f64::from(g)).exp())) as f32)
        .collect()
}
...
/// Deterministic inputs. The gate spans the saturating tails as well as the
/// linear region, as in the aprender-gpu device test.
fn make_inputs(n: usize, seed: u64) -> (Vec<f32>, Vec<f32>) {
    ...
    let x: Vec<f32> = (0..n).map(|_| next() * 2.0).collect();
    let gate: Vec<f32> = (0..n)
        .map(|i| next() * 6.0 + (i % 3) as f32 - 1.0)
        .collect();
    (x, gate)
}
...
fn measure(got: &[f32], x: &[f32], gate: &[f32], us: f64, eager_us: f64) -> Measured {
    let want = cpu_sigmoid_gate(x, gate);
    Measured {
        cos: cosine(got, &want),
...
fn run_oxide(
    ...
    let (x, gate) = make_inputs(n, seed(n));
    ...
    let launch = |d_out: &mut DeviceBuffer<f32>| {
        if ex2 {
            let p = module.prepare_sigmoid_gate_ex2(config).expect("prepare ex2");
            module
                .sigmoid_gate_ex2(&stream, &p, &d_x, &d_gate, d_out)
                .expect("launch ex2");
        } else {
            let p = module.prepare_sigmoid_gate_exp(config).expect("prepare exp");
            module
                .sigmoid_gate_exp(&stream, &p, &d_x, &d_gate, d_out)
                .expect("launch exp");
        }
    };
    launch(&mut d_out);
    ...
    measure(&got, &x, &gate, us, eager_us)
}

/// The hand PTX for width `n`, loaded from its committed baseline and launched on
/// the same data: grid ceil(n/256), block 256, params (x, gate), in place.
fn run_handptx(ctx: &Arc<CudaContext>, sm: &str, n: usize) -> (Measured, u32) {
    let ptx_path = format!("baseline-ptx/gdn_sigmoid_gate_n{n}.{sm}.ptx");
    let ptx = std::fs::read_to_string(&ptx_path).unwrap_or_else(|e| {
        eprintln!("missing hand-PTX baseline {ptx_path}: {e}");
        eprintln!("regenerate: APR_BLESS_PTX=1 cargo test -p aprender-gpu --features cuda --lib gdn_sigmoid_gate_ptx_golden");
        std::process::exit(2);
    });
    let stream = ctx.new_stream().expect("stream");
    let (x, gate) = make_inputs(n, seed(n));
    ...
    let func = module
        .load_function("gdn_sigmoid_gate")
        .expect("gdn_sigmoid_gate");
    let regs = func.num_registers().expect("regs");
    let d_x = DeviceBuffer::from_host(&stream, &x).expect("x");
    let d_gate = DeviceBuffer::from_host(&stream, &gate).expect("gate");
    let mut ptrs = [d_x.cu_deviceptr(), d_gate.cu_deviceptr()];
    let grid = n.div_ceil(BLOCK) as u32;
    let mut launch = || {
        let mut params: Vec<*mut std::ffi::c_void> =
            ptrs.iter_mut().map(|p| (p as *mut u64).cast()).collect();
        // SAFETY: two device pointers matching the entry's two .u64 params; each
        // buffer holds n f32, and the baked-in bound stops every thread at n.
        unsafe {
    ...
    // Parity from the first launch. Timing then re-gates x in place, which does
    // the same loads, arithmetic and store on already-gated values.
    launch();
    ...
    (measure(&got, &x, &gate, us, eager_us), regs)
}
...
fn main() {
    ...
    println!("== #3522 cuda-oxide sigmoid gate ({sm}) ==");
    println!("   block={BLOCK}, 1 element/thread");
    ...
    let regs_hand;
    {
        ...
        let ok = h.ok();
        println!(
            "  hand PTX n={}: cos={:.7} maxdiff={:.3e} regs={r} {}",
            TIMED_NS[0],
            h.cos,
            h.maxdiff,
            if ok { "PASS" } else { "FAIL" }
    ...
    for ex2 in [false, true] {
        let (variant, entry) = if ex2 {
            ("ex2", "sigmoid_gate_ex2")
        } else {
            ("exp", "sigmoid_gate_exp")
        };
    ...
    println!("#3522 SIGMOID GATE DONE");
}
