#!/usr/bin/env bash
# #3522 OXIDE-001 port: run the sigmoid gate port on THIS host and write
# evidence/kernels/gdn_sigmoid_gate/<host>.json (schema apr-kernel-receipt/v1).
#
...
host="$(hostname -s)"
out_dir="$repo/evidence/kernels/gdn_sigmoid_gate"
log="$(mktemp)"
trap 'rm -f "$log"' EXIT
...
print(json.dumps({"schema": "apr-kernel-receipt/v1", "kernel": "gdn_sigmoid_gate",
