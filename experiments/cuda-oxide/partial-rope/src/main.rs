// #3522 OXIDE-001 — pure-Rust cuda-oxide port of the GDN partial NEOX RoPE.
//
// Target = hand-PTX `PartialNeoxRopeKernel`
// (crates/aprender-gpu/src/kernels/gdn/partial_rope.rs, entry `gdn_partial_neox_rope`):
// grid (heads), block half = n_rot/2, one thread per (head, pair); head_dim and half
// baked into the PTX; params (x, position, theta_scale), in place.
//
//   theta_j = pos * theta_scale^j        (j multiplications, the CPU's order)
//   (a, b) = (x[h*head_dim + j], x[h*head_dim + j + half])
//   x[h*head_dim + j]        = a*cos(theta_j) - b*sin(theta_j)
//   x[h*head_dim + j + half] = a*sin(theta_j) + b*cos(theta_j)
//
// Dimensions n_rot..head_dim of each head are NOT rotated.
//
// CPU reference = `apply_partial_neox_rope` (forward_qwen35.rs:191) with theta built
// in f32 exactly as the CPU builds it (one ulp of theta is the whole budget at
// pos 1000, see the hand kernel's module docs), then sin/cos and the rotation in f64.
//
...
// The write goes through `DisjointSlice<f32, RuntimeRowMajorTiles<2, 1>>` over the
// buffer viewed as rows of width HALF (bound on the host with `RowWidth`). A head
// is 2*RATIO such rows (RATIO = head_dim / n_rot), so tile-row r covers rows 2r and
// 2r+1: for r = h*RATIO that is exactly the pair (j, j + half) of head h. The pair
// is the thread's own tile, disjoint from every other thread's, and `tile_2d32_rt`
// checks it stays inside the row and the slice. Tile-rows r % RATIO != 0 are the
// unrotated tails: those threads return without touching anything.
//
...
//
// Geometry: grid (1, heads*RATIO), block (HALF, 1): coordinate (col = j, row = r).
// Half the blocks (the tails) exit at once; the hand kernel launches only the heads.
//
// Two variants, differing only in sin/cos:
//   (A) partial_rope_approx — the hand PTX's Cody-Waite reduction with a two-word
//       2*pi, then `sin.approx` / `cos.approx`
//   (B) partial_rope_libm   — `f32::sin_cos`, libdevice's full-range sincosf
//
...
// evidence/kernels/gdn_partial_rope/<host>.json.
...
const RATIO: u32 = (HEAD_DIM / N_ROT) as u32;
...
/// (heads, position, freq_base). Position 0 is the identity; 30000 drives theta far
/// outside [-pi, pi], where an unreduced `sin.approx` would fail.
const PARITY_CASES: [(usize, u32, f32); 5] = [
    (1, 0, 10_000.0),
    (16, 1, 10_000.0),
    (16, 1000, 10_000.0),
    (16, 17, 1_000_000.0),
    (48, 30_000, 1_000_000.0),
];
...
mod kernels {
    ...
    /// written to this thread's pair.
    #[inline(always)]
    fn rotate(
        x: &[f32],
        out: &mut DisjointSlice<f32, RuntimeRowMajorTiles<2, 1>>,
        c: thread::ThreadCoord2D32,
        sin: f32,
        cos: f32,
    ) {
        let w = out.row_width() as usize;
        let ia = c.row() as usize * 2 * w + c.col() as usize;
        let (a, b) = (x[ia], x[ia + w]);
    ...
    #[launch_bounds(64)]
    #[launch_contract(domain = 2, coordinates = u32, block = (64, 1, 1))]
    pub fn partial_rope_approx(
        x: &[f32],
        mut out: DisjointSlice<f32, RuntimeRowMajorTiles<2, 1>>,
        position: u32,
        theta_scale: f32,
    ) {
        ...
        let theta = theta_j(position, theta_scale, c.col());
    ...
    pub fn partial_rope_libm(
        ...
        let (sin, cos) = theta_j(position, theta_scale, c.col()).sin_cos();
        rotate(x, &mut out, c, sin, cos);
    }
}

/// `apply_partial_neox_rope` with theta in f32 (the CPU's), the rest in f64.
fn cpu_partial_rope(x: &[f32], heads: usize, pos: u32, freq_base: f32) -> Vec<f32> {
...
fn measure(
    got: &[f32],
    x: &[f32],
    (heads, pos, freq_base): (usize, u32, f32),
    us: f64,
    eager_us: f64,
) -> Measured {
    let want = cpu_partial_rope(x, heads, pos, freq_base);
    Measured {
        cos: cosine(got, &want),
...
fn seed((heads, pos, _): (usize, u32, f32)) -> u64 {
    0x3522_000C + (heads as u64) * 65_536 + u64::from(pos)
}
...
fn run_oxide(
    ...
    let (heads, pos, freq_base) = case;
    let ts = theta_scale(freq_base);
    let stream = ctx.new_stream().expect("stream");
    let x = make_inputs(heads, seed(case));
    ...
    let config = LaunchConfig2D::new((1, heads as u32 * RATIO), (HALF as u32, 1), 0);
    let launch = |d_out: &mut DeviceBuffer<f32>| {
        let out = RowWidth::new(d_out, HALF as u32);
        if libm {
            let p = module
                .prepare_partial_rope_libm(config)
                .expect("prepare libm");
            module
                .partial_rope_libm(&stream, &p, &d_x, out, pos, ts)
                .expect("launch libm");
        } else {
            let p = module
                .prepare_partial_rope_approx(config)
                .expect("prepare approx");
            module
                .partial_rope_approx(&stream, &p, &d_x, out, pos, ts)
...
/// grid (heads), block HALF, params (x, position, theta_scale), in place.
fn run_handptx(ctx: &Arc<CudaContext>, sm: &str, case: (usize, u32, f32)) -> (Measured, u32) {
    let (heads, pos, freq_base) = case;
    let ptx_path = format!("baseline-ptx/gdn_partial_neox_rope.{sm}.ptx");
    let ptx = std::fs::read_to_string(&ptx_path).unwrap_or_else(|e| {
        eprintln!("missing hand-PTX baseline {ptx_path}: {e}");
        eprintln!("regenerate: APR_BLESS_PTX=1 cargo test -p aprender-gpu --lib gdn_partial_rope_ptx_golden");
        std::process::exit(2);
    });
    let stream = ctx.new_stream().expect("stream");
    let x = make_inputs(heads, seed(case));
    ...
    let func = module
        .load_function("gdn_partial_neox_rope")
        .expect("gdn_partial_neox_rope");
    ...
    let mut launch = || {
        ...
        // the buffer holds heads * HEAD_DIM f32, the head width this PTX was baked
        // with, and a heads x HALF launch touches only the rotated prefixes.
        unsafe {
...
fn main() {
    ...
    println!("== #3522 cuda-oxide partial NEOX RoPE ({sm}) ==");
    println!(
        "   head_dim={HEAD_DIM} n_rot={N_ROT} 1 thread/(head, pair), timed at pos={TIMED_POS} freq_base={TIMED_FREQ_BASE:e}"
    );
    ...
    for libm in [false, true] {
        ...
        for case in PARITY_CASES {
            ...
            all_ok &= ok;
            println!(
                "  parity {name} heads={} pos={} freq_base={:e}: cos={:.7} maxdiff={:.3e} tail={} {}",
                case.0,
                case.1,
                case.2,
                r.cos,
    ...
    for libm in [false, true] {
        let (variant, entry) = if libm {
            ("libm", "partial_rope_libm")
        } else {
            ("approx", "partial_rope_approx")
        };
    ...
    println!("#3522 PARTIAL ROPE DONE");
}
