#!/usr/bin/env bash
# #3522 OXIDE-001 port: run the partial NEOX RoPE port on THIS host and write
# evidence/kernels/gdn_partial_rope/<host>.json (schema apr-kernel-receipt/v1).
#
...
host="$(hostname -s)"
out_dir="$repo/evidence/kernels/gdn_partial_rope"
log="$(mktemp)"
trap 'rm -f "$log"' EXIT
...
print(json.dumps({"schema": "apr-kernel-receipt/v1", "kernel": "gdn_partial_rope",
