#!/usr/bin/env bash
# #3522 OXIDE-001 port: run the row-batched partial NEOX RoPE port on THIS host and write
# evidence/kernels/gdn_partial_rope_rows/<host>.json (schema apr-kernel-receipt/v1).
#
...
host="$(hostname -s)"
out_dir="$repo/evidence/kernels/gdn_partial_rope_rows"
log="$(mktemp)"
trap 'rm -f "$log"' EXIT
...
print(json.dumps({"schema": "apr-kernel-receipt/v1", "kernel": "gdn_partial_rope_rows",
