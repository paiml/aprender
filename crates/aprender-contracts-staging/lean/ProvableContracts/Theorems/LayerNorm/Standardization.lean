...
theorem variance_layernorm_ratio {n : ℕ} (x : RVec (n + 1)) (eps : ℝ)
  ...
  have hmean : mean (layernorm x (fun _ => 1) (fun _ => 0) eps) = 0 := by
    ...
    unfold mean
    simp
  -- denom² = variance x + ε (ln_denom kept folded in the statement).
  ...
  have hLi : ∀ i, layernorm x (fun _ => 1) (fun _ => 0) eps i
    ...
    intro i
    simp [layernorm]
  -- Expand variance everywhere; keep ln_denom opaque.
  unfold variance at hdenom ⊢
  rw [hmean]
  simp only [sub_zero, hLi, div_pow]
  rw [← Finset.sum_div, hdenom]
  ring
