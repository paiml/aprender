...
/// Regression probe for the `DEVICE_INIT_LOCK` reentrancy self-deadlock.
///
...
/// reentrant, so this hung FOREVER whenever `list_adapters()` was the first
/// GPU call in the process — precisely what a fresh CI lane does. It never
/// reproduced under a full test run because `is_available()` primed the
/// `OnceLock` without holding the lock.
///
/// This test MUST stay first and MUST NOT be preceded by an `is_available()`
/// call, or it stops testing anything.
#[test]
fn list_adapters_does_not_deadlock_as_first_gpu_call() {
    let adapters = GpuDevice::list_adapters();
    eprintln!("list_adapters() -> {} adapter(s)", adapters.len());
