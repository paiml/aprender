...
///
/// `Clone` so `apr rag` can hand an already-parsed command to [`dispatch`]
...
///
/// Split out from [`run`] so a caller that did its own parsing -- `apr rag`
/// -- executes the identical code path rather than a copy of it.
///
/// # Errors
/// Propagates whatever the selected command returns.
