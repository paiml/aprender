...
impl PtxAnalyzer {
    ...
    pub fn analyze(&self, ptx: &str) -> PtxValidationResult {
        ...
        let shared_mem_u64 =
            Regex::new(r"[sl]t\.shared\.[^\[]+\[%rd\d+\]").expect("valid regex for shared mem");
        let entry_point =
            Regex::new(r"\.visible\s+\.entry\s+(\w+)").expect("valid regex for entry point");
        ...
        for line in &lines {
            ...
            if let Some(caps) = loop_label.captures(trimmed) {
                let label = caps
                    .get(1)
                    .expect("capture group 1 in loop_label regex")
                    .as_str();
        ...
        for (line_num, line) in lines.iter().enumerate() {
            ...
            if let Some(caps) = entry_point.captures(trimmed) {
                kernel_names.push(
                    caps.get(1)
                        .expect("capture group 1 in entry_point regex")
                        .as_str()
                        .to_string(),
                );
            }
            ...
            if let Some(caps) = branch_instr.captures(trimmed) {
                let target = caps
                    .get(1)
                    .expect("capture group 1 in branch_instr regex")
                    .as_str();
