...
fn process_certification_result(
    ...
    config: aprender_qa_runner::ExecutionConfig,
    tier: CertTier,
    tier_str: &str,
    ...
    oracle_enhance: bool,
    output_dir: &PathBuf,
    certifications: &mut [aprender_qa_certify::ModelCertification],
    short: &str,
    ...
    match execute_playbook(playbook, config) {
        Ok(result) => {
            ...
            let profile = if matches!(tier, CertTier::DimensionalSmoke) {
                aprender_qa_runner::SixColumnProfile::default()
            } else {
...
fn print_certification_scores(
    ...
    raw_score: u32,
    grade: &str,
    status: aprender_qa_certify::CertificationStatus,
) {
...
fn print_execution_summary(result: &aprender_qa_runner::ExecutionResult) {
...
fn compute_certification_scores(
    ...
    u32,
    aprender_qa_certify::CertificationStatus,
    String,
    aprender_qa_report::MqsScore,
)> {
    use aprender_qa_certify::{CertificationTier, grade_from_tier, score_from_tier, status_from_tier};
...
fn run_profiling_phase(
    ...
    apr_binary: &str,
    fail_fast: bool,
) -> aprender_qa_runner::SixColumnProfile {
    let has_failures = result.failed > 0 || result.gateway_failed.is_some();
    let mut profile = aprender_qa_runner::SixColumnProfile::default();
    ...
    match aprender_qa_runner::run_six_column_profile(apr_binary, &model_dir, 1, 2) {
        Ok(p) => {
            profile = p;
...
fn print_profiling_results(profile: &aprender_qa_runner::SixColumnProfile) {
...
fn check_profiling_assertions(
    profile: &mut aprender_qa_runner::SixColumnProfile,
    playbook: &aprender_qa_runner::Playbook,
) {
...
fn update_certification_record(
    ...
    status: aprender_qa_certify::CertificationStatus,
    tier_str: &str,
    mqs: &aprender_qa_report::MqsScore,
    profile: &aprender_qa_runner::SixColumnProfile,
) {
    use aprender_qa_certify::CertificationStatus;
    use chrono::Utc;
...
fn save_evidence(model_output: &std::path::Path, result: &aprender_qa_runner::ExecutionResult) -> bool {
...
fn run_oracle_enhancement(
    ...
    use aprender_qa_runner::{OracleEnhancer, generate_checklist_markdown};
    ...
    let grade = aprender_qa_certify::grade_from_score(mqs);
