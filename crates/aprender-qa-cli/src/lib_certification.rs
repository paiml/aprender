...
pub fn bootstrap_playbook_from_contract(
    ...
    use aprender_qa_gen::bootstrapper;
    use aprender_qa_runner::family_contract::FamilyRegistry;
    ...
    let config = aprender_qa_gen::BootstrapConfig {
...
pub fn generate_lock_file(dir: &Path, output: &Path) -> Result<usize, String> {
    use aprender_qa_runner::{PlaybookLockFile, generate_lock_entry, save_lock_file};
    use std::collections::HashMap;

    // Walk directory for .playbook.yaml files
    /// Recursively walk a directory collecting playbook lock entries
    fn walk_dir(dir: &Path, entries: &mut HashMap<String, aprender_qa_runner::PlaybookLockEntry>) {
...
pub fn execute_auto_tickets(evidence: &[Evidence], _repo: &str) -> Vec<UpstreamTicket> {
    let defect_map = match aprender_qa_report::defect_map::load_defect_fixture_map() {
        Ok(map) => map,
        Err(e) => {
