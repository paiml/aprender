...
fn export_csv(evidence_dir: &Path, output: &Path, append: bool) {
    use aprender_qa_report::write_models_csv;
...
fn load_existing_csv_rows(output: &Path, append: bool) -> Vec<aprender_qa_report::CertificationRow> {
    use aprender_qa_report::read_models_csv;
...
fn process_evidence_files(
    evidence_dir: &Path,
    rows: &mut Vec<aprender_qa_report::CertificationRow>,
) -> (usize, usize) {
...
fn process_single_file(
    ...
    let Ok(export) = serde_json::from_str::<aprender_qa_report::EvidenceExport>(&content) else {
...
fn update_row_from_export(
    ...
    use aprender_qa_report::CertificationRow;
    use chrono::Utc;
...
fn derive_status_from_mqs(mqs: &aprender_qa_report::MqsExport) -> aprender_qa_report::ModelStatus {
    use aprender_qa_report::ModelStatus;
...
fn update_gateway_flags(
    row: &mut aprender_qa_report::CertificationRow,
