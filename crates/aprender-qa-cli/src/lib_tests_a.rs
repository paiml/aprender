use super::*;
use aprender_qa_gen::{Backend, Format, Modality, QaScenario};
...
fn test_parse_failure_policy_stop_on_first() {
    let policy = parse_failure_policy("stop-on-first").expect("known failure policy parses");
    assert!(matches!(policy, FailurePolicy::StopOnFirst));
}

#[test]
fn test_parse_failure_policy_stop_on_p0() {
    let policy = parse_failure_policy("stop-on-p0").expect("known failure policy parses");
    assert!(matches!(policy, FailurePolicy::StopOnP0));
}

#[test]
fn test_parse_failure_policy_collect_all() {
    let policy = parse_failure_policy("collect-all").expect("known failure policy parses");
...
fn test_parse_failure_policy_fail_fast() {
    let policy = parse_failure_policy("fail-fast").expect("known failure policy parses");
...
fn test_scenarios_to_yaml() {
    ...
    assert!(yaml.is_ok());
    let yaml_str = yaml.expect("scenarios serialize to YAML");
...
fn test_scenarios_to_json() {
    ...
    assert!(json.is_ok());
    let json_str = json.expect("scenarios serialize to JSON");
...
fn test_generate_html_report() {
    let evidence = vec![make_corroborated_evidence()];
    let collector = collect_evidence(evidence);
    let mqs = calculate_mqs_score("test/model", &collector).expect("MQS score calculates");
    ...
    assert!(html.is_ok());
    assert!(html.expect("HTML report renders").contains("<html"));
}
...
fn test_generate_junit_report() {
    ...
    assert!(xml.is_ok());
    assert!(xml
        .expect("JUnit XML report renders")
        .contains("<testsuite"));
}
...
fn test_build_execution_config() {
    ...
    let exec = exec_config.expect("execution config builds");
    assert!(!exec.dry_run);
...
fn test_build_execution_config_with_options() {
    ...
    let exec_config = build_execution_config(&config).expect("execution config builds");
...
fn test_scenarios_yaml_roundtrip() {
    let scenarios = generate_model_scenarios("test/model", 1);
    let yaml = scenarios_to_yaml(&scenarios).expect("scenarios serialize to YAML");
...
fn test_scenarios_json_roundtrip() {
    let scenarios = generate_model_scenarios("test/model", 1);
    let json = scenarios_to_json(&scenarios).expect("scenarios serialize to JSON");
    // Should be valid JSON that can be parsed back
    let parsed: Vec<aprender_qa_gen::QaScenario> =
