...
fn run_certification(
    ...
    use aprender_qa_certify::write_csv;
...
fn load_certification_csv() -> (PathBuf, Vec<aprender_qa_certify::ModelCertification>) {
    use aprender_qa_certify::parse_csv;
...
fn determine_models_to_certify(
    ...
    family: Option<&str>,
    model_ids: &[String],
    certifications: &[aprender_qa_certify::ModelCertification],
) -> Vec<String> {
    if all {
...
fn resolve_models_for_certification(
    ...
    kernel_class.map_or_else(
        ...
        |kc_str| {
            let kc: aprender_qa_gen::KernelClass = match kc_str.parse() {
                Ok(k) => k,
                Err(e) => {
                    eprintln!("{e}");
                    std::process::exit(1);
            ...
            let families_and_ids: Vec<(String, String)> = certifications
                ...
                .collect();
            let models = aprender_qa_gen::models_in_class(kc, &families_and_ids);
            if models.is_empty() {
...
fn verify_playbook_lock_or_exit(playbook_path: &std::path::Path, playbook_name: &str) {
    ...
    if lock_path.exists() {
        match aprender_qa_runner::load_lock_file(lock_path) {
            Ok(lock_file) => {
                if let Err(e) = aprender_qa_runner::verify_playbook_integrity(
                    playbook_path,
                    &lock_file,
                    playbook_name,
                ) {
...
fn verify_playbook_lock(
    ...
    match aprender_qa_runner::load_lock_file(lock_path) {
        Ok(lock_file) => {
            if let Err(e) =
                aprender_qa_runner::verify_playbook_integrity(playbook_path, &lock_file, playbook_name)
            {
                eprintln!("  [INTEGRITY] {e}");
                eprintln!(
