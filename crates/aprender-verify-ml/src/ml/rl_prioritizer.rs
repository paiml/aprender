...
impl RLTestPrioritizer {
    ...
    fn sample_failure_probability<R: rand::Rng>(&self, sig: &FeatureSignature, rng: &mut R) -> f64 {
        ...
        // Beta::new(1.0, 1.0) is mathematically guaranteed to succeed with positive params
        let beta_dist = Beta::new(alpha, beta).unwrap_or_else(|_| Beta::new(1.0, 1.0).expect("Beta(1.0, 1.0) is always valid"));
