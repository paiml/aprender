...
//!      — and when git cannot answer at all, HEAD read straight from `.git` with no git binary
//!      (#4254: the release asset said `v0.69.1+no-git` because `git` fails inside the sibling
//!      build container over a bind-mounted checkout, while the checkout's `.git` is right there)
...
fn resolve_git_sha() -> String {
    ...
    let git_head = run_git(&head)
        ...
        .or_else(read_head_sha_from_dot_git);
