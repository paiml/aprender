...
fn deleting_a_model_of_removes_exactly_that_modules_credit() {
    ...
    let credit = |models: &[Model]| {
        let mut g = Grounding::from_summary(Ok(green()), Some("t"));
        g.require_refinement(&l4_modules(&judge(models, &tree(), resolver)));
        g
    };
    let g = credit(&both);
    assert!(g.grounds_all(&["T.sm".into(), "T.gelu".into()]));
    assert!(g.unrefined.is_empty());

    let g = credit(&both[..1]);
    assert!(g.grounds_all(&["T.sm".into()]));
    assert!(
        !g.grounds_all(&["T.gelu".into()]),
        "GELU lost its model, so its credit"
    );
