...
//! `ont:Kernel` focus node, and the tracked kernel receipts (`apr-kernel-receipt/v1`) are joined to it.
//!
...
//! (pure Rust → PTX). This extractor adds one `rdf:type ont:Kernel` to the SAME IRI and declares
//! `ont:Kernel rdfs:subClassOf ont:Symbol`, so every `ont:Symbol` shape still sees it. A hand-written PTX string
...
pub fn resolve(g: &mut Graph, inputs: &Inputs) -> KernelStats {
    ...
    for k in &kernels {
        let name = literal(g, k, &sym("name"))
            .first()
            .map(|s| (*s).to_string())
            .unwrap_or_default();
        let module = literal(g, k, &sym("module"))
            .first()
            .map(|s| (*s).to_string())
            .unwrap_or_default();
        let unsafe_free = literal(g, k, &sym("unsafeFree")).contains(&"true");
        let bounds_checked = literal(g, k, &sym("boundsChecked")).contains(&"true");
        g.insert(k.clone(), RDF_TYPE, Term::iri(ont("Kernel")));
        g.insert(k.clone(), kern("authoring"), Term::string("oxide"));
        match reference_of(g, k) {
            Some(r) => g.insert(k.clone(), kern("reference"), Term::iri(r)),
            None => g.insert(
                k.clone(),
        ...
        if let Some(ex) = inputs.exemptions.get(&name) {
            g.insert(k.clone(), kern("exemptionReceipt"), Term::string(ex));
        }
        let mut parity_on: BTreeSet<String> = BTreeSet::new();
        let mut timing_on: BTreeSet<String> = BTreeSet::new();
        ...
        for (fi, rf) in inputs.receipts.iter().enumerate() {
            for (ri, row) in rf.rows.iter().enumerate() {
                ...
                g.insert(k.clone(), kern("parityReceipt"), Term::iri(&rnode));
                measured.insert(row.host.clone());
                if let Some(b) = row.register_budget {
                    budget = true;
                    g.insert(k.clone(), kern("registerBudget"), Term::integer(b));
                }
                if row.cos.is_some_and(|c| c >= PARITY_MIN_COS)
                    && row.maxdiff.is_some_and(|d| d.abs() < PARITY_MAX_DIFF)
                {
                    parity_on.insert(row.host.clone());
                }
                let within = match (row.oxide_us, row.ptx_us) {
                    (Some(o), Some(p)) if p > 0.0 => o / p <= TIMING_MAX_RATIO,
                    _ => false,
                };
                if within || inputs.exemptions.contains_key(&name) {
                    timing_on.insert(row.host.clone());
                }
            }
        }
        let required: BTreeSet<String> = match &inputs.required_hosts {
            Some(h) => h.iter().cloned().collect(),
            None => measured,
        };
        for h in &parity_on {
            g.insert(k.clone(), kern("parityOn"), Term::string(h));
        }
        for h in &timing_on {
            g.insert(k.clone(), kern("timingOn"), Term::string(h));
        }
        for h in required.difference(&parity_on) {
            g.insert(k.clone(), kern("missingParityHost"), Term::string(h));
        }
        for h in required.difference(&timing_on) {
            g.insert(k.clone(), kern("missingTimingHost"), Term::string(h));
        }
        let safe = unsafe_free && bounds_checked && budget;
        g.insert(k.clone(), kern("safety"), Term::string(safe.to_string()));
    }
...
pub fn positive_control() -> bool {
    ...
    !ghost.objects(&k, &kern("referenceMissing")).is_empty()
        ...
        && ok.instances_of(&ont("Kernel")).contains(&k.as_str())
