...
fn format_playbook_output(
    ...
    match args.format {
        probador::PlaybookOutputFormat::Json => {
            let result = probador::handlers::comply::json_object([
...
"#,
        ...
        probador::ComplyReportFormat::Json => probador::handlers::comply::json_object([
            ...
            (
                ...
                results
                    .iter()
                    .map(|r| {
                        probador::handlers::comply::json_object([
                            ("id", r.id.clone().into()),
...
mod tests {
    ...
    mod run_coverage_tests {
        ...
        fn test_run_coverage_with_json() {
            ...
            // Verify JSON content
            let content = std::fs::read_to_string(&json_path).expect("read file");
    ...
    mod compliance_check_tests {
        ...
        fn test_check_c001_with_wasm_and_tests() {
            ...
            fs::write(temp.path().join("app.wasm"), b"wasm").expect("write file");
            fs::write(temp.path().join("test.rs"), b"#[test]").expect("write file");
        ...
        fn test_check_c003_with_custom_elements() {
            ...
            fs::write(temp.path().join("index.html"), html).expect("write file");
        ...
        fn test_check_c006_with_htaccess() {
            let temp = TempDir::new().expect("construct");
            fs::write(temp.path().join(".htaccess"), "Header set").expect("write file");
        ...
        fn test_check_c006_with_vercel() {
            let temp = TempDir::new().expect("construct");
            fs::write(temp.path().join("vercel.json"), "{}").expect("write file");
        ...
        fn test_check_c006_with_netlify() {
            let temp = TempDir::new().expect("construct");
            fs::write(temp.path().join("netlify.toml"), "").expect("write file");
        ...
        fn test_check_c009_wasm_size_under_limit() {
            let temp = TempDir::new().expect("construct");
            fs::write(temp.path().join("small.wasm"), vec![0u8; 1000]).expect("write file");
        ...
        fn test_check_c009_wasm_size_over_limit() {
            let temp = TempDir::new().expect("construct");
            fs::write(temp.path().join("large.wasm"), vec![0u8; 10000]).expect("write file");
...
panic = "abort""#;
            fs::write(temp.path().join("Cargo.toml"), cargo).expect("write file");
    ...
    mod cross_origin_config_tests {
        ...
        fn test_check_makefile_cross_origin_package_json() {
            ...
            fs::write(temp.path().join("package.json"), pkg).expect("write file");
