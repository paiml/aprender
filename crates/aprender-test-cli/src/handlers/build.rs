...
mod tests {
    ...
    fn test_find_html_files_with_files() {
        ...
        std::fs::write(temp.path().join("about.html"), "<html></html>")
            .expect("write test fixture");
    ...
    fn test_find_html_files_nested() {
        let temp = TempDir::new().expect("create temp dir");
        std::fs::create_dir_all(temp.path().join("pages")).expect("create test directory tree");
        std::fs::write(temp.path().join("index.html"), "<html></html>")
            .expect("write test fixture");
        std::fs::write(
    ...
    fn test_html_references_wasm_true() {
        ...
        std::fs::write(&html_path, r#"<script src="app.wasm"></script>"#)
            .expect("write test fixture");
    ...
    fn test_html_references_wasm_false() {
        let temp = TempDir::new().expect("create temp dir");
        let html_path = temp.path().join("index.html");
        std::fs::write(&html_path, r"<html><body>Hello</body></html>").expect("write test fixture");
    ...
    fn test_find_wasm_pages() {
        ...
        std::fs::write(&html1, r#"<script src="app.wasm"></script>"#).expect("write test fixture");
        std::fs::write(&html2, r"<html></html>").expect("write test fixture");
    ...
    fn test_scan_files_recursive_skips_target() {
        ...
        std::fs::write(temp.path().join("lib.rs"), "fn main() {}").expect("write test fixture");
        std::fs::write(temp.path().join("target").join("output.rs"), "")
            .expect("write test fixture");
