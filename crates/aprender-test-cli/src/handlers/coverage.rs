...
mod tests {
    ...
    fn test_load_coverage_from_json_array_format() {
        ...
        let cells = load_coverage_from_json(&path).expect("load coverage from json");
    ...
    fn test_load_coverage_from_json_invalid() {
        ...
        std::fs::write(&path, "not valid json").expect("write file");
    ...
    fn test_execute_coverage_with_json_output() {
        ...
        let _: PixelCoverageReport = serde_json::from_str(&content).expect("parse");
    }
}
