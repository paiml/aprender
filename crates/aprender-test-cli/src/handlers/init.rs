...
mod tests {
    ...
    fn test_is_valid_init_path_nonempty_dir() {
        let temp = TempDir::new().expect("create temp dir");
        std::fs::write(temp.path().join("existing.rs"), "// content").expect("write test fixture");
        assert!(!is_valid_init_path(temp.path()));
    }
    ...
    fn test_is_valid_init_path_dir_with_hidden_files() {
        let temp = TempDir::new().expect("create temp dir");
        std::fs::write(temp.path().join(".gitignore"), "target/").expect("write test fixture");
        assert!(is_valid_init_path(temp.path()));
    }
    ...
    fn test_execute_init_force_overwrites() {
        ...
        std::fs::create_dir_all(project_path.join("tests")).expect("create test directory tree");
