...
mod tests {
    ...
    fn test_find_edl_files_empty() {
        let temp = tempfile::TempDir::new().expect("construct");
    ...
    fn test_find_edl_files_with_edl() {
        let temp = tempfile::TempDir::new().expect("construct");
        std::fs::write(temp.path().join("video.edl.json"), "{}").expect("write file");
        std::fs::write(temp.path().join("other.json"), "{}").expect("write file");
        let files = find_edl_files(temp.path());
    ...
    fn test_find_video_for_edl_mp4() {
        ...
        std::fs::write(&edl, "{}").expect("write file");
        std::fs::write(&video, "").expect("write file");
    ...
    fn test_load_edl_valid() {
        ...
        std::fs::write(&edl_path, r#"{"video_id":"test","decisions":[]}"#).expect("write file");
        let edl = load_edl(&edl_path).expect("load edl");
    ...
    fn test_load_edl_invalid_json() {
        ...
        std::fs::write(&edl_path, "not json").expect("write file");
