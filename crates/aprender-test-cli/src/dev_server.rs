...
mod tests {
    ...
    async fn test_serve_static_directory_serves_index_html() {
        ...
        std::fs::write(subdir.join("index.html"), "<html>test</html>").expect("write test fixture");

        let directory = Arc::new(temp_dir.path().to_path_buf());
        let uri: axum::http::Uri = "/subdir/".parse().expect("parse test literal");
    ...
    async fn test_serve_static_directory_without_trailing_slash() {
        ...
        std::fs::create_dir(&subdir).expect("create test directory");
        std::fs::write(subdir.join("index.html"), "<html>works</html>")
            .expect("write test fixture");

        let directory = Arc::new(temp_dir.path().to_path_buf());
        let uri: axum::http::Uri = "/mydir".parse().expect("parse test literal");
    ...
    async fn test_serve_static_directory_no_index_returns_error() {
        ...
        let directory = Arc::new(temp_dir.path().to_path_buf());
        let uri: axum::http::Uri = "/empty/".parse().expect("parse test literal");
    ...
    fn test_module_validator_validates_existing_file() {
        ...
        std::fs::create_dir(&pkg_dir).expect("create test directory");
        std::fs::write(pkg_dir.join("app.js"), "export default {}").expect("write test fixture");
        std::fs::write(
