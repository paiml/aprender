...
mod tests {
    ...
    fn test_lint_html_missing_doctype() {
        ...
        std::fs::write(&html_path, "<html><head></head><body></body></html>").expect("write file");
    ...
    fn test_lint_css_mismatched_braces() {
        ...
        std::fs::write(&css_path, "body { color: red;").expect("write file");
    ...
    fn test_lint_json_invalid() {
        ...
        std::fs::write(&json_path, "{invalid json}").expect("write file");
    ...
    fn test_lint_json_valid() {
        let temp = TempDir::new().expect("construct");
        let json_path = temp.path().join("test.json");
        std::fs::write(&json_path, r#"{"key": "value"}"#).expect("write file");
    ...
    fn test_lint_wasm_invalid_magic() {
        ...
        std::fs::write(&wasm_path, b"not wasm data here").expect("write file");
    ...
    fn test_render_lint_json() {
        ...
        let json = render_lint_json(&report).expect("render lint json");
    ...
    fn test_lint_directory_full() {
        ...
        .expect("write file");
        std::fs::write(temp.path().join("style.css"), "body { color: red; }").expect("write file");
        std::fs::write(temp.path().join("app.js"), "function test() {}").expect("write file");
        std::fs::write(temp.path().join("data.json"), r#"{"key": "value"}"#).expect("write file");
    ...
    fn test_lint_directory_with_errors() {
        ...
        std::fs::write(temp.path().join("bad.html"), "<html>no doctype</html>")
            .expect("write file");
        std::fs::write(temp.path().join("bad.css"), "body { color: red").expect("write file"); // missing }
        std::fs::write(temp.path().join("bad.json"), "{invalid}").expect("write file");
    ...
    fn test_lint_directory_skips_hidden() {
        ...
        std::fs::create_dir(&hidden).expect("create dir");
        std::fs::write(hidden.join("test.html"), "<html>bad</html>").expect("write file");
    ...
    fn test_lint_directory_skips_node_modules() {
        ...
        std::fs::create_dir(&node_modules).expect("create dir");
        std::fs::write(node_modules.join("lib.js"), "console.log('test');").expect("write file");
    ...
    fn test_lint_file_unknown_extension() {
        ...
        std::fs::write(&txt_path, "Just some text").expect("write file");
    ...
    fn test_lint_file_mjs_extension() {
        ...
        std::fs::write(&mjs_path, "export function test() {}").expect("write file");
    ...
    fn test_lint_disabled_html() {
        let temp = TempDir::new().expect("construct");
        let html_path = temp.path().join("test.html");
        std::fs::write(&html_path, "<html>no doctype</html>").expect("write file");
    ...
    fn test_lint_disabled_css() {
        let temp = TempDir::new().expect("construct");
        let css_path = temp.path().join("test.css");
        std::fs::write(&css_path, "body { color: red").expect("write file"); // Missing brace
    ...
    fn test_lint_disabled_js() {
        let temp = TempDir::new().expect("construct");
        let js_path = temp.path().join("test.js");
        std::fs::write(&js_path, "console.log('debug');").expect("write file");
    ...
    fn test_lint_disabled_wasm() {
        let temp = TempDir::new().expect("construct");
        let wasm_path = temp.path().join("test.wasm");
        std::fs::write(&wasm_path, b"not valid wasm").expect("write file");
    ...
    fn test_lint_disabled_json() {
        let temp = TempDir::new().expect("construct");
        let json_path = temp.path().join("test.json");
        std::fs::write(&json_path, "{invalid}").expect("write file");
    ...
    fn test_lint_wasm_too_small() {
        ...
        std::fs::write(&wasm_path, b"tiny").expect("write file");
    ...
    fn test_lint_js_console_log() {
        let temp = TempDir::new().expect("construct");
        let js_path = temp.path().join("test.js");
        std::fs::write(&js_path, "console.log('debugging');").expect("write file");
    ...
    fn test_lint_js_mismatched_braces() {
        let temp = TempDir::new().expect("construct");
        let js_path = temp.path().join("test.js");
        std::fs::write(&js_path, "function test() { return 1;").expect("write file");
    ...
    fn test_lint_js_mismatched_parens() {
        let temp = TempDir::new().expect("construct");
        let js_path = temp.path().join("test.js");
        std::fs::write(&js_path, "function test( { return 1; }").expect("test");
