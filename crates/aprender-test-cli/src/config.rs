...
mod tests {
    ...
    mod verbosity_tests {
        ...
        #[test]
        fn test_serialize() {
            let json = serde_json::to_string(&Verbosity::Debug).expect("serialize to JSON string");
            assert!(json.contains("Debug"));
        }

        #[test]
        fn test_deserialize() {
            let v: Verbosity = serde_json::from_str("\"Quiet\"").expect("deserialize test input");
    ...
    mod color_choice_tests {
        ...
        fn test_serialize() {
            let json =
                serde_json::to_string(&ColorChoice::Always).expect("serialize to JSON string");
            assert!(json.contains("Always"));
        }

        #[test]
        fn test_deserialize() {
            let c: ColorChoice = serde_json::from_str("\"Never\"").expect("deserialize test input");
    ...
    mod cli_config_tests {
        ...
        fn test_deserialize() {
            ...
            let config: CliConfig = serde_json::from_str(json).expect("deserialize test input");
            assert_eq!(config.verbosity, Verbosity::Debug);
            assert_eq!(config.color, ColorChoice::Always);
            assert_eq!(config.parallel_jobs, 4);
            assert!(config.fail_fast);
