...
// The kernels are AVX2/FMA/AVX-VNNI: `std::arch::x86_64` plus inline `asm!` with
// ymm registers. On any other target the example still builds and runs (G3.EX: every
// example must build AND run before a tag) and says so -- it does not pretend to bench.
...
mod x86 {
    ...
    unsafe fn dot_avx2(q4_data: &[u8], q8_scales: &[f32], q8_quants: &[i8]) -> f32 {
        ...
        for block_idx in 0..num_blocks {
            ...
            let q4_hi =
                _mm256_and_si256(_mm256_cvtepu8_epi16(_mm_srli_epi16(q4_packed, 4)), low_mask);
...
fn main() {
    eprintln!(
        "bench_simd_dot: x86_64-only kernels (AVX2/FMA/AVX-VNNI); nothing to run on this target"
    );
}
