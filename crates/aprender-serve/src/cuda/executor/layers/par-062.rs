...
impl CudaExecutor {
    ...
    pub fn batched_gpu_argmax(
        ...
        let block_max_vals_ptr = self.argmax_block_vals.as_ref().expect("argmax_block_vals must be allocated before reduce").as_ptr();
        let block_max_idxs_ptr = self.argmax_block_idxs.as_ref().expect("argmax_block_idxs must be allocated before reduce").as_ptr();
        let batched_results_base = self.batched_argmax_results.as_ref().expect("batched_argmax_results must be allocated before reduce").as_ptr();
