    ...
    pub fn forward_all_layers_gpu_to_logits(
        ...
        if stage_dump_cfg.is_some() {
            ...
            if let Some(buf) = last_buf {
                let mut host = vec![0.0f32; (hidden_dim as usize).min(buf.len())];
                if buf.copy_to_host(&mut host).is_ok() {
                    if let Err(e) =
                    ...
                    {
                        eprintln!("[SHIP-007-PR-B] post_ffn_residual dump failed: {e}");
                    }
                }
            }
        }
        ...
        if stage_dump_cfg.is_some() {
            let mut host = vec![0.0f32; (hidden_dim as usize).min(normed_hidden.len())];
            if normed_hidden.copy_to_host(&mut host).is_ok() {
                if let Err(e) = crate::inference_trace::gpu_stage_dump::maybe_dump_host_buffer(
                    ...
                    eprintln!("[SHIP-007-PR-B] final_norm dump failed: {e}");
                }
            }
        }
