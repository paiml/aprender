//! #4234: `batched_mwv_q{4,6}k_gemv_into` is `mwv_q{4,6}k_gemv_into` per vector, BITWISE.
//!
...
/// Deterministic Q6_K rows: random ql/qh/scales, a sane `d` (f16 at byte 208).
fn q6k_rows(n: usize, k: usize, seed: u64) -> Vec<u8> {
    ...
    for sb in out.chunks_exact_mut(SB_BYTES) {
        ...
        // d = 0.0123 as an f16 bit pattern (finite, small).
        sb[208..210].copy_from_slice(&0x224C_u16.to_le_bytes());
    }
    out
}
...
#[test]
#[serial]
fn batched_mwv_q6k_gemv_is_bitwise_the_single_vector_kernel_per_vector() {
    let mut executor = crate::cuda_executor_or_skip!(0);
    // (k, n): the lm_head's k and an ffn_down-like k; Q6_K Mwv needs k % 256 == 0.
    for (case, &(k, n)) in [(1024u32, 96u32), (3584, 40)].iter().enumerate() {
        let name = format!("bmwv6_4234_{case}");
        executor
            .load_quantized_weights(&name, &q6k_rows(n as usize, k as usize, 11 + case as u64))
            .expect("load weights");
        let w = executor.get_quantized_weight_ptr(&name).expect("ptr");
        for m in 1u32..=12 {
            let x = activations(m as usize, k as usize);
            ...
            let output = GpuBuffer::<f32>::new(executor.context(), (m * n) as usize).expect("out");
            executor
                .batched_mwv_q6k_gemv_into(w, &input, &output, m, n, k)
                .expect("batched");
            executor.synchronize().expect("sync");
            let mut got = vec![0.0f32; (m * n) as usize];
            output.copy_to_host(&mut got).expect("download");
            ...
            for r in 0..m as usize {
                ...
                let one_out = GpuBuffer::<f32>::new(executor.context(), n as usize).expect("out");
                executor
                    .mwv_q6k_gemv_into(w, &one_in, &one_out, n, k)
                    .expect("single");
                executor.synchronize().expect("sync");
                let mut want = vec![0.0f32; n as usize];
                one_out.copy_to_host(&mut want).expect("download");
...
#[test]
#[serial]
fn batched_mwv_q6k_gemv_refuses_an_unaligned_k() {
    let mut executor = crate::cuda_executor_or_skip!(0);
    let (k, n) = (512u32, 8u32);
    executor
        .load_quantized_weights("bmwv6_4234_refuse", &q6k_rows(n as usize, k as usize, 1))
        .expect("load");
    let w = executor
        .get_quantized_weight_ptr("bmwv6_4234_refuse")
        .expect("ptr");
    let input = GpuBuffer::<f32>::new(executor.context(), k as usize).expect("in");
    let output = GpuBuffer::<f32>::new(executor.context(), n as usize).expect("out");
    let err = executor
        .batched_mwv_q6k_gemv_into(w, &input, &output, 1, n, 300)
        .expect_err("k=300");
    assert!(err.to_string().contains("multiple of 256"), "{err}");
}

/// `n` rows of `k` Q5_K weights (176-byte super-blocks: d, dmin, scales, qh, qs)
/// with small finite `d` / `dmin` and pseudo-random scales and quants.
fn q5k_rows(n: usize, k: usize, seed: u64) -> Vec<u8> {
    const SB_BYTES: usize = 176;
    let sbs = k.div_ceil(256);
    let mut s = seed;
    let mut next = move || {
        s = s
    ...
    for sb in out.chunks_exact_mut(SB_BYTES) {
        ...
        // d = 0.0123, dmin = 0.0061 as f16 bit patterns (finite, small).
        sb[0..2].copy_from_slice(&0x224C_u16.to_le_bytes());
        sb[2..4].copy_from_slice(&0x1E40_u16.to_le_bytes());
    }
    out
}
...
#[test]
#[serial]
fn batched_q5k_gemv_is_bitwise_the_single_vector_kernel_per_vector() {
    let mut executor = crate::cuda_executor_or_skip!(0);
    // (k, n): attn_qkv's and ssm_out's k, and a k that is not a multiple of 256
    // (the kernel bounds-checks the last super-block's activations).
    for (case, &(k, n)) in [(1024u32, 96u32), (2048, 40), (1000, 24)]
        .iter()
        .enumerate()
    {
        let name = format!("bq5_4234_{case}");
        executor
            .load_quantized_weights(&name, &q5k_rows(n as usize, k as usize, 23 + case as u64))
            .expect("load weights");
        let w = executor.get_quantized_weight_ptr(&name).expect("ptr");
        for m in 1u32..=12 {
            let x = activations(m as usize, k as usize);
            ...
            let output = GpuBuffer::<f32>::new(executor.context(), (m * n) as usize).expect("out");
            executor
                .batched_q5k_gemv_into(w, &input, &output, m, n, k)
                .expect("batched");
            executor.synchronize().expect("sync");
            let mut got = vec![0.0f32; (m * n) as usize];
            output.copy_to_host(&mut got).expect("download");
            ...
            for r in 0..m as usize {
                ...
                let one_out = GpuBuffer::<f32>::new(executor.context(), n as usize).expect("out");
                executor
                    .q5k_gemv_into(w, &one_in, &one_out, n, k)
                    .expect("single");
                executor.synchronize().expect("sync");
                let mut want = vec![0.0f32; n as usize];
                one_out.copy_to_host(&mut want).expect("download");
