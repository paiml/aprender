...
/// Assert one `GenerateResponse`. `text` is the completion alone (#3991): it
/// generated something whose raw decode has the marker (the fixture engaged),
/// yet carries none.
fn assert_cut(uri: &str, tokenizer: &BPETokenizer, resp: &serde_json::Value) {
    ...
    // #3991: a native route's `text` is the completion alone, with no prompt echo.
    let completion = text;
