...
fn no_estimator_loaded(state: &AppState) -> (StatusCode, Json<ErrorResponse>) {
    let error = if state.model_loaded() {
        "This server has a generative model loaded, not an APR estimator. \
         /v1/predict serves APR classifier/regressor models; use /v1/completions \
         or /v1/chat/completions for text generation."
    } else {
        "No APR estimator model is loaded. Start the server with an APR \
