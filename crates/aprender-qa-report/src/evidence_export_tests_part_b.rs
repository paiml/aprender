...
fn test_falsify_oracle_004_field_mapping() {
    ...
    assert_eq!(
        export.gates.get("G1-MODEL-LOADS").expect("gate G1-MODEL-LOADS present").passed,
        cert_row.g1
    );
    assert_eq!(
        export.gates.get("G2-BASIC-INFERENCE").expect("gate G2-BASIC-INFERENCE present").passed,
        cert_row.g2
    );
    assert_eq!(
        export.gates.get("G3-NO-CRASHES").expect("gate G3-NO-CRASHES present").passed,
        cert_row.g3
    );
    assert_eq!(
        export.gates.get("G4-OUTPUT-QUALITY").expect("gate G4-OUTPUT-QUALITY present").passed,
        cert_row.g4
    );
}
