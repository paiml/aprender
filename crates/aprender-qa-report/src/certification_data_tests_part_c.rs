...
fn test_size_category_huge_roundtrip() {
    ...
    assert_eq!(s, "huge");
    let parsed: SizeCategory = s.parse().expect("parse");
...
fn test_model_status_untested_roundtrip() {
    ...
    let parsed: ModelStatus = s.parse().expect("parse");
...
fn test_write_csv_all_tps_fields() {
    ...
    let r = &read_back[0];
    assert!((r.tps_gguf_cpu.expect("tps gguf cpu") - 15.5).abs() < 0.1);
    assert!((r.tps_gguf_gpu.expect("tps gguf gpu") - 120.0).abs() < 0.1);
    assert!((r.tps_apr_cpu.expect("tps apr cpu") - 14.0).abs() < 0.1);
    assert!((r.tps_apr_gpu.expect("tps apr gpu") - 110.5).abs() < 0.1);
    assert!((r.tps_st_cpu.expect("tps st cpu") - 3.2).abs() < 0.1);
    assert!((r.tps_st_gpu.expect("tps st gpu") - 25.8).abs() < 0.1);
}
