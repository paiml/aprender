...
fn test_gateway_g1_failure() {
    ...
    let g1 = score.gateways.iter().find(|g| g.id == "G1").expect("gateway G1 present");
    assert!(!g1.passed);
}
...
fn test_gateway_g2_failure() {
    ...
    let g2 = score.gateways.iter().find(|g| g.id == "G2").expect("gateway G2 present");
    assert!(!g2.passed);
}
...
fn test_gateway_g4_failure_garbage_output() {
    ...
    let g4 = score.gateways.iter().find(|g| g.id == "G4").expect("gateway G4 present");
    assert!(!g4.passed);
}
...
fn test_gateway_g0_integrity_failure() {
    ...
    let g0 = score.gateways.iter().find(|g| g.id == "G0").expect("gateway G0 present");
    assert!(!g0.passed);
    assert!(g0.failure_reason.as_ref().expect("failure reason set").contains("1 G0 check"));
}
...
fn test_gateway_g0_integrity_multiple_failures() {
    ...
    assert!(g0.failure_reason.as_ref().expect("failure reason set").contains("3 G0 check"));
}
...
fn test_with_proof_bonus_adds_points() {
    ...
    assert_eq!(score.proof_bonus.as_ref().expect("proof bonus present").bonus_points, 25);
}
