...
// Estimator impl so DecisionTreeClassifier works with generic cross_validate /
// grid_search (Pillar 1). Labels round-trip through f32; inherent API unchanged.
impl crate::traits::Estimator for DecisionTreeClassifier {
    fn fit(
        &mut self,
        ...
        DecisionTreeClassifier::fit(self, x, &labels)
    }
    fn predict(&self, x: &crate::primitives::Matrix<f32>) -> crate::primitives::Vector<f32> {
        let labels: Vec<usize> = DecisionTreeClassifier::predict(self, x);
    ...
    fn score(&self, x: &crate::primitives::Matrix<f32>, y: &crate::primitives::Vector<f32>) -> f32 {
        let preds: Vec<usize> = DecisionTreeClassifier::predict(self, x);
        let n = y.len();
        if n == 0 {
            return 0.0;
        }
