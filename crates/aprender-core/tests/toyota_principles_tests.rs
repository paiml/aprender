...
fn p8b_no_unsafe_code() {
    ...
    let root_toml = provable_contracts::workspace_file_or_skip!("p8b_no_unsafe_code", "Cargo.toml");
    let forbids =
        |t: &str| t.contains("unsafe_code") && (t.contains("forbid") || t.contains("deny"));
    let forbids_unsafe = forbids(crate_toml) || root_toml.as_deref().is_some_and(forbids);
