//! BEAT-SKLEARN-MINMAXSCALER-SPEED — Pillar-1 speed cascade (PMAT-727). **NIGHTLY ONLY.**
//! cargo test -p aprender-core --release --test beat_sklearn_minmax_speed -- --ignored --nocapture
#![cfg(test)]
...
fn time_apr(x: &aprender::Matrix<f32>) -> f64 {
    {
        ...
        let _ = m.fit_transform(x).expect("warmup");
    }
    let mut times = Vec::with_capacity(RUNS);
    for _ in 0..RUNS {
        let t = Instant::now();
        let mut m = MinMaxScaler::new();
        let _z = m.fit_transform(x).expect("fit_transform");
...
ts = []
m = MinMaxScaler(); _ = m.fit_transform(X)
for _ in range({runs}):
    t = time.perf_counter()
    m = MinMaxScaler(); _ = m.fit_transform(X)
...
fn beat_sklearn_minmax_speed() {
    ...
    eprintln!(
        "BEAT-SKLEARN-MINMAXSCALER-SPEED: apr={apr_ms:.3}ms sklearn={sklearn_ms:.3}ms \
         ratio={ratio:.3} (apr {:.2}x faster) on {N_SAMPLES}x{N_FEATURES}, median of {RUNS}",
        sklearn_ms / apr_ms
    );
    assert!(
        ratio <= RATIO_CEILING,
        "FALSIFY: ratio {ratio:.3} > {RATIO_CEILING:.2} (apr={apr_ms:.3}ms, sklearn={sklearn_ms:.3}ms)"
    );
}
