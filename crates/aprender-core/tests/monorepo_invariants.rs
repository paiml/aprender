...
// ===========================================================================
// FALSIFY-GGML-001/002 (PMAT-3430): the ggml traits fixture is pinned
//
// Operator ruling 2026-09-20, clause 2a: the TRAITS fixture records the sha it
// was extracted from, and a test asserts that sha still equals
// scripts/llama_pin.toml `build_commit`. A pin bump therefore turns this RED
// until the fixture is regenerated by the committed extraction command — the
// bump's cost is visible in the BUMP's PR, not discovered later. #3563 asks for
// the general form of this across every pin-derived artifact in the tree.
//
// WHY IT LIVES HERE AND NOT IN aprender-quant. trueno_quant is published; a
// packaged crate has no scripts/llama_pin.toml, so a test there could only
// skip — and a skip is how this kind of check becomes decoration. This file is
// already wired into CI (ci/explicit-test-commands.d/030-*), so it is not a
// dark target.
// ===========================================================================

/// Read one quoted key out of scripts/llama_pin.toml, the same narrow parse
/// `llama_pin_get` does in shell. tomllib is 3.11+ and the runner is 3.10, which
/// is why the shell side hand-parses; this side does not get to differ.
fn llama_pin_value(key: &str) -> String {
    let path = workspace_root().join("scripts/llama_pin.toml");
    let text = std::fs::read_to_string(&path)
        .unwrap_or_else(|e| panic!("scripts/llama_pin.toml must be readable: {e}"));
    ...
    panic!("scripts/llama_pin.toml declares no `{key}`");
}

fn ggml_fixture() -> serde_json::Value {
    let path = workspace_root().join("crates/aprender-quant/fixtures/ggml_traits.json");
    let text = std::fs::read_to_string(&path)
        .unwrap_or_else(|e| panic!("the ggml traits fixture must be readable: {e}"));
    serde_json::from_str(&text).expect("the ggml traits fixture must be valid JSON")
}

/// FALSIFY-GGML-001: the fixture was extracted at the pin of record.
#[test]
fn falsify_ggml_001_traits_fixture_records_the_current_pin() {
    let pin = llama_pin_value("build_commit");
    let fixture = ggml_fixture();
    ...
    assert_eq!(
        recorded, pin,
        "crates/aprender-quant/fixtures/ggml_traits.json was extracted at {recorded}, but \
         scripts/llama_pin.toml build_commit is now {pin}. Regenerate it with \
         `bash scripts/extract_ggml_traits.sh` against a checkout at {pin} and commit the \
         result in the SAME PR as the bump — every TRAITS row is an upstream reading, and a \
         bump may change any of them."
    );

    // The full sha the extractor resolved must still be the pin's prefix: the
    // pin is abbreviated, and an abbreviation that stopped matching would mean
    // the fixture and the pin name different commits.
...
#[test]
fn falsify_ggml_002_traits_fixture_is_the_whole_id_space() {
    let fixture = ggml_fixture();
    let types = fixture["types"]
        .as_array()
        .expect("fixture has no types array");
    let count = fixture["ggml_type_count"]
        .as_u64()
        .expect("no ggml_type_count");
    let live = fixture["live_count"].as_u64().expect("no live_count");
    let removed = fixture["removed_count"].as_u64().expect("no removed_count");

    assert_eq!(
        types.len() as u64,
        count,
        "the fixture lists {} of {count} ids",
        types.len()
    );
    assert_eq!(
        live + removed,
        count,
        "live {live} + removed {removed} != {count}"
    );

    // Ids are dense and in order, because TRAITS is indexed by id.
    for (i, row) in types.iter().enumerate() {
        assert_eq!(
            row["id"].as_u64(),
            Some(i as u64),
            "the fixture's rows are not in dense id order at position {i}"
        );
    }
}

/// FALSIFY-GGML-003: the extractor's own case table passes.
///
/// `scripts/extract_ggml_traits_selftest.sh` proves the live/Removed
/// classification refuses the four ways it can be wrong, on synthetic inputs —
/// no llama.cpp checkout, no compiler. It is invoked from HERE rather than from
/// a new ci.yml step because this target is already wired
/// (ci/explicit-test-commands.d/030-*), and a case table nothing runs is the
/// thing it was written to prevent.
#[test]
fn falsify_ggml_003_extractor_case_table_passes() {
    let root = workspace_root();
    let script = root.join("scripts/extract_ggml_traits_selftest.sh");
    assert!(
        script.is_file(),
        "scripts/extract_ggml_traits_selftest.sh is missing"
    );
    ...
    let out = Command::new("bash")
        ...
        .current_dir(&root)
        .output()
        .expect("the extractor self-test must be runnable");

    let stdout = String::from_utf8_lossy(&out.stdout);
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        out.status.success(),
        "extract_ggml_traits_selftest.sh failed ({}):\n{stdout}\n{stderr}",
        out.status
    );
    // A case table that runs zero cases passes vacuously. Require that it says
    // how many it ran, and that the count is not zero.
    let ran = stdout
        .lines()
        .find_map(|l| l.strip_suffix(" case(s), 0 failure(s)"))
