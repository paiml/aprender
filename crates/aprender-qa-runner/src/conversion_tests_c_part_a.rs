...
fn test_conversion_failure_type_serde() {
    ...
    for ft in types {
        let json = serde_json::to_string(&ft).expect("serialise to string");
        let parsed: ConversionFailureType = serde_json::from_str(&json).expect("parse JSON");
        assert_eq!(parsed, ft);
    }
}
...
fn test_quant_type_serde() {
    ...
    for qt in types {
        let json = serde_json::to_string(&qt).expect("serialise to string");
        let parsed: QuantType = serde_json::from_str(&json).expect("parse JSON");
        assert_eq!(parsed, qt);
    }
}
...
fn test_tensor_naming_serde() {
    ...
    for tn in &variants {
        let json = serde_json::to_string(tn).expect("serialise to string");
        let parsed: TensorNaming = serde_json::from_str(&json).expect("parse JSON");
        assert_eq!(parsed, *tn);
    }
}
...
fn test_resolve_model_path_apr_cache_structure() {
    ...
    std::fs::create_dir_all(&safetensors_dir).expect("create dir");
    ...
    assert!(result.is_ok());
    assert_eq!(result.expect("call under test succeeds"), model_file);
}
...
fn test_find_hf_snapshot_found() {
    ...
    let snapshot = tmp
        ...
        .join("snapshots")
        .join("abc123");
    std::fs::create_dir_all(&snapshot).expect("create dir");
    std::fs::write(snapshot.join("model.safetensors"), b"fake").expect("write file");

    let result = find_hf_snapshot(tmp.path(), "Test", "Model");
    assert!(result.is_some());
    assert_eq!(result.expect("call under test succeeds"), snapshot);
}
...
fn test_find_apr_cache_found() {
    ...
    assert!(result.is_some());
    assert_eq!(result.expect("call under test succeeds"), apr_cache);
}
