impl Executor {
    ...
    fn run_golden_rule_test(&mut self, model_path: &Path, model_id: &ModelId) -> (usize, usize) {
        ...
        let resolved_path = match resolve_model_path(model_path, aprender_qa_gen::Format::SafeTensors) {
            Ok(p) => p,
            Err(e) => {
                let ev = Evidence::falsified(
                    "F-GOLDEN-RULE-001",
    ...
    fn golden_scenario(model_id: &ModelId) -> aprender_qa_gen::QaScenario {
        aprender_qa_gen::QaScenario::new(
            model_id.clone(),
            aprender_qa_gen::Modality::Run,
            aprender_qa_gen::Backend::Cpu,
            aprender_qa_gen::Format::Apr,
