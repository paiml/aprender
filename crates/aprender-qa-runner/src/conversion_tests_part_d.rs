...
fn test_find_hf_snapshot_found() {
    ...
    assert_eq!(result.expect("call under test succeeds"), snapshot_dir);
}
...
fn test_find_hf_snapshot_sharded() {
    ...
    std::fs::write(snapshot_dir.join("model.safetensors.index.json"), "{}").expect("write file");
    std::fs::write(
...
fn test_resolve_with_dirs_sharded_hf_cache() {
    let hf_cache = tempfile::tempdir().expect("create temp dir");
    ...
    std::fs::write(snapshot.join("model.safetensors.index.json"), "{}").expect("write file");
    std::fs::write(snapshot.join("model-00001-of-00002.safetensors"), "s1").expect("write file");
...
fn test_find_apr_cache_found() {
    ...
    let result = find_apr_cache(dir.path(), "Qwen", "Qwen2.5-Coder-0.5B");
    assert!(result.is_some());
    assert_eq!(result.expect("call under test succeeds"), cache_dir);
}
...
fn test_resolve_with_dirs_hf_cache() {
    ...
    let snapshot = hf_cache
        ...
        .join("abc123");
    std::fs::create_dir_all(&snapshot).expect("create dir");
    std::fs::write(snapshot.join("model.safetensors"), "fake").expect("write file");
...
fn test_resolve_with_dirs_apr_cache() {
    ...
    let result =
        resolve_hf_repo_with_dirs("Qwen/Qwen2.5-Coder-0.5B", hf_cache.path(), home.path());
    assert!(result.is_ok());
    assert_eq!(result.expect("call under test succeeds"), apr_dir);
}
...
fn test_resolve_with_dirs_hf_preferred() {
    ...
    let snapshot = hf_cache
        ...
        .join("abc");
    std::fs::create_dir_all(&snapshot).expect("create dir");
    std::fs::write(snapshot.join("model.safetensors"), "hf").expect("write file");
