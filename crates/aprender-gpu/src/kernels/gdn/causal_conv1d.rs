...
mod ptx_tests {
    ...
    /// committed under `experiments/cuda-oxide/causal-conv1d/baseline-ptx/`.
    /// `channels` and `kernel_size` are baked into the PTX, so there is one
    /// baseline per timed width (K = 4, Qwen3.5's `conv_kernel`). If this emitter
    /// changes and the baselines do not, this golden check fails.
    ///
    /// Regenerate: `APR_BLESS_PTX=1 cargo test -p aprender-gpu --features cuda --lib gdn_conv1d_ptx_golden`
    #[test]
    fn gdn_conv1d_ptx_golden() {
        let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../experiments/cuda-oxide/causal-conv1d/baseline-ptx");
        let bless = std::env::var_os("APR_BLESS_PTX").is_some();
        for channels in [2048u32, 4096, 6144] {
            let kernel = CausalConv1dSiluKernel::new(channels, 4);
            for target in ["sm_89", "sm_121"] {
                let path = dir.join(format!("gdn_causal_conv1d_silu_c{channels}.{target}.ptx"));
