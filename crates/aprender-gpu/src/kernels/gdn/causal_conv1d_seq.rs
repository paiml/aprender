...
mod ptx_tests {
    ...
    /// committed under `experiments/cuda-oxide/causal-conv1d-seq/baseline-ptx/`.
    /// `channels`, `kernel_size` and both row strides are baked into the PTX, so there
    /// is one baseline per timed width (K = 4, strides = channels, as
    /// `qwen35_conv1d_rows` builds it). If this emitter changes and the baselines do
    /// not, this golden check fails.
    ///
    /// Regenerate: `APR_BLESS_PTX=1 cargo test -p aprender-gpu --features cuda --lib gdn_conv1d_seq_ptx_golden`
    #[test]
    fn gdn_conv1d_seq_ptx_golden() {
        let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../experiments/cuda-oxide/causal-conv1d-seq/baseline-ptx");
        let bless = std::env::var_os("APR_BLESS_PTX").is_some();
        for channels in [2048u32, 4096, 6144] {
            let kernel = CausalConv1dSiluSeqKernel::new(channels, 4, channels, channels);
            for target in ["sm_89", "sm_121"] {
                let path = dir.join(format!(
                    "gdn_causal_conv1d_silu_seq_c{channels}.{target}.ptx"
                ));
