...
#[cfg(feature = "cuda")]
// Both variants are ~2 KB (2288 vs 2040 bytes) and there is one per layer, never moved in a hot loop:
// boxing the larger saves nothing and changes every match site (#3837).
...
impl CudaNf4TransformerBlock {
    ...
    pub(crate) fn forward(
        ...
        if let Some(w_q_fp16) = self.w_q_fp16.as_ref().filter(|_| fp16_gemm) {
            let f16_act = scratch.norm1_out_f16.as_ref().unwrap();
            gemm_f16_to_f32_forward(f16_act, w_q_fp16, &mut scratch.q,
        ...
        if let Some(w_k_fp16) = self.w_k_fp16.as_ref().filter(|_| fp16_gemm) {
            let f16_act = scratch.norm1_out_f16.as_ref().unwrap();
            gemm_f16_to_f32_forward(f16_act, w_k_fp16, &mut scratch.k,
