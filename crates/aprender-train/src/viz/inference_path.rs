//! Entrenar inference monitoring visualization.
//!
//! Provides visualization extensions for decision paths and audit trails.
//!
//! # Provenance (APR-MONO §S, #1978)
//!
//! This module was re-homed from `aprender-viz` (`trueno_viz::interop::entrenar`).
//! In the self-contained monorepo DAG, `aprender-train` (`entrenar`) depends on
//! `aprender-viz` (`trueno_viz`) for training plots — the correct direction. The
//! reverse edge (viz → entrenar) closed a `train ↔ viz` cycle and was dropped in
//! #1975. Because these visualizations consume `entrenar::monitor::inference`
//! types, their natural home is *inside* `aprender-train`, which already depends
//! on `trueno_viz`. No cycle: `train → viz`.
//!
//! # Features
//!
...
//! use entrenar::viz::inference_path::DecisionPathViz;
//!
...
fn tree_path_to_graph(
    splits: &[TreeSplit],
    leaf: &crate::monitor::inference::path::LeafInfo,
    width: u32,
    height: u32,
) -> Result<Framebuffer> {
    if splits.is_empty() {
...
mod tests {
    use super::*;
    use crate::monitor::inference::path::LeafInfo;
