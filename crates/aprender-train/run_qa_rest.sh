...
# SEC001: run the command as an explicit argv array instead of building and
# `eval`-executing a string — every call site below already passes the
# command as separate words (an env-var prefix goes through
# `env VAR=val cmd ...` instead of a re-parsed shell string).
run_test() {
    id=$1
    desc=$2
    shift 2
...
run_test "68" "Env Var" env DATA_DIR=/tmp/data cargo run --example mnist_train
