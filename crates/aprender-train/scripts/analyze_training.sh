...
echo "🔍 Analyzing LLaMA Training Profile"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
echo -e "${BLUE}═══ ML Anomaly Detection Results ══${NC}"
echo ""
...
if [[ "$SILHOUETTE" != "N/A" ]]; then
    ...
    if (( $(echo "$SCORE > 0.7" | bc -l) )); then
        echo -e "  ${GREEN}✓ Excellent cluster separation${NC}"
    elif (( $(echo "$SCORE > 0.5" | bc -l) )); then
        echo -e "  ${YELLOW}~ Good cluster separation${NC}"
    else
        echo -e "  ${RED}✗ Poor cluster separation - noisy data${NC}"
    fi
fi
...
# Outlier detection
echo -e "${RED}═══ Outlier Detection ══${NC}"
echo ""
...
if [[ "$OUTLIER_COUNT" -gt 0 ]]; then
    ...
    echo ""
    jq -r '.ml_analysis.outliers[] | "  - \(.syscall): \(.latency_us)μs (z-score: \(.z_score))"' "$PROFILE_FILE" 2>/dev/null || echo "Error parsing outliers"
else
    echo -e "${GREEN}✓ No outliers detected${NC}"
fi

echo ""

# Real-time anomaly summary
echo -e "${BLUE}═══ Real-Time Anomaly Summary ══${NC}"
echo ""
...
echo -e "  ${RED}🔴 High (>5.0σ):${NC}   $HIGH_ANOMALIES (likely hardware issues)"
echo -e "  ${YELLOW}🟡 Medium (4-5σ):${NC}  $MEDIUM_ANOMALIES (investigate)"
echo -e "  ${GREEN}🟢 Low (3-4σ):${NC}    $LOW_ANOMALIES (noise)"
...
if [[ "$HIGH_ANOMALIES" != "N/A" && "$HIGH_ANOMALIES" -gt 0 ]]; then
    echo -e "${RED}⚠️  High severity anomalies detected!${NC}"
...
if [[ "$OUTLIER_COUNT" -gt 5 ]]; then
    echo -e "${YELLOW}⚠️  Multiple outliers detected${NC}"
...
if [[ "$SILHOUETTE" != "N/A" ]]; then
    ...
    if (( $(echo "$SCORE < 0.5" | bc -l) )); then
        echo -e "${YELLOW}⚠️  Poor clustering quality${NC}"
