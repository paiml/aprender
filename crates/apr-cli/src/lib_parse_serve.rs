        ...
        .spawn(|| {
            ...
            let possible_values = |path: &[&str]| -> Vec<String> {
                ...
                let arg = cmd
                    ...
                    .unwrap_or_else(|| panic!("`--backend` not found on `{}`", path.join(" ")));
            ...
            let expected: Vec<String> = crate::BACKEND_VALUES
                .iter()
                .map(|s| (*s).to_string())
                .collect();
...
fn test_finetune_accepts_its_own_gpu_backends_2583() {
    for value in crate::FINETUNE_GPU_BACKEND_VALUES {
        let cli = parse_cli(vec!["apr", "finetune", "model.apr", "--gpu-backend", value])
            .unwrap_or_else(|e| panic!("`apr finetune --gpu-backend {value}` must parse: {e}"));
        match *cli.command {
...
/// E8 R-1b: `--recipe` supplies model/method/rank/data/epochs/lr, so an
/// explicit flag for any of them beside it is a conflict, not a silent
/// override in either direction.
#[cfg(feature = "training")]
#[test]
fn test_finetune_recipe_conflicts_with_the_flags_it_supplies() {
    let cli = parse_cli(vec!["apr", "finetune", "--recipe", "r.yaml"])
        .expect("`apr finetune --recipe r.yaml` must parse");
    match *cli.command {
        Commands::ModelOps(ModelOpsCommands::Finetune { ref recipe, .. }) => {
            assert_eq!(recipe.as_deref(), Some(std::path::Path::new("r.yaml")));
        }
        _ => panic!("Expected Finetune command"),
    }
    for extra in [
        ...
        vec!["--data", "d.jsonl"],
        vec!["--epochs", "1"],
        vec!["--learning-rate", "0.1"],
    ] {
        let mut argv = vec!["apr", "finetune", "--recipe", "r.yaml"];
        argv.extend(extra.iter().copied());
        let err = parse_cli(argv).expect_err("recipe + covered flag must not parse");
        assert_eq!(
            err.kind(),
