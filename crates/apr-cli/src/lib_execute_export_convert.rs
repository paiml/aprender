        ...
        let built: Vec<(String, Option<String>, bool)> = shapes
            ...
            .map(|(name, argv)| {
                ...
                let (run_prompt, chat_template) =
                    run_prompt_and_chat(
                    prompt.as_ref().map(batuta_common::cli_roles::PromptText::as_string),
                    positional.as_ref().map(batuta_common::cli_roles::PromptText::as_string),
                    &source,
                    chat,
                );
                ...
                let config = crate::commands::run::realizar_config(
                    ...
                    input.as_deref().map(std::path::Path::to_path_buf).as_ref(),
                    &options,
                )
                .expect("config");
