...
#[cfg(test)]
mod run_preflight_tests {
    use super::*;

    /// GH-326 precedence at the surface #2696 was MEASURED through. S3b (#3041)
    /// lifted `apr run`'s preflight out of a 60-line match arm into
    /// `run_preflight`; `effective_no_gpu` is the only value that arm produced,
    /// so this is what keeps the decomposition from quietly changing which runs
    /// are CPU runs. Every case here asks for CPU or nothing, so none of them
    /// can refuse on any host.
    #[test]
    fn the_preflight_returns_the_effective_cpu_decision() {
        assert!(
            !run_preflight(false, false, None).expect("a default run never refuses"),
            "no flag: not a forced cpu run"
        );
        assert!(
            run_preflight(false, true, None).expect("--no-gpu never refuses"),
            "--no-gpu is a cpu run"
        );
        assert!(
            run_preflight(false, false, Some("cpu")).expect("--backend cpu never refuses"),
            "--backend cpu is a cpu run"
        );
    }

    /// `--gpu` wins over `--no-gpu`, so the run is NOT a cpu run — or, on a
    /// build/host the registry says cannot honour it, the whole request is
    /// refused. Asserting both is what makes this test say something on a CPU
    /// box and on a GPU box; a silent cpu run is exactly #2696.
    #[test]
    fn gpu_beats_no_gpu_or_the_request_is_refused_never_downgraded() {
        match run_preflight(true, true, None) {
            Ok(effective_no_gpu) => assert!(
                !effective_no_gpu,
                "--gpu overrides --no-gpu; it must not resolve to a cpu run"
            ),
            Err(e) => assert!(
                e.to_string().contains("--gpu"),
                "the refusal quotes the flag the user typed: {e}"
            ),
        }
    }
}
