...
pub fn run(args: AwqLintArgs) -> crate::error::Result<()> {
    let path = Path::new(&args.observation_file);
    let obs = lint_input::read_json_observation("apr awq-lint", path)?;
    ...
    if reports.is_empty() {
        return Err(CliError::ValidationFailed(
            "FALSIFY-CRUX-B-08: observation has none of quality/compression/flags".into(),
        ));
    }
