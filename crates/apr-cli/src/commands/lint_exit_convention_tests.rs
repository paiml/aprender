...
fn family() -> Vec<Member> {
    vec![
        ...
        Member {
            ...
            json_input: true,
            // `--request-file` is REQUIRED in non-streaming mode: the tool-name
            // allowlist has nothing to check a response against without the
            // request that declared the tools. Passing None made this member
            // fail that argument guard (exit 5) before it ever tried to parse,
            // so the parse-failure exit code it is here to assert was never
            // exercised. The same fixture file serves as both.
            invoke: |p| super::ollama_tools_lint::run(p, Some(p), false, false),
        },
        Member {
