...
mod tests {
    ...
    fn every_converted_lint_gives_four_input_classes_four_exit_codes() {
        ...
        for (name, missing, isdir, malformed_code, gate) in rows {
            assert_eq!(
                missing,
                ExitCode::from(3),
                "{name}: missing observation must be exit 3"
            );
            assert_eq!(
                isdir,
                ExitCode::from(7),
                "{name}: a directory must be exit 7"
            );
            assert_eq!(
                malformed_code,
                ExitCode::from(4),
            ...
            assert_eq!(
                gate,
                ExitCode::from(5),
                "{name}: a gate verdict must be exit 5"
            );
            // The property that actually matters, stated without naming numbers:
            // no two input classes may be confusable.
            let codes = [missing, isdir, malformed_code, gate];
