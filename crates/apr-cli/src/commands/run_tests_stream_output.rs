...
fn stream_output_emits_n_plus_one_json_lines() {
    ...
    write_stream_output(&mut buf, &result, "model.gguf", 32, false, None).expect("write must succeed");
...
fn stream_token_events_carry_their_own_decoded_text() {
    ...
    let mut buf: Vec<u8> = Vec::new();
    write_stream_output(&mut buf, &result, "model.gguf", 8, false, None).expect("write must succeed");
    let s = String::from_utf8(buf).expect("utf-8");
    let lines: Vec<&str> = s.lines().collect();
...
fn stream_token_events_degrade_to_empty_text_without_a_tokenizer() {
    ...
    let mut buf: Vec<u8> = Vec::new();
    write_stream_output(&mut buf, &result, "model.apr", 8, false, None).expect("write");
    let s = String::from_utf8(buf).expect("utf-8");
    let lines: Vec<&str> = s.lines().collect();
...
fn stream_output_no_tokens_emits_only_final() {
    ...
    let mut buf: Vec<u8> = Vec::new();
    write_stream_output(&mut buf, &result, "noprompt.apr", 1, false, None).expect("write must succeed");
    let s = String::from_utf8(buf).expect("utf-8");
    let lines: Vec<&str> = s.lines().collect();
...
fn stream_output_none_tokens_emits_only_final() {
    ...
    let mut buf: Vec<u8> = Vec::new();
    write_stream_output(&mut buf, &result, "x.apr", 1, false, None).expect("write");
    let s = String::from_utf8(buf).expect("utf-8");
...
fn build_final_json_matches_legacy_json_shape() {
    ...
    let v = build_final_json(&result, "src.apr", 100, false, None);
