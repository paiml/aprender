...
fn serve_used_gpu(bin: &str, model: &str) -> bool {
    ...
    let body = r#"{"model":"default","prompt":"Hi","max_tokens":4,"temperature":0}"#;
    let deadline = Instant::now() + Duration::from_secs(300);
    loop {
        if let Some(resp) = http_post(port, "/v1/completions", body) {
            let v: serde_json::Value = serde_json::from_str(&resp)
                .unwrap_or_else(|e| panic!("/v1/completions is not JSON ({e}): {resp}"));
            return used_gpu_of(&v).unwrap_or_else(|| panic!("/v1/completions carries no used_gpu: {v}"));
        }
        assert!(Instant::now() < deadline, "apr serve never answered /v1/completions within 300 s");
...
fn falsify_4089_serve_and_run_take_the_same_default_backend() {
    let (Ok(bin), Ok(model)) = (std::env::var("APR_BIN"), std::env::var("APR_FALSIFY_MODEL")) else {
