...
The repair records the coverage the run achieved, per surface, and rows 23/24 are the
pair that keeps the record honest in **both** directions:

| row | coverage | verdict | expected |
|---|---|---|---|
| 23 | `shell/python/config/docs/other: none` | `PASS` | **RED B1** — an unsearched surface must not read clean |
| 24 | the SAME map | `DEGRADED` | **GREEN** — discrimination case |

Row 24 is not decoration. Without it the rule would punish the honest receipt exactly as
