...
- **`llama-cli` does not accept `-no-cnv`, and it did not at the old pin either.** `common/arg.cpp` registers `-cnv`/`-no-cnv` only for `LLAMA_EXAMPLE_COMPLETION`, so `llama-cli` rejects it with `error: invalid argument: -no-cnv` (rc=1, before any model is read). Raw completion is `llama-completion`. `-st` / `--single-turn` is **not** a replacement: it runs a single-turn *conversation* and applies the chat template, which changes the tokens a parity test compares. The bump did **not** introduce this. The old pin's `llama-cli` (`39173bcac`), given the same flags, printed `--no-conversation is not supported by llama-cli, please use llama-completion instead` in a loop until `timeout` killed it: rc=124, 532,293,836 bytes of stdout. The orchestrator's first re-run hit the rejection and it looked like a load failure.
- **At this commit, `--log-disable` empties `llama-completion`'s stdout.** With the parity test's flags plus `--log-disable`, `llama-completion` exits rc=0 and writes 0 bytes, and the prompt is not echoed even without `--no-display-prompt`. Without `--log-disable`, the same `-n 1` run writes the generated token. A caller that keeps `--log-disable` would compare against an empty string.
