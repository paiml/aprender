...
- **intel** (`mac-server`, Xeon W-3245, AVX-512, glibc 2.35), in `~/parity-ref/emulation-3091/` and `/tmp/emul3091/` only:
  - fixture harness at 21:05:57Z, load 36.66/38.98/53.16; rerun at 21:06:33Z, load 30.28/37.01/51.90; post-refactor rerun at 21:35:35Z, load 36.06/44.59/40.73 (`build_and_run.transcript` is the last);
...
**Fixtures** are generated by calling ggml's C directly. `emulation/ggml_emul_fixtures.c` (sha256 `bf56215b…32be80ad`; the first run used `80518776…77b29aaf`, whose `gen_input` was split for the pre-commit complexity gate and rerun at 21:35:35Z with byte-identical `fixtures.*`, `refactor_rerun.transcript`) is linked against intel's `libggml-cpu.so`/`libggml-base.so` by `emulation/build_and_run.sh`. There are 10 cases:
...
| `logits_gap.py` | `d1795868…0257fc1af` (first run `ea30daa6…eee7f42e`; `main` split for the complexity gate, all 5 `gap-*.tsv` reproduced byte for byte, `refactor_rerun.transcript`) |
