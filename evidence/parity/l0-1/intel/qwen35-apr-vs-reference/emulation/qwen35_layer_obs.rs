...
//!
//! PMAT-3091 emulation pass: prints the APR_EMULATE_GGML_VECDOT mask and the emulated-matmul counters at exit.
//!
...
fn main() -> Res<()> {
    ...
    // PMAT-3091 mechanism proof: the switch as read, and how many matmuls it routed through the emulation.
    let mask = realizar::quantize::ggml_vecdot_emul::emulation_mask()?;
    let counts = realizar::quantize::ggml_vecdot_emul::emulated_matmul_counts();
    println!("qwen35_layer_obs: ggml_vecdot_emul mask={mask:?} emulated_matmuls[Q4_K,Q5_K,Q6_K,Q8_0]={counts:?}");
