...
gpu-q --prio 1 -- bash -c '
  ...
  llama-server -m '"$M"' -ngl 99 -c 40960 --jinja --port 18261 -np 1 > out/llama-server-margin.log 2>&1 & sp=$!
  for _ in $(seq 1 120); do curl -sf http://127.0.0.1:18261/health >/dev/null && break; sleep 1; done
  python3 margin.py > out/margin.log 2>&1; orc=$?
  kill "$sp"; wait "$sp"; exit $orc'
echo "margin rc=$?" >> out/runs.log
