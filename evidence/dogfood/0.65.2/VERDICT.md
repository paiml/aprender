...
`check_multiplatform_dogfood.sh` on these receipts: install rows ok on all four hosts; bench rows ok on lambda and intel (REPORT on gx10 and mini: `apr bench` measured 7.7 and 8.1 tok/s, below its own H12 floor of 10, so no block); parity rows FAIL on every host — lambda because the 4090 host needs a cuda lane and the default-feature binary resolves 0 GPU layers, intel/gx10/mini because the run could not emit a block.
