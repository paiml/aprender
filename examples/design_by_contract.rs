...
fn main() {
    ...
    println!(
        "   Q4_K: {} bytes/block ({} elements/block)",
        block_sizes::Q4_K,
        block_sizes::QK_K
    );
    println!(
        "   Q5_K: {} bytes/block ({} elements/block)",
        block_sizes::Q5_K,
        block_sizes::QK_K
    );
    println!(
        "   Q6_K: {} bytes/block ({} elements/block)",
        block_sizes::Q6_K,
        block_sizes::QK_K
    );
    ...
    if qwen35_path.exists() {
        ...
        println!(
            "   Family: {} (vendor: {})",
            family.display_name, family.vendor
        );
        ...
        if let Some(variant) = family.size_variants.get("9b") {
            println!("   9B variant:");
            println!(
                "     hidden_dim={}, head_dim={} (explicit, not hidden/heads)",
                variant.hidden_dim, variant.head_dim
            );
            println!(
                "     num_heads={}, num_kv_heads={} (GQA ratio: {}:1)",
                variant.num_heads,
                variant.num_kv_heads,
                variant.num_heads / variant.num_kv_heads
            );
            println!(
                "     vocab_size={}, max_pos_embed={}",
            ...
            println!(
                "     Invariant: {}*{} = {} == hidden_dim (PASS)",
                variant.num_heads, variant.head_dim, computed
            );
        }

        println!(
            "   Constraints: has_bias={}, activation={:?}, mlp={:?}",
            family.constraints.has_bias, family.constraints.activation, family.constraints.mlp_type
        );
    } else {
    ...
    println!(
        ...
        ft_config.hidden_size, ft_config.num_hidden_layers
    );
    println!(
        "   num_heads={}, num_kv_heads={} (GQA {}:1)",
        ft_config.num_attention_heads,
        ft_config.num_kv_heads,
    ...
    println!(
        "   use_bias={} (KEY: no attention bias)",
        ft_config.use_bias
    );
    ...
    if qwen35_path.exists() {
        ...
        if let Some(variant) = family.size_variants.get("9b") {
            assert_eq!(
                ft_config.hidden_size, variant.hidden_dim,
                "hidden_size mismatch: config vs contract"
            );
            assert_eq!(
                ft_config.vocab_size, variant.vocab_size,
            ...
            assert_eq!(
                ft_config.num_hidden_layers, variant.num_layers,
    ...
    println!(
        "   LoRA targets: q_proj [{q_proj_dim}, {}], v_proj [{kv_proj_dim}, {}]",
