...
| Self-parity is blind to what CPU and GPU share (tokenizer, template) | token ids vs pinned `llama-tokenize` (`scripts/tokenizer_parity.sh`); CRUX: same model + prompts through apr, llama.cpp, ollama AND Hugging Face `transformers` (pinned `uv run --frozen`) for every derived verb, apr right wherever a competitor is right; HF tokenizer ids and `apply_chat_template` rendering byte-equal | pre-publish | #3726, #3739 |
