...
| Self-parity is blind to what CPU and GPU share (tokenizer, template) | token ids vs pinned `llama-tokenize` (`scripts/tokenizer_parity.sh`); CRUX: same model + prompts through apr, llama.cpp, ollama, Hugging Face `transformers` (pinned `uv run --frozen`) AND llamafile (pinned, sha256-verified) for every derived verb, apr right wherever a competitor is right; HF tokenizer ids and `apply_chat_template` rendering byte-equal | pre-publish | #3726, #3739 |
...
| A receipt named what it inferred (ladder `sha` = worktree HEAD, not the binary it ran) | the judge requires the receipt's binary `--version` sha to equal the pinned binary | ladder judge | #3771 |
