...
| Provable contracts | **<!-- CONTRACT_COUNT_START -->1847<!-- CONTRACT_COUNT_END -->** provable contracts | `contracts/census.json` `.n_files` — the set `pv lint` walks (`pv census`, ONT-001 ONT-1; regenerated by `make contracts`, written by `make readme-sync`, guarded by `scripts/check_readme_claims.sh`) |
| CLI commands | **112** CLI commands | `contracts/apr-cli-commands-v1.yaml` §`commands` (parse the list; `apr --help` prints 113 because it lists `help` itself, and `grep -c '^  - name:'` gives 118 — other same-indent `name:` keys exist in the file) |
...
The tree carries <!-- CONTRACT_COUNT_START -->1847<!-- CONTRACT_COUNT_END --> contracts across inference, training, quantization, attention, FFN,
