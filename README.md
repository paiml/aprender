...
| Provable contracts | **<!-- CONTRACT_COUNT_START -->1833<!-- CONTRACT_COUNT_END -->** provable contracts | `contracts/census.json` `.n_files` — the set `pv lint` walks (`pv census`, ONT-001 ONT-1; regenerated by `make contracts`, written by `make readme-sync`, guarded by `scripts/check_readme_claims.sh`) |
...
**0.69.1 was certified under an operator emergency scope.** The release gate for `apr 0.69.1`
(tag `v0.69.1` → `d8a6df53a`) was a **CRUX smoke on both GPU hosts**, with apr against
llama.cpp, ollama, vLLM and Hugging Face on the same weights:
- lambda (RTX 4090): **134/134 GREEN**.
- gx10 (GB10): **134/134 GREEN**.

Per model:
- Qwen3.5-2B-Q4_K_M: thinking-off 32/32 per host.
- Qwen3.5-4B-Q4_K_M: 32/32 off + 19/19 on per host.
- Qwen3.5-4B-UD-Q4_K_XL: 32/32 off + 19/19 on per host.

MoE Qwen3-30B-A3B: 92/92 positions, cosine 1.000000, on both hosts. The full model ladder
moved to the nightly run (#4040) and did **not** gate this tag.

**The published binaries themselves** (the v0.69.1 release assets, sha256-verified) pass all
5 [apr-cookbook `apr` recipes](https://github.com/paiml/apr-cookbook/pull/452) on
both hosts: MoE on CUDA, `--thinking on/off`, the `apr code` JSON envelope and
`apr capability`. That is 10/10 receipts, bound to each asset's sha256, because the binaries
print `v0.69.1+no-git` (#4110).

The matrix below is rendered from the most recent **ladder** receipts, and its footer names
the binary that produced them. That binary (`9b7739951`) is **not** the release binary. The
matrix re-renders when the nightly ladder runs on `d8a6df53a`. Its qwen3moe `FAIL` rows
predate the CUDA MoE path this release ships (#3714, #3987).
...
| `qwen3-1.7b-q4km` | pass | pass |
| `qwen3-8b-q4km` | pass | pass |
| `qwen35-0.8b-q4km` | pass | pass |
...
The tree carries <!-- CONTRACT_COUNT_START -->1833<!-- CONTRACT_COUNT_END --> contracts across inference, training, quantization, attention, FFN,
