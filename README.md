...
| Provable contracts | **<!-- CONTRACT_COUNT_START -->1839<!-- CONTRACT_COUNT_END -->** provable contracts | `contracts/census.json` `.n_files` — the set `pv lint` walks (`pv census`, ONT-001 ONT-1; regenerated by `make contracts`, written by `make readme-sync`, guarded by `scripts/check_readme_claims.sh`) |
...
The tree carries <!-- CONTRACT_COUNT_START -->1839<!-- CONTRACT_COUNT_END --> contracts across inference, training, quantization, attention, FFN,
