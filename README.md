...
| Provable contracts | **<!-- CONTRACT_COUNT_START -->1831<!-- CONTRACT_COUNT_END -->** provable contracts | `contracts/census.json` `.n_files` — the set `pv lint` walks (`pv census`, ONT-001 ONT-1; regenerated by `make contracts`, written by `make readme-sync`, guarded by `scripts/check_readme_claims.sh`) |
...
**0.69.1 was certified under an operator emergency scope.** The release gate for
`apr 0.69.1 (d8a6df53a)` was a **CRUX smoke on both GPU hosts** (lambda: RTX 4090;
gx10: GB10). apr was checked against llama.cpp, ollama, vLLM and Hugging Face on the same
weights, on Qwen3.5-2B, Qwen3.5-4B and Qwen3.5-4B-UD-Q4_K_XL: TODO-CELL:crux-smoke-summary.
The full model ladder moved to the nightly run and did **not** gate this tag.

The matrix below is rendered from the most recent ladder receipts. Its footer names the
binary that produced them, and that binary is **not** the release binary until the nightly
ladder re-runs on `d8a6df53a` and this section is re-rendered. Read a `FAIL` there as the
state at that sha: the qwen3moe rows predate the CUDA MoE path this release ships (#3714,
#3987).
...
The tree carries <!-- CONTRACT_COUNT_START -->1831<!-- CONTRACT_COUNT_END --> contracts across inference, training, quantization, attention, FFN,
